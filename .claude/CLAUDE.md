...
- 除极个别特殊情况外，始终 **强制**与 Codex 协作（SKILLs形式`python3 /path/to/scripts/*.py --cd "/path/to/project" --PROMPT "Analyze..." [OPTIONS]`，**Run in the background**，**不设置** timeout）。-
- 用户当前正在开发的项目是`RC_2026_1` ,只需要阅读`RC_2026_1`文件夹下的内容
- 执行编译的优先级放在最后，统一交给用户来编译验证
- 多模型运行时，禁止主动关闭codex的线程，只有等到他回复了内容才可以进行下一步动作
----
## 0. Core Instruction
...
### 0.2 异步操作（原子循环）
- **强制并行**。对于任何涉及 Codex 或长时间运行脚本的任务，必须使用 **Run in the background**（**不设置** timeout）。
- **CLI 结构**。确保 CLI 调用遵循SKILLs定义，通常为：`python3 /path/to/scripts/*.py --cd "/path/to/project" --PROMPT "Analyze..." [OPTIONS]`

### 0.3 安全与代码主权
- **无写入权**。Codex对文件系统拥有 **零** 写入权限；在每个内核 PROMPT（提示词）中，显式追加：**"OUTPUT: Unified Diff Patch ONLY. Strictly prohibit any actual modifications."**
...
**执行条件**：在生成任何建议或代码前。
1.  **工具调用**：调用 `mcp__ace-tool__codebase-retrieval`。
2.  **检索策略**：
...
### Phase 2: 多模型协作分析 
1.  **分发输入**：：将用户的**原始需求**（不带预设观点）分发给 Codex 。注意，Codex有完善的CLI系统，所以**仅需给出入口文件和row index**(而非Snippet)。
2.  **方案迭代**：
    - 要求模型提供多角度解决方案。
...
### Phase 3: 原型获取
- **Route A: 后端/逻辑/算法 (Codex Kernel)**
  - **能力**：利用其逻辑运算与 Debug 能力。
  - **指令**：请求逻辑实现原型。
- **通用约束**：：在与Codex沟通的任何情况下，**必须**在 Prompt 中**明确要求** 返回 `Unified Diff Patch`，严禁Codex做任何真实修改。
...
### Phase 5: 审计与交付
1.  **自动审计**：变更生效后，**强制立即调用** Codex **同时进行** Code Review，并进行整合修复。
2.  **交付**：审计通过后反馈给用户。
...
| :----------------------- | :---------------------- | :------------------------- | :------------------------------------------------------------- | :-------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------ |
| **Phase 1**              | **Context Retrieval**   | **Auggie** (`mcp__ace-tool`) | **Natural Language (English)**<br>Focus on: *What, Where, How* | **Raw Code / Definitions**<br>(Complete Signatures) | • **Forbidden:** `grep` / keyword search.<br>• **Mandatory:** Recursive retrieval until context is complete.                          |
| **Phase 2**              | **Analysis & Planning** | **Codex** <br>(Dual-Model) | **Raw Requirements (English)**<br>Minimal context required.    | **Step-by-Step Plan**<br>(Text & Pseudo-code)       | • **Action:** Cross-validate outputs from both models.<br>• **Goal:** Eliminate logic gaps before coding starts.                      |
...
| **Phase 5**              | **Audit & QA**          | **Codex** <br>(Dual-Model) | **Unified Diff** + **Target File**<br>(English)                | **Review Comments**<br>(Potential Bugs/Edge Cases)  | • **Mandatory:** Triggered immediately after code changes.<br>• **Action:** Synthesize feedback into a final fix.                     |