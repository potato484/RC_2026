...
`rc26_decision` 是 R2 的主决策包，通过 BehaviorTree.CPP 组织比赛流程，并通过 `NavToPose`、机构动作和视觉/黑板状态驱动整车自动链路。

## 适用场景

- 单独验证行为树是否正常 tick
- 验证 Nav2、`rc26_mechanism`、视觉和行为树黑板是否已被决策正确消费
...
- `rc26_kfs_keepout` 已归档，当前决策链不等待 `/kfs_filter_mask` 或 `/kfs_keepout_heartbeat`
...
- 自动链卡在 gate：先检查 `/r2/bt/debug_state`、`/r2/bt/blackboard`、`/mechanism/status` 和定位健康话题。
