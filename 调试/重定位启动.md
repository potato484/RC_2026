...
本文档对应 R2 的测试重定位链路，核心模块为 `rc26_localization`。重定位依赖先验点云，推荐先完成一轮建图和一轮基础定位，再回到本文件验证重锚定恢复。
...
- 该入口固定复用 `odometry.launch.py + localization.launch.py`
- 默认 `competition_mode:=false`
- 默认 `enable_graph_backend:=false`，先把重定位恢复流程单独跑通
