...
- tip 单目 test 链当前入口为 `launch/test_tip_vision.launch.py`，默认参数文件为 `config/tip_vision_params.yaml`，默认模型资产位于 `models/` 根目录
...
- tip test 节点默认通过 `vision_config_file + model_id` 读取 `models/tip.onnx` 和 `models/tip_labels.txt`；如果你替换模型，优先更新 `config/vision_models.yaml` 对应 profile。
...
- `models/tip.onnx` 当前是固定 `640x640` 输入；单改 profile 的 `input_w / input_h` 不能把这份模型降到 `320`，实际输入尺寸以模型 tensor shape 为准。
...
- `src/nodes`：可执行入口
- `test`：tip 单目 test 链单文件源码；`config/`、`models/`：tip 单目 test 链参数与模型资产，当前 tip 资源直接位于这两个目录根下
