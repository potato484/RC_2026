...
6. **模块级 CI 独立测试**：按 `rc26_serial`、`rc26_mcu_transport`、`rc26_decision`、`rc26_vision`、`rc26_bringup` 等模块拆分 CI 入口和测试矩阵，优先根据变更路径只构建并测试受影响包；主分支合并前使用模块级检查快速发现问题，完整 ROS 2 工作区构建保留给发布、评审或手动验收场景。
7. **多人维护分层**：按驱动层、算法层、决策层拆分维护边界，冻结第一版跨层消息、服务和 Action 契约。
