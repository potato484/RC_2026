...
void RegistrationPCL<PointSource, PointTarget>::setInputSource(const PointCloudSourceConstPtr& cloud) {
    ...
    source_tree_ =
        std::make_shared<small_gicp::KdTree<pcl::PointCloud<PointSource>>>(input_, KdTreeBuilderOMP(num_threads_));
    source_covs_.clear();
...
void RegistrationPCL<PointSource, PointTarget>::setInputTarget(const PointCloudTargetConstPtr& cloud) {
    ...
    target_tree_ =
        std::make_shared<small_gicp::KdTree<pcl::PointCloud<PointTarget>>>(target_, KdTreeBuilderOMP(num_threads_));
    target_covs_.clear();
...
void RegistrationPCL<PointSource, PointTarget>::computeTransformation(PointCloudSource& output, const Matrix4& guess) {
    ...
    if (registration_type_ == "GICP") {
        result_ = registration.align(target_proxy, source_proxy, *target_tree_,
                                     Eigen::Isometry3d(guess.template cast<double>()));
        ...
        result_ = registration.align(*target_voxelmap_, source_proxy, *target_voxelmap_,
                                     Eigen::Isometry3d(guess.template cast<double>()));
    } else {
