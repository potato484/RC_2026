...
/// @note  This function can only handle simple PLY files with float properties (Property names must be "x", "y", "z").
/// Do not use this for general PLY IO.
/// @param filename  Filename
/// @return          Points
static std::vector<Eigen::Vector4f> read_ply(const std::string& filename) {
    ...
    if (line.size() < 3 || properties[0].size() != 1 || properties[1].size() != 1 || properties[2].size() != 1 ||
        std::tolower(properties[0][0]) != 'x' || std::tolower(properties[1][0]) != 'y' ||
        std::tolower(properties[2][0]) != 'z') {
