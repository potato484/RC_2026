...
public:
    OnlineOdometryEstimation(const OdometryEstimationParams& params)
        : OdometryEstimation(params), z_range(-5.0f, 5.0f) {}
    ...
    std::vector<Eigen::Isometry3d> estimate(std::vector<PointCloud::Ptr>& points) override {
        ...
        for (size_t i = 0; i < points.size(); i++) {
            ...
            if (params.visualize) {
                ...
                async_viewer->update_points(guik::anon(), voxelgrid_sampling(*downsampled, 1.0)->points,
                                            guik::Rainbow(T));
