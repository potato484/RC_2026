...
///        Bai et al., "Faster-LIO: Lightweight Tightly Coupled Lidar-Inertial Odometry Using Parallel Sparse
///        Incremental Voxels", IEEE RA-L, 2022
/// @note  This container stores only up to max_num_points_in_cell points and avoids insertings points that are too
/// close to existing points (min_sq_dist_in_cell).
...
public:
    ...
    ///        If there is a point that is too close to the input point, or there are too many points in the cell, the
    ///        input point will not be ignored.
    ...
    void add(const Setting& setting, const Eigen::Vector4d& transformed_pt, const PointCloud& points, size_t i,
             const Eigen::Isometry3d& T) {
        if (this->points.size() >= setting.max_num_points_in_cell ||  //
            std::any_of(
                this->points.begin(), this->points.end(),
...
public:
    ...
    std::conditional_t<HasNormals, std::vector<Eigen::Vector4d>, Empty>
        normals;                                                            ///< Normals (Empty if HasNormals is false)
...
struct Traits<FlatContainer<HasNormals, HasCovs>> {
    ...
    static bool has_normals(const FlatContainer<HasNormals, HasCovs>& container) {
        return HasNormals && container.size();
    }
    ...
    static const Eigen::Vector4d& normal(const FlatContainer<HasNormals, HasCovs>& container, size_t i) {
        return container.normals[i];
    }
    static const Eigen::Matrix4d& cov(const FlatContainer<HasNormals, HasCovs>& container, size_t i) {
        return container.covs[i];
    }

    static size_t nearest_neighbor_search(const FlatContainer<HasNormals, HasCovs>& container,
                                          const Eigen::Vector4d& pt, size_t* k_index, double* k_sq_dist) {
        return container.nearest_neighbor_search(pt, k_index, k_sq_dist);
    }

    static size_t knn_search(const FlatContainer<HasNormals, HasCovs>& container, const Eigen::Vector4d& pt, size_t k,
                             size_t* k_index, double* k_sq_dist) {
        return container.knn_search(pt, k, k_index, k_sq_dist);
    }

    template <typename Result>
    static void knn_search(const FlatContainer<HasNormals, HasCovs>& container, const Eigen::Vector4d& pt,
                           Result& result) {
