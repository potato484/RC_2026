...
public:
    ...
    static AxisAlignedProjection find_axis(const PointCloud& points, IndexConstIterator first, IndexConstIterator last,
...
public:
    ...
    double operator()(const Eigen::Vector4d& pt) const {
        return Eigen::Map<const Eigen::Vector3d>(normal.data()).dot(pt.head<3>());
    }
    ...
    template <typename PointCloud, typename IndexConstIterator>
    static NormalProjection find_axis(const PointCloud& points, IndexConstIterator first, IndexConstIterator last,
                                      const ProjectionSetting& setting) {
        const size_t N = std::distance(first, last);
        Eigen::Vector4d sum_pt = Eigen::Vector4d::Zero();
