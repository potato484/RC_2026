...
public:
    ...
    VisualizationNode() : Node("rc26_visualization_node") {
        ...
        sync_->registerCallback(
            std::bind(&VisualizationNode::imageCallback, this, std::placeholders::_1, std::placeholders::_2));
        ...
        if (show_depth_window_) {
            RCLCPP_INFO(this->get_logger(), "  Depth window: ON (colormap=%d, range=%.1f-%.1fm)", depth_colormap_,
                        depth_vis_min_, depth_vis_max_);
        }
        RCLCPP_INFO(this->get_logger(), "  Press 'q' to quit");
    }

    ~VisualizationNode() { cv::destroyAllWindows(); }
...
private:
    void imageCallback(const sensor_msgs::msg::Image::ConstSharedPtr& color_msg,
        ...
        if (show_fps_) {
            ...
            cv::putText(frame, fps_text, cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 255, 0), 2);
        }

        // 显示检测数量
        char det_text[64];
        snprintf(det_text, sizeof(det_text), "Detections: %zu", detections.size());
        cv::putText(frame, det_text, cv::Point(10, 60), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 255, 255), 2);
    ...
    void drawDepthColorbar(cv::Mat& img) {
        ...
        cv::putText(img, label, cv::Point(bar_x - 35, bar_y + 10), cv::FONT_HERSHEY_SIMPLEX, 0.4,
                    cv::Scalar(255, 255, 255), 1);
        snprintf(label, sizeof(label), "%.1fm", (depth_vis_min_ + depth_vis_max_) / 2);
        cv::putText(img, label, cv::Point(bar_x - 35, bar_y + bar_height / 2), cv::FONT_HERSHEY_SIMPLEX, 0.4,
                    cv::Scalar(255, 255, 255), 1);
        snprintf(label, sizeof(label), "%.1fm", depth_vis_min_);
        cv::putText(img, label, cv::Point(bar_x - 35, bar_y + bar_height - 5), cv::FONT_HERSHEY_SIMPLEX, 0.4,
                    cv::Scalar(255, 255, 255), 1);
    }
    ...
    void runInference(const cv::Mat& frame, std::vector<DetectedObject>& detections) {
        // 预处理：创建 blob
        cv::Mat blob;
        cv::dnn::blobFromImage(frame, blob, 1.0 / 255.0, cv::Size(input_size_, input_size_), cv::Scalar(0, 0, 0), true,
                               false);
    ...
    void drawDetections(cv::Mat& frame, const std::vector<DetectedObject>& detections) {
        for (const auto& det : detections) {
            ...
            if (det.distance > 0.0f) {
                snprintf(label_text, sizeof(label_text), "%s: %.2f (%.2fm)", class_name.c_str(), det.score,
                         det.distance);
            } else {
                snprintf(label_text, sizeof(label_text), "%s: %.2f", class_name.c_str(), det.score);
            }
            ...
            // 绘制标签背景
            cv::rectangle(frame, cv::Point(det.box.x, label_y - text_size.height - 5),
                          cv::Point(det.box.x + text_size.width + 5, label_y + 5), color, cv::FILLED);

            // 绘制标签文本
            cv::putText(frame, label_text, cv::Point(det.box.x + 2, label_y), cv::FONT_HERSHEY_SIMPLEX, 0.5,
                        cv::Scalar(255, 255, 255), 1);
