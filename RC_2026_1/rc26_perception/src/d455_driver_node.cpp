    ...
    D455DriverNode() : Node("d455_driver_node"), running_(true) {
        ...
        if (width_ <= 0 || height_ <= 0) {
            RCLCPP_WARN(this->get_logger(),
                        "Invalid resolution %dx%d, fallback to 640x480", width_, height_);
            width_ = 640;
            height_ = 480;
        }
        if (fps_ <= 0 || fps_ > 120) {
            RCLCPP_WARN(this->get_logger(),
                        "Invalid fps=%d, fallback to 30", fps_);
            fps_ = 30;
        }
        ...
        if (depth_max_ < depth_min_) {
            RCLCPP_WARN(this->get_logger(),
                        "Invalid depth range %.2f-%.2f, fallback to 0.1-10.0", depth_min_, depth_max_);
            depth_min_ = 0.1;
            depth_max_ = 10.0;
        }
        ...
        try {
            ...
            RCLCPP_INFO(this->get_logger(), "RealSense D455 started: %dx%d@%dfps",
                        width_, height_, fps_);
            RCLCPP_INFO(this->get_logger(), "  Color: %s, Depth: %s, Align: %s",
                        enable_color_ ? "ON" : "OFF",
        ...
        for (auto& sensor : device.query_sensors()) {
            ...
            if (sensor.is<rs2::depth_sensor>()) {
                ...
                if (sensor.supports(RS2_OPTION_VISUAL_PRESET)) {
                    try {
                        ...
                        if (depth_preset_ == 1) preset = 3; // High Accuracy
                        else if (depth_preset_ == 2) preset = 4; // High Density
            ...
            if (sensor.supports(RS2_OPTION_ENABLE_AUTO_EXPOSURE)) {
                try {
                    ...
                    RCLCPP_INFO(this->get_logger(), "  Auto exposure: %s", 
                                enable_auto_exposure_ ? "ON" : "OFF");
                } catch (...) {}
            }
        ...
        while (rclcpp::ok() && running_) {
            try {
                ...
                if (has_color && color_pub_) {
                    cv::Mat img(cv::Size(color_frame.get_width(), color_frame.get_height()),
                                CV_8UC3, (void*)color_frame.get_data(), cv::Mat::AUTO_STEP);
                    cv::Mat img_copy = img.clone();
                    auto msg = cv_bridge::CvImage(
                        std_msgs::msg::Header(), sensor_msgs::image_encodings::BGR8, img_copy
                    ).toImageMsg();
                ...
                if (has_depth && depth_pub_) {
                    ...
                    auto msg = cv_bridge::CvImage(
                        std_msgs::msg::Header(), sensor_msgs::image_encodings::TYPE_16UC1, depth_copy
                    ).toImageMsg();
                    msg->header.stamp = stamp;
                    msg->header.frame_id = frame_id_;
