...
public:
    ...
    PerceptionNode() : Node("rc26_perception_node") {
        ...
        if (input_size_ <= 0 || input_size_ > 2048) {
            RCLCPP_WARN(this->get_logger(), "Invalid input_size=%d, fallback to 640", input_size_);
            input_size_ = 640;
        }
        ...
        if (!model_path_.empty()) {
            ...
            if (!engine_->init(engine_cfg)) {
                RCLCPP_WARN(this->get_logger(), "YOLO engine init failed, running in pass-through mode");
                engine_.reset();
            }
        } else {
            RCLCPP_WARN(this->get_logger(), "No model_path specified, running in pass-through mode");
        }
        ...
        sync_->registerCallback(
            std::bind(&PerceptionNode::imageCallback, this, std::placeholders::_1, std::placeholders::_2));
...
private:
    void imageCallback(const sensor_msgs::msg::Image::ConstSharedPtr& color_msg,
        ...
        if (engine_ && engine_->isInitialized()) {
            ...
            postprocess::YoloPost::runCxcywh(box_data, box_bytes, score_data, score_bytes, objs, (float)conf_thres_,
                                             (float)iou_thres_, input_size_, color_img.cols, color_img.rows, scale,
                                             /*box_ch_first=*/true, /*sco_ch_first=*/true, num_classes_,
                                             /*max_det=*/300);
            ...
            RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000, "Published %zu detections", objs.size());
        }
