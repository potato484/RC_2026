...
PoseSender::PoseSender(rclcpp::Node& node, std::shared_ptr<rc26_decision::SerialDriver> serial_driver, Config config)
    ...
    RCLCPP_INFO(node_.get_logger(), "PoseSender 启动，cmd_vel: %s, odom: %s, 发送频率: %d Hz",
...
void PoseSender::sendTimerCallback() {
    if (serial_driver_ && serial_driver_->isOpen()) {
        float vx, vy, wx, wy, wz, roll, pitch, yaw;
        {
            std::lock_guard<std::mutex> lock(data_mutex_);
            vx = vx_;
            vy = vy_;
        ...
        serial_driver_->sendPose(vx, vy, wx, wy, wz, roll, pitch, yaw);
    }
}
