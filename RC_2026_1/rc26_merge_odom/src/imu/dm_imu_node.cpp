...
public:
    DmImuNode() : Node("dm_imu_node")
    {
        ...
        timer_pub_ = this->create_wall_timer(
            ...
            std::bind(&DmImuNode::onTimerPublish, this));

        timer_stats_ = this->create_wall_timer(
            std::chrono::seconds(2),
            std::bind(&DmImuNode::onTimerStats, this));
    }
...
private:
    void onTimerPublish()
    {
        dm_imu::ImuData data = driver_.getLatestData();
        ...
        // 检查是否有新数据
        if (data.euler_ts <= last_euler_ts_ &&
            data.gyro_ts <= last_gyro_ts_ &&
            data.accel_ts <= last_accel_ts_)
        {
            return;
        }
        ...
        if (verbose_)
        {
            RCLCPP_INFO(this->get_logger(),
                "#%u RPY=(%.2f, %.2f, %.2f) Gyro=(%.2f, %.2f, %.2f) Accel=(%.2f, %.2f, %.2f)",
                pub_count_,
    ...
    void onTimerStats()
    {
        if (verbose_)
        {
            ...
            RCLCPP_INFO(this->get_logger(),
                "[stats] ok=%u crc=%u short=%u nohdr=%u",
                stats.frames_ok, stats.frames_crc_error,
                stats.frames_short, stats.frames_no_header);
        }
    }
