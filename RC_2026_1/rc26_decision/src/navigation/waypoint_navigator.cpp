...
CommandID navModeToCommandId(NavMode mode) {
    ...
    case NavMode::StairUp:
        return CommandID::NAV_STAIR_UP;
    case NavMode::StairDown:
...
WaypointNavigator::WaypointNavigator(rclcpp::Node& node, std::shared_ptr<SerialDriver> cmd_serial,
                                     std::string nav2_action_name, std::string goal_frame)
    : node_(node), cmd_serial_(std::move(cmd_serial)), nav2_action_name_(std::move(nav2_action_name)),
...
bool WaypointNavigator::sendNav2Goal(uint8_t waypoint_id) {
    ...
    if (!nav2_client_->wait_for_action_server(std::chrono::seconds(5))) {
        RCLCPP_ERROR(node_.get_logger(), "WaypointNavigator: Nav2 action server not ready after 5s: %s",
