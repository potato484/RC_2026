  ...
  std::shared_ptr<tf2_ros::Buffer> tf, std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".use_rotate_to_heading_threshold", rclcpp::ParameterValue(0.1));
  declare_parameter_if_not_declared(
  ...
  node->get_parameter(
    plugin_name_ + ".use_rotate_to_heading_threshold", use_rotate_to_heading_threshold_);
  node->get_parameter(
  ...
  double forward_dist, double backward_dist) const
{
  ...
  for (size_t i = 0; i < path.poses.size(); ++i) {
    double dist = std::hypot(
      path.poses[i].pose.position.x - lookahead_pose.pose.position.x,
...
PLUGINLIB_EXPORT_CLASS(
  rc26_omni_controller::OmniPidPursuitController, nav2_core::Controller)
