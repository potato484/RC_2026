...
ImuProcess::ImuProcess() : b_first_frame_(true), imu_need_init_(true), logger(rclcpp::get_logger("ImuProcess")) {
    imu_en = true;
    init_iter_num = 1;
...
void ImuProcess::Set_init(const Eigen::Vector3d& tmp_gravity, Eigen::Matrix3d& rot) {
    ...
    constexpr double ALIGNMENT_NORM_THRESHOLD = 1e-6;

    M3D hat_grav;
    hat_grav << 0.0, gravity_(2), -gravity_(1), -gravity_(2), 0.0, gravity_(0), gravity_(1), -gravity_(0), 0.0;

    // Avoid division by zero
    const double gravity_norm = gravity_.norm();
    const double tmp_gravity_norm = tmp_gravity.norm();
    if (gravity_norm < ALIGNMENT_NORM_THRESHOLD || tmp_gravity_norm < ALIGNMENT_NORM_THRESHOLD) {
        rot = Eye3d;
        return;
    }

    double align_norm = (hat_grav * tmp_gravity).norm() / gravity_norm / tmp_gravity_norm;
    double align_cos = gravity_.transpose() * tmp_gravity;
    align_cos = align_cos / gravity_norm / tmp_gravity_norm;
    if (align_norm < 1e-6) {
        if (align_cos > 1e-6) {
            rot = Eye3d;
        } else {
