...
private:
  ...
  void performGlobalRelocalization(
    pcl::PointCloud<pcl::PointXYZ>::Ptr source_cloud = nullptr);
  bool detectKidnapping(double fitness_score);
  
  // 多假设初值生成
  std::vector<Eigen::Matrix4f> generateCandidateTransforms(
  ...
  std::mutex result_mutex_;  // 保护 result_t_
