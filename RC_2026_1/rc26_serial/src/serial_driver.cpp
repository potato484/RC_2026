...
bool SerialDriver::writeAll(const uint8_t* data, size_t size)
{
    ...
    while (offset < size)
    {
        ...
        if (!write_error_active_)
        {
            ...
            RCLCPP_ERROR(
                serialLogger(),
                "串口写入失败：port=%s, size=%zu, errno=%d(%s)",
                port_.c_str(),
                size,
                err,
                std::strerror(err));
...
bool SerialDriver::sendCommand(uint8_t cmd, const std::vector<uint8_t>& payload)
{
    ...
    for (uint8_t retry = 0x00; retry <= MAX_RETRY_VALUE; ++retry)
    {
        ...
        if (got_response)
        {
            RCLCPP_WARN(
                serialLogger(),
                "指令收到 NACK (retry=0x%02X, 第%u轮)：cmd=0x%02X, seq=%u",
                retry,
                round,
                cmd,
                seq);
        ...
        else
        {
            RCLCPP_WARN(
                serialLogger(),
                "指令等待超时 (retry=0x%02X, 第%u轮)：cmd=0x%02X, seq=%u",
                retry,
                round,
                cmd,
                seq);
    ...
    RCLCPP_ERROR(
        serialLogger(),
        "指令重发失败 cmd=0x%02X, 已达最大重试次数(0x09)，触发串口重连",
        cmd);

    reconnect();

    setLastError(
        "等待 ACK 超时：cmd=" + std::to_string(static_cast<int>(cmd)) +
        ", 已重试至0x09并触发重连");
    return false;
}
...
void SerialDriver::parseReceivedData()
{
    ...
    while (recv_len_ >= MIN_FRAME_SIZE)
    {
        ...
        if (recv_buffer_[frame_size - 2] != FRAME_TAIL_0 ||
            recv_buffer_[frame_size - 1] != FRAME_TAIL_1)
        {
            ++parse_error_count_;
            if (parse_error_count_ <= 5 || (parse_error_count_ % 100 == 0))
            {
        ...
        if (expected_crc != actual_crc)
        {
            ...
            if (parse_error_count_ <= 5 || (parse_error_count_ % 100 == 0))
            {
                RCLCPP_DEBUG(
                    serialLogger(),
                    "解析失败：CRC 错误(expected=0x%08X actual=0x%08X)，已累计丢弃 %u 次",
                    expected_crc,
                    actual_crc,
                    parse_error_count_);
            }
