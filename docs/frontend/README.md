...
- `merlin-bt-visualizer`
  ...
  - 编辑模式：把原始 XML 解析成可逆编辑语义，当前已经支持官方节点与机器人模块注册表、中文节点库、拖拽插入、同支线前后插入、显式新增支线、节点包裹、复合节点切换、定义内参数/黑板绑定编辑和 XML 导出；其中同支线插入现在必须显式选择顺序 / 回退 / 并行等控制包装，并始终新建一层可见包装节点，不再把新动作偷偷塞进现有 `Sequence`。浏览器也不再允许凭空添加任意附加属性。开发态还可通过本地保存适配层把当前区域 XML 写回源文件。
