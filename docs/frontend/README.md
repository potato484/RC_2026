...
- `merlin-bt-visualizer`
  ...
  - 编辑模式：把原始 XML 解析成可逆编辑语义，当前已经支持官方节点与机器人模块注册表、按业务分类浏览的节点知识库、当前类目内检索、独立插入菜单、同支线前后插入、显式新增支线、节点包裹、定义内数值参数编辑和 XML 导出；其中知识库只负责查节点解释和端口说明，打开后会覆盖当前编辑画布区域，不再透出底层旧内容，新增节点统一走显式插入入口。同支线插入仍必须选择顺序 / 回退 / 并行等控制包装，并始终新建一层可见包装节点，不再把新动作偷偷塞进现有 `Sequence`。检查器现在只保留纯数字固定值参数，不再把字符串、布尔或黑板绑定端口作为常规编辑入口。浏览器也不再允许凭空添加任意附加属性。开发态还可通过本地保存适配层把当前区域 XML 写回源文件。
