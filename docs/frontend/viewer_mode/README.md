...
1. `src/store/useStore.ts:9-14`
   在模块初始化阶段通过 `behaviorTreeSources.ts` 统一读取三份区域 XML，并立刻调用 `parseBTXml()`。
2. `src/store/useStore.ts:44-62`
   建立默认区域 `梅林区`、默认主树和展示初始状态。
3. `src/utils/btDisplay.ts:1-205`
   先把动作名、树名、参数和值转换成查看/编辑共用的中文展示规则。
4. `src/utils/btParser.ts:58-168`
   把原始 BehaviorTree XML 转成 `ParsedArea -> ParsedTree -> BTNode[]/edges[]` 的展示模型。
5. `src/utils/btParser.ts:5-56`
   在转换后执行装饰器压缩，把部分 Decorator 节点折叠进真正执行节点，避免画布过碎。
6. `src/utils/btParser.ts:181-238`
   使用 `dagre` 做从左到右的树布局，再交给 React Flow。
7. `src/components/TreeVisualizer.tsx:13-87`
   只负责把 store 中的 `nodes/edges` 交给 React Flow，并把点击和折叠行为回写 store。
8. `src/components/Sidebar.tsx:63-110`
   按 `parentTreeId` 把主树和子树组织成分层列表，而不是简单平铺树名。
9. `src/components/RightPanel.tsx:14-150`
...
- `src/utils/behaviorTreeSources.ts:1-10`
  - 三个区域 XML 的统一原始导入。
- `src/store/useStore.ts:7-10`
  - 查看模式对三个区域的首轮解析。
...
- `src/utils/btDisplay.ts:1-205`
  - 中文动作名、参数名、树名和值翻译规则；编辑态也复用这套展示解释。
- `src/utils/btParser.ts:5-56`
  - 装饰器压缩逻辑。
- `src/utils/btParser.ts:58-168`
  - BehaviorTree XML 到展示模型的主解析流程。
- `src/utils/btParser.ts:181-238`
  - `dagre` 横向布局。
...
- 如果改了中文显示规则，优先先改 `btDisplay.ts`，再确认 `btParser` 和编辑态投影出来的文案都仍然一致。
