...
- `merlin-bt-visualizer` 虽然现在已经有按区域草稿生效的撤销/重做，但仍然没有 schema 校验、冲突处理、多人协作、拖线重连后的完整编辑语义。
...
- `merlin-bt-visualizer`
  ...
  - 一套浏览器内“定义内参数编辑 + 结构编排 + XML 导出”的本地编辑器；浏览器不会凭空新增任意属性，同支线插入也必须显式选择顺序 / 回退 / 并行等控制包装。开发态可通过本地适配层把当前区域 XML 写回源文件。
...
- `merlin-bt-visualizer`: 读取 `rc26_decision` 行为树 XML 的本地可视化/演示工具，加上一套在节点定义约束内工作的 XML 编辑器。
