...
`behavior_trees/mf_preselection_tree.xml` 是梅林区预选赛专属正式入口，完整 bringup 可通过 `r2_runtime.paths.behavior_tree_file` 指向它。该树先按 `mf_preselect_entry2_nav_enable` 决定是否执行 2 号入口 `NavToPose`：关闭时跳过入口导航并假设机器人已经位于 2 号入口预备姿态；开启时若 `NavToPose` 返回 `FAILURE`，整棵预选赛树会停止在入口阶段，不会继续进入 `MfPreselectionFlow` 下发机构或运动命令。入口 `NavToPose` 默认通过 `mf_preselect_entry2_nav_behavior_tree_file` 指向 `rc26_bringup/config/nav2_bt_mf_preselect_entry_x_positive_y_negative.xml`，让 Nav2 使用 `MFPreselectEntryXPositiveYNegative` controller；该 controller 当前保留低速上限但不再硬限制车体系 x/y 方向，并在 `FollowPath` 中显式指定 `general_goal_checker`，避免 Humble Nav2 把空 goal checker 名称传给 controller server。入口导航只覆盖到 2 号入口前的 Nav2 阶段，不影响后续 `MfPreselectionFlow` 内部直接发布 `/cmd_vel` 的相对移动、台阶、转向和离场状态机。当前 `rc26_bringup/config/nav2_params.yaml` 的共享 Nav2 costmap 加载 obstacle layer 与 inflation layer，因此该入口导航会消费动态障碍，并在合成 costmap 中对静态和动态障碍生成膨胀代价。入口导航成功后会立即执行 `GridTurn -> GridHeadingAlign`，目标 yaw 来自 `grid_heading_target_yaw_rad`，也就是 `grid_heading_direction` 和 `grid_heading_*_yaw_rad` 参数选出的方向；任一阶段失败都会阻止进入 `MfPreselectionFlow`。随后执行单个 `MfPreselectionFlow` 状态机，流程成功驶出梅林后进入 `WaitForever`，保持永久静止。

`MfPreselectionFlow` 只消费 `rc26_vision` 的 KFS 模型快照、`/odom`、`/cmd_vel` 和 `rc26_mcu_transport` 提供的 `/mechanism/send_command` / `/mechanism/command_feedback`。视觉标签按当前模型语义处理：`T_*` 是 R2 可夹取 KFS，`R_R1/B_R1` 是 R1 阻挡目标，`F_*` 是假 KFS；这些标签列表和前缀都由 `mf_preselect_*` 参数配置。R2 KFS 单局最多夹取 `mf_preselect_max_pickup_count` 个，默认 2 个，达到上限后不再触发 KFS 夹取。检测阶段稳定看到 `T_*` 后，正式流程不会直接发送夹取命令，而是先用同一个视觉快照单帧锁定 `label + bbox + sequence + depth + bbox中心像素偏差`；锁定帧必须同时具备彩色帧、检测框和 `mf_preselect_depth_min_m ~ mf_preselect_depth_max_m` 内的有效深度，否则当前 tick 仍按未看到可夹取目标处理。进入 `kfs_visual_align` 后，目标中心线不是写死图像中心，而是 `图像中心 + mf_preselect_kfs_align_target_offset_px`，用于补偿相机中心线和机械臂夹取线的安装偏置。状态机会按当前帧 `offset_px` 和 `mf_preselect_kfs_align_px_to_m` 规划一段横向开环对齐距离，用 `mf_preselect_kfs_align_*` 速度参数发布 `cmd_vel.linear.y`；每段横移结束后必须停车，并把横移完成时刻的最新视觉序号作为复核门槛，只接受更晚的新有效视觉帧确认对齐。只有复核帧 offset 进入 `mf_preselect_kfs_align_tolerance_px` 后才允许进入前向靠近，否则继续按复核帧重新规划下一段横移，直到对齐总耗时超过 `mf_preselect_kfs_align_timeout_s`；即使锁定帧一开始已经在容差内，也仍会等待后一帧确认，不会直接前进夹取。横移对齐无法规划、等待复核超时或总超时时，会忽略该目标、不计数，并按“该检测点未夹到目标”的路线继续。新帧确认对齐后，按确认帧深度计算 `max(0, locked_depth - mf_preselect_kfs_grab_distance_m) / mf_preselect_kfs_approach_speed_mps` 做 x 方向纯开环趋近；若当前是向下夹取，则在前向开环前先发送 `ARM_SECOND_LOWER(0x0E)`，等待同 `seq` 的 `ARM_SECOND_LOWER_DONE(0x0A)`，确认第二节机械臂彻底放下后才启动前向开环计时；若锁定距离已经为 0，也仍先等待 `0x0A` 后直接夹取。前向开环阶段不再依据持续识别框或实时深度闭环停车，框消失、框跳变或深度波动不会触发重搜；计划时长到达后停车并发送方向对应的夹取命令：入口高侧夹取发送 `ENTRY_GRAB_KFS_UP(0x0F)` 并等待同 `seq` 的 `ENTRY_GRAB_KFS_UP_DONE(0x0B)`，其它高侧/低侧夹取继续发送 `GRAB_KFS_UP(0x03)` 或 `GRAB_KFS_DOWN(0x02)` 并只等待 ACK。夹取计数不在 ACK 或完成反馈后立即提交：状态机会保存开环前锁定的 `label + bbox + sequence`，命令确认后进入最多 `mf_preselect_grab_verify_timeout_s` 的视觉验证窗口；只有同标签且 bbox IoU 达到 `mf_preselect_grab_verify_iou_threshold` 的原目标连续 `mf_preselect_grab_verify_lost_stable_frames` 个新推理帧不可见，才判定物理夹取成功并更新计数。同目标匹配 helper 与 `rc26_vision` 的独立 KFS action test 共用 `shared/target/visual_target_match`，避免两条链路的 bbox/label 语义分叉。若验证超时、原目标仍可见、没有新视觉帧，当前目标会被本轮忽略、不计数，并按“该检测点未夹到目标”的路线继续；service rejected、ACK 超时、入口高夹取完成反馈超时、第二节机械臂放下完成反馈超时、视觉启动失败和前向开环计划超过安全超时仍是流程失败。
...
入口阶段先检测 2 号入口；若无 R2 KFS，则发送 `ARM_HIGH_RAISE(0x0D)` 并等待同 `seq` 的 `ARM_HIGH_RAISE_DONE(0x09)`，再按配置横移探测 1 号、3 号阶梯；这个高抬升只服务预选赛入口侧探测，不替代梅林内部普通机械臂升降。入口左右横移和返回中间入口横移期间如果单帧锁定到带有效深度的 R2 KFS，会先执行完整夹取流程；夹取成功后依据入口横向 odom 参考回到 2 号入口再上首阶，夹取失败则忽略该目标并补完原横移路线。任一入口检测点探测到 R2 KFS 后都会先执行“横移开环段 + 新视觉帧复核”的对齐闭环，确认 offset 进入容差后再基于确认帧深度执行前向开环趋近；入口高侧夹取动作使用专用协议 `ENTRY_GRAB_KFS_UP(0x0F)`，并等待同 `seq` 的 `ENTRY_GRAB_KFS_UP_DONE(0x0B)` 后再进入视觉消失验证。返回 2 号入口后，上阶梯前若已经处于入口高抬升保持态，不重复发送普通 `ARM_RAISE(0x04)`；否则先发送普通抬升并等待 `ARM_RAISE_DONE(0x02)`。进入梅林内部后，格间高低台阶切换观察前、第 2/3 行前方守卫检测前以及第 2/3 行左侧/背向周身扫描前，都会按静态格高差先执行普通机械臂预调：低到高发送 `ARM_RAISE(0x04)` 并等待 `ARM_RAISE_DONE(0x02)`，高到低发送 `ARM_LOWER(0x05)` 并等待 `ARM_LOWER_DONE(0x03)`；随后才进入对应检测窗口，检测到 R2 KFS 时按该高低侧语义选择普通 `GRAB_KFS_UP(0x03)` 或 `GRAB_KFS_DOWN(0x02)`，其中向下夹取仍先发送 `ARM_SECOND_LOWER(0x0E)` 并等待 `ARM_SECOND_LOWER_DONE(0x0A)` 后再开环趋近。
...
其中 `mf_preselect_state` 现在会出现 `kfs_visual_align`、`kfs_second_arm_lower`、`kfs_open_loop_approach`、`entry_center_align`、`grid_center_align` 与 `final_exit_center_align`，分别表示正式 MF 内部正在进行基于锁定帧像素偏差的 KFS 夹取前横移开环对齐、向下夹取前等待第二节机械臂放下、定时开环趋近、入口 grid2 中心参考建立、格间台阶后的目标格中心归位和最终离场虚拟目标归位。`mf_preselect_kfs_align_target_offset_px` 是机械臂夹取目标线相对图像中心的像素偏置，`mf_preselect_kfs_align_px_to_m` 是像素偏差到横移距离的现场标定，`mf_preselect_kfs_align_timeout_s` 是“横移段 + 新帧复核”的对齐总安全超时；`mf_preselect_kfs_align_stable_frames` 与 `mf_preselect_kfs_lost_stop_frames` 仅保留为兼容旧配置的声明参数，当前正式夹取前横移对齐不再依赖后续视觉帧稳定或丢失计数。`mf_preselect_kfs_grab_distance_m` 是机械臂可触达距离/开环停止距离，不是前进途中闭环深度阈值。

旧 `KfsStairPickup` 阶梯等待 BT 节点和 `kfs_stair_*_test_tree.xml` 独立测试树已删除；MF 预选赛的 R2 KFS 夹取由 `MfPreselectionFlow` 内部状态机完成“稳定检测并单帧锁定 bbox/depth/offset -> 按当前 offset 执行横移开环段 -> 横移完成后等更晚的新视觉帧复核直到进入容差 -> 向下夹取时等待 `ARM_SECOND_LOWER_DONE(0x0A)` -> 按确认帧深度定时前向开环趋近 -> 入口高夹取 `ENTRY_GRAB_KFS_UP(0x0F)` 等 `0x0B` / 普通 `GRAB_KFS_UP/DOWN` 等 ACK -> 原目标视觉消失验证”。独立 `T_*` 对齐、趋近、夹取实机验收仍可由 `rc26_vision/test_kfs_vision.launch.py action_enable:=true` 承担，但该测试入口不替代正式 MF 决策权威。
