...
- `second_preselection_combo_tree.xml`：second managed 组合入口，入口 `WaitPreselectionBranchGate` 保持同时接受人工触发外部限位 1/2 上行 `0x06/0x10`，两条分支都使用 `switch_start_profile="second"` / `continue_start_profile="second"` 下发 `SECOND_PRESELECTION_START(0x11)`，并只接受同 `seq` 的 `SECOND_PRESELECTION_START_DONE(0x0D)`。入口直接收到 `0x10` 时，握手完成后请求切换到 `second_preselection_climb_place_tree.xml`，不执行斜坡；入口收到 `0x06` 时，首次握手成功后执行 `PreselectionRampForwardTree` 合并斜坡前进并停车，再由只接受 `0x10` 的斜坡后 gate 完成第二次 `0x11/0x0D` 握手并请求切换到同一目标树。两个 gate 提交切树请求后都保持 `RUNNING`，`decision_node` 在当前 tick 结束后 halt 组合树并加载目标树，因此组合树不会继续执行其它动作，也不再 include 或内嵌 `SecondPreselectionTree`。
...
- `second_preselection_climb_place_tree.xml`：既可显式选择调试，也是 `preselection_mode: second` 两条 `0x10` 分支的统一切树目标。树内不新增外部限位 gate，不发送 `0x11`，也不等待 `0x0D`；managed 使用时启动握手已由组合树完成，切树后的下一轮 tick 直接从 `OdomDriveX(second_preselect_climb_place_forward_x_m)` 开始。其后保持 `OdomDriveY(red +0.3m / blue -0.3m) -> Delay(20s)`、人工前激光上阶、后轮 odom 前进、预装 KFS 夹取和最终放置流程。`SecondPreselectionClimbFrontStage` 在前推杆伸出前尝试 yaw 对齐；对齐成功时正常继续，对齐失败或超时时先发布零速并记录 WARN，但不再让行为树失败，仍发送前推杆伸出 `0x08` 并在 ACK 后零速稳定 `stair_climb_front_extend_delay_s`；稳定完成后仍持续发布全零 `/cmd_vel`，不再调用前轮行驶速度曲线，也不做 yaw 运动，直到收到本等待阶段开始后的无 seq 上行人工前激光 `0x15`。旧 `0x15`、错误反馈和后轮 `0x05` 均不会放行；等待超过 `stair_front_event_timeout_s` 时失败停车。有效 `0x15` 到达后才并发发送前收 `0x09` 与后伸 `0x0A`，稳定后由独立 `OdomDriveX(+0.6m)` 推动后轮上阶；该节点单独启用 `succeed_on_timeout=true`，在已经捕获有效 odom 目标后达到 `second_preselect_climb_place_rear_timeout_s` 时会先发布零速、记为成功并继续后续逻辑。尚未捕获有效 odom 目标、距离输入非法或运行上下文缺失仍返回失败。`SecondPreselectionRearRetractPickupPlace` 随后同 tick 下发后收 `0x0B` 与预装 KFS 夹取 `0x15`，两条命令 ACK 后同时等待 25s 和下行 `0x15` 同 seq 的上行 `0x14`，满足后仅发送一次最终 `0x13` 并等待通用 ACK。错误 seq、错误 feedback、旧事件、失败与 halt 继续沿用现有忽略、失败或零速停车语义。
...
- `second_preselect_climb_place_*`：上阶夹取放置目标树的专属参数，既服务显式独立运行，也服务 second managed 切树后的执行。路线默认 `forward_x=1.5m`、红方基准 `lateral_y=+0.3m`、`pre_climb_delay=20000ms`，blue 在参数加载时自动镜像为 `-0.3m`；前半上阶命令/反馈为 `0x08 -> 零速等待上行0x15 -> 0x09+0x0A`，推杆稳定延时、best-effort heading 预对齐和人工前激光等待超时复用 `stair_*`；heading 对齐失败或超时会停车告警后继续，不阻断前推杆流程，但这棵目标树在 `0x08` 伸出后不再使用 `stair_climb_front_drive_*` 速度曲线。后轮段使用 `rear_forward_x=0.6m`、`rear_max/min_speed=0.40/0.10m/s` 的 `OdomDriveX`，当前红蓝运行配置把 `rear_timeout` 设为 `5.0s`，并复用通用 odom 增益、容差和新鲜度；该段启用 `succeed_on_timeout`，有效目标捕获后的动作超时会停车成功并继续收尾，不把其它初始化或目标捕获错误吞成成功。收尾参数为 `rear_retract=0x0B`、`preload_pickup=0x15/done=0x14`、`final_delay=25s`、`final=0x13`；机构 service/topic、通用 ACK 超时、done 超时和日志周期继续复用其它 `second_preselect_*` 公共参数。
...
2026-07-13 同步：`SecondPreselectionClimbFrontStage` 的上阶前 yaw 对齐改为 best-effort。对齐成功时行为不变；对齐失败、无有效 heading 或达到对齐超时时，节点先发布零速并记录 WARN，然后仍发送前推杆伸出 `0x08`，继续零速等待人工前激光 `0x15` 和后续流程。运行上下文初始化失败、`0x08` 机构命令失败、人工 `0x15` 等待超时等非 yaw 错误仍保持失败停车。

2026-07-13 同步：通用 `OdomDriveX/OdomDriveY` 新增默认关闭的 `succeed_on_timeout` 端口，仅 `second_preselection_climb_place_tree.xml` 的后轮 `+X 0.6m` 节点启用。该段在捕获有效 odom 目标后达到当前红蓝配置的 `5.0s` 动作超时时发布零速、返回成功并继续 `SecondPreselectionRearRetractPickupPlace`；未建立 odom 目标、输入非法和运行上下文错误仍保持失败。该扩展不改变 ROS 接口、MCU 协议、blackboard 公共键或其它 OdomDrive 节点的默认超时语义。
...
2026-07-13 同步：新增的 `second_preselection_climb_place_tree.xml` 既可通过显式 `tree_file` 选择，也作为 second managed 统一目标树；不增加第三种 `preselection_mode`。树内按 `+X 1.5m -> red +Y 0.3m / blue -Y 0.3m -> 20s -> 人工前激光 0x15 上阶前段 -> odom +X 0.6m -> 0x0B + 下行0x15 -> 同 seq 0x14 与 25s 双门控 -> 0x13` 执行，且不重复 managed 入口已完成的 `0x11/0x0D` 握手；后轮 `0x05` 不参与流程。新增的两个异步节点继续只消费现有 `/cmd_vel`、`/mechanism/send_command`、`/mechanism/command_feedback` 契约，不新增串口 ID、ROS 接口或运动命令权威。
