...
- `second_preselection_tree.xml`：第二个预选赛独立树。若不是从 branch gate 直达，会先执行 `SECOND_PRESELECTION_START(0x11)` 并等待同 `seq` 的 `SECOND_PRESELECTION_START_DONE(0x0D)`。`SecondPreselectionKfsPickup` 在首次取得新鲜 odom、尚未发布搜索速度时固化搜索原点 `x/y/yaw`，随后沿 `+X` 搜索 KFS、视觉横移对齐，按 `0x14/同 seq 0x12` 确认机械臂放下，根据锁定深度完成夹取趋近，再按 `0x12/同 seq 0x11` 和目标消失验证确认夹取。夹取后树按顺序执行 `OdomDriveX(+1.5m) -> OdomDriveY(red +0.75m / blue -0.75m) -> SecondPreselectionDriveToTotalX`；总 X 是当前位置相对搜索原点沿搜索起始 yaw 的有符号 odom 净投影，搜索、夹取趋近和固定 `+X 1.5m` 自动计入，入口斜坡与最终放置趋近不计入，目标为 `4.2m`、容差 `0.03m`，进入节点时已经超调则停车成功且不倒退。随后 `SecondPreselectionKfsPlacePrepare` 先用连续新视觉帧判断正前方上下双层 KFS 框占位：中心需位于宽度 `20%~80%`，上框中心位于高度 `20%~55%`、下框位于 `55%~100%`，横向中心差不超过 `120px`，连续 `2` 帧确认占据或清空；首次占据 red `+Y 0.56m` / blue `-Y 0.56m`，再次占据反向 `1.10m`，第三次仍占据则写入立即放置状态并跳过视觉对齐和深度趋近。每次避让完成后会丢弃当前已有帧，只从下一张新视觉帧重新累计占据/清空稳定；避让段使用独立横移超时，不计入无真实深度等待。场地清空后，节点在配置的 R2/R1 标签与名称包含 `KFS` 的检测框集合中选择任一真实深度有效目标，优先画面中心最近，平局再比较深度和置信度；深度直接复用 `MfPreselectionLogicResult::sampleKfsDepthFromBbox`，只接受 `CenterRoi` / `BboxMultiRoi`，稳定对齐帧深度取中位数，不使用单目尺寸估距。若连续新视觉帧在 `second_preselect_kfs_approach_timeout_s` 内始终没有真实深度候选，则节点保持视觉运行并启动累计 `+X second_preselect_place_no_depth_forward_x_m` 降级前进；首次启动时固化 odom 原点和 yaw，后续中断、占位避让和恢复都按该原点净投影计算累计进度。降级前进中任意 KFS 框中心进入宽度 `20%~80%`、高度 `20%~100%` 的中心 ROI 时，单张新帧立即停车并从完整占位检查重新开始；若再次无真实深度超时，只继续尚未完成的距离。降级累计到达 `1.5m` 或该运动达到 `second_preselect_nav_timeout_s` 时进入立即放置；若中断后获得真实深度并完成对齐，则取消剩余降级距离并进入正常趋近。`SecondPreselectionPlaceApproach` 按 `max(0, depth-second_preselect_place_arm_reach_m)` 沿 `+X` odom 闭环趋近，超时同样复用 `second_preselect_kfs_approach_timeout_s`，超时停车并降级成功；初次夹取趋近、无深度等待和最终放置趋近共用该参数。上述第三次占位、降级走满、降级导航超时或正常趋近完成后，树都只执行唯一一次 ACK-only 下行 `SECOND_PRESELECTION_PLACE_KFS(0x13)`，再执行 `OdomDriveX(-1.5m)` 和原有上阶链；初始化、参数或运行上下文错误仍返回失败。可选 OpenCV UI 贯穿搜索夹取、占位判断、降级前进中断和任意 KFS 放置对齐；失败、halt、odom/视觉/机构异常均保持停车语义。`start_r2_auto.sh` 的上行 `feedback_id=0x13` 红蓝切换监听器不改变这里的下行放置命令语义。
...
`VisualServoGrabAction` 内嵌相机采集和端头推理，在工作线程中执行目标锁定、无端头横移搜寻、横移 P 控制、短暂丢框预测伺服、近中心丢框禁止固定搜索保护、odom yaw 姿态保持、限位前探和 `GRAB_TIP(0x01)` service 请求。它只通过 `/cmd_vel`、`/mechanism/send_command` 和 `/mechanism/command_feedback` 与下层交互，不直接打开串口。
...
- `second_preselect_*`：第二个预选赛独立树参数。命令参数仍为 `0x11/0x0D` 开始握手、`0x14/0x12` 机械臂放下握手、`0x12/0x11` KFS 夹取触发与动作完成握手、下行 `0x13` ACK-only 放置；搜索/夹取链继续使用 `second_preselect_search_*`、R2/R1 标签集合、`second_preselect_kfs_align_*`、`second_preselect_kfs_approach_*`、bbox 深度采样、可选单目兜底和夹取消失验证参数。夹取后导航使用 `second_preselect_post_pickup_forward_x_m=1.5`、`second_preselect_nav_y1_m=0.75`、`second_preselect_nav_max_speed_mps/min_speed_mps`、`second_preselect_total_x_target_m=4.2`、`second_preselect_total_x_tolerance_m=0.03`；`second_preselection_tree.xml` 内三段 `OdomDriveX/Y` 不再复用通用 `odom_relative_nav_max_speed_mps/min_speed_mps`，速度必须由 second 专用参数维护；`second_preselect_nav_x2_m` 与 `second_preselect_place_forward_x_m` 已从正式流程和红蓝配置删除。放置准备使用 `second_preselect_place_occupied_center_x_min_ratio/max_ratio`、`upper_y_min_ratio/split_y_ratio`、`max_center_dx_px`、稳定帧、两级横移距离、横移速度和超时参数；红蓝方向通过内部 `team_mirror_sign` 派生。最终放置只使用 MF bbox 多 ROI 返回的真实深度，`second_preselect_place_arm_reach_m=0.45` 独立于夹取臂长；`second_preselect_place_no_depth_forward_x_m=1.5` 控制清空后无真实深度的累计降级前进上限，运动超时复用 `second_preselect_nav_timeout_s`。`second_preselect_kfs_approach_timeout_s=8.0` 统一用于初次夹取趋近、清空后连续无真实深度等待和最终放置趋近，旧 `second_preselect_place_approach_timeout_s` 已删除。放置准备会写入降级是否启动、累计/剩余距离、视觉中断次数、当前模式和终止原因等 `second_preselect_place_*` blackboard 观测状态。放置后仍由 `second_preselect_post_place_retreat_x_m=-1.5` 和 `second_preselect_post_place_*` 参数完成后退与上阶；人工前轮激光 `0x15` 不等同于台阶前轮高度突变 `0x04`。`second_preselect_dynamic_roi_ui_enable` 控制搜索夹取、占位判断、降级中断和任意 KFS 放置对齐的本地 OpenCV 调试窗口。上行 `feedback_id=0x13` 只由 `start_r2_auto.sh` 监听器消费并写回 `r2_active_side.yaml`，不属于 decision 行为树内部切换。
...
2026-07-10 同步：第二预选赛在 `SecondPreselectionKfsPickup` 首次搜索速度发布前记录新鲜 odom 搜索原点；夹取后执行 `+X 1.5m -> red +Y 0.75m / blue -Y 0.75m -> 总 X 净投影补齐至 4.2m`。`SecondPreselectionKfsPlacePrepare` 在双框占位两级避让后等待任意真实深度 KFS；连续 `second_preselect_kfs_approach_timeout_s` 无真实深度时，按首次降级 odom 原点累计前进 `second_preselect_place_no_depth_forward_x_m=1.5m`。前进中中心 ROI 任意 KFS 单帧触发停车并重跑完整占位/避让/对齐流程，多次中断只恢复剩余距离；获得真实深度后取消降级剩余距离，走 `depth-0.45m` 正常趋近。降级走满或导航超时直接进入唯一一次首次 `0x13`，随后保留 `-X 1.5m` 后退与原放置后上阶链；旧 `second_preselect_place_approach_timeout_s` 已删除，初次夹取趋近、无深度等待和最终趋近统一复用 `second_preselect_kfs_approach_timeout_s`。
