...
2026-07-04 同步：第二预选赛 `SecondPreselectionTree` 内部流程改为搜索夹取链。保留 `second_preselection_combo_tree.xml` 的 `0x11/0x0D` gate、斜坡和 90° 转向入口；树内新增 `SecondPreselectionKfsPickup`，在 `0x11` start-once 后沿 `+X` 搜索最近 R2 KFS，按 MF 口径做视觉横移对齐、odom 前向趋近、发送 `0x12` 夹取触发并用原目标视觉消失验证夹取成功。夹取后再执行红方基准 `+Y 0.7m -> +X 2.5m` 到放置观察位；旧固定第一段、观察前高抬节点及对应兼容参数已删除。`SecondPreselectionObserve` 新增放置前 KFS 可见性要求，默认动态九宫格 ROI 内一直没有有效 KFS 检测会失败停车，不再把空画面当作中列可放置；看到前方 KFS 后继续复用中层三格选空位、`selected_lateral`、`0x13` 放置和后退链路。
