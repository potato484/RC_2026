...
- `second_preselection_tree.xml`：第二个预选赛独立树。若不是从 branch gate 直达，会先执行 `SECOND_PRESELECTION_START(0x11)` 并等待同 `seq` 的 `SECOND_PRESELECTION_START_DONE(0x0D)`。`SecondPreselectionKfsPickup` 在首次取得新鲜 odom、尚未发布搜索速度时固化搜索原点 `x/y/yaw`，随后沿 `+X` 搜索 KFS、视觉横移对齐，按 `0x14/同 seq 0x12` 确认机械臂放下，根据锁定深度完成夹取趋近，再按 `0x12/同 seq 0x11` 和目标消失验证确认夹取。夹取后树按顺序执行 `OdomDriveX(+1.5m) -> OdomDriveY(red +0.75m / blue -0.75m) -> SecondPreselectionDriveToTotalX`；总 X 是当前位置相对搜索原点沿搜索起始 yaw 的有符号 odom 净投影，搜索、夹取趋近和固定 `+X 1.5m` 自动计入，入口斜坡与最终放置固定前进不计入，目标为 `4.2m`、容差 `0.03m`，进入节点时已经超调则停车成功且不倒退。随后 `SecondPreselectionKfsPlacePrepare` 只用中/下层视觉分层判断放置前准备：KFS 中心需位于画面宽度 `20%~80%`，中层高度 `12%~45%` 只表示当前检查点占据，下层高度 `45%~100%` 只作为横向中心对齐候选，上层不参与占据判断，也不要求上下框成对或读取深度。连续 `2` 帧中层占据时触发原两段横移避让，首次 red `+Y 0.56m` / blue `-Y 0.56m`，再次占据反向 `1.10m`；每次避让完成后会丢弃当前已有帧，只从下一张新视觉帧重新累计占据/清空稳定。若中层不存在且下层存在，则跳过避让，先以下层 KFS 做视觉横向中心对齐；若中层不存在且下层也不存在，则视为可放置并直接进入固定前进。若两段避让后仍只有中层、没有下层，也不失败，直接进入固定前进；若最终检查点同时有中层和下层，则先以下层完成横向中心对齐。放置准备完成后统一写入 `second_preselect_place_approach_distance_m=1.8`，`SecondPreselectionPlaceApproach` 仅按 odom 沿 `+X` 固定前进 `1.8m`，不再使用 D455 深度、`depth-arm_reach` 或无深度降级距离；固定前进超时使用 `second_preselect_place_fixed_forward_timeout_s=30.0`，超时停车后仍返回成功并继续后续唯一一次 ACK-only 下行 `SECOND_PRESELECTION_PLACE_KFS(0x13)`，再执行 `OdomDriveX(-1.5m)` 和原有上阶链。初始化、参数或运行上下文错误仍返回失败；可选 OpenCV UI 贯穿搜索夹取、分层占据判断和下层 KFS 放置对齐；失败、halt、odom/视觉/机构异常均保持停车语义。`start_r2_auto.sh` 的上行 `feedback_id=0x13` 红蓝切换监听器不改变这里的下行放置命令语义。
