...
- `mc_mf_preselection_tree.xml`：当前默认完整入口，结构参考主树，先等待人工 `FRONT_LIMIT_SWITCH_TRIGGERED(0x06)` 启动信号，再下发 `COMPETITION_START(0x10)` 通知下位机比赛开始，ACK 后继续等待同 `seq` 的 `COMPETITION_START_DONE(0x0C)`，随后采集 MC USB 相机空夹爪基准帧并延时 500ms 执行 `MCAreaTree`，待 MC 末尾通过配准前景视觉 gate 并延时 5s 后，再进入 `MFPreselectionAfterMCTree`。
...
- `mf_preselection_start_tree.xml`：带启动 gate 的 MF 预选独立入口，先复用 `WaitStartSignalAndNotify` 等待人工 `0x06`、下发 `COMPETITION_START(0x10)` 并等待同 `seq` 的 `COMPETITION_START_DONE(0x0C)`，随后执行与 `mf_preselection_tree.xml` 相同的可选入口导航和 `MfPreselectionFlow`。该树不作为当前默认入口，也不替换 MF 外部触发目标；需要启动 gate 时显式选择。
...
- `second_preselection_tree.xml`：第二个预选赛独立入口，不改变当前红/蓝默认 `behavior_tree_file`。流程为 `SECOND_PRESELECTION_START(0x11)` 等待同 `seq` 的 `SECOND_PRESELECTION_START_DONE(0x0D)`，随后按红方基准执行车体系 `+X 1.8m -> +Y 1.2m -> +X 2.5m`，blue 启动时由 `team_mirror_sign` 自动翻转 Y 段。高抬升后只观察一次九宫格动态 3x3 ROI：`SecondPreselectionObserve` 在观察开始时捕获 odom 参考点，后续用 `D=D0-Δx`、`S=S0+sign*Δy` 和 Color 640x480 内参把九宫格 9 个格子的实地中心投影到图像 ROI；列中心按红方图纸非等宽建模，左列宽 `0.54m`、中列宽 `0.58m`、右列宽 `0.50m`，所以中列到左列中心距 `0.56m`、中列到右列中心距 `0.54m`，每行中心距 `0.54m`。检测框中心点落入某格 ROI 且 `class_name` 非空时，该格本帧占据。当前放置策略只使用中层 `r=0` 三格，按中列、场地可移动空间侧、另一侧的顺序选择空格；场地可移动空间侧在 red 为车体 `+Y`，在 blue 为车体 `-Y`。选位结果会写入已镜像的 `second_preselection_selected_lateral_m`：默认中列被占时 red 场地可移动空间侧约 `+0.56m`，blue 镜像后约 `-0.56m`；另一侧分别约 `-0.54m` / `+0.54m`。随后执行一次 `OdomDriveY` 对齐选中列，再沿用 `+X 0.7m -> 0x13 -> -X 0.7m` 放置/后退。中层全占、视觉失败、odom 超时、命令超时或导航超时会停车并返回 `FAILURE`。
...
`WaitForRegistrationConfirmAction` 是 MC 到 MF 预选组合流程之间的视觉 gate。节点先读取 `VisualServoGrabAction` 写入的 `mc_tip_grab_confirmed`：若前序动作已经通过“`GRAB_TIP` 后端头持续消失”确认夹取完成，则直接返回 `SUCCESS`，因为此时端头可能已经离开 MC 相机视野，前景配准不再具备观测条件。若没有该确认，节点复用 MC 相机打开参数，读取 `WaitStartSignalAndNotifyAction` 写入黑板的空夹爪灰度基准帧；当前帧先在背景 ROI 内通过 LK 光流 + RANSAC 仿射配准到基准帧，特征提取会剔除中央端头 ROI，避免夹到端头后的大块前景污染配准。配准可信后，节点在中央端头 ROI 内做灰度差分，并按 `mc_registration_min_changed_area_px`、`mc_registration_min_changed_ratio`、`mc_registration_min_changed_bbox_height_px`、`mc_registration_min_changed_bbox_width_px`、`mc_registration_min_inliers`、`mc_registration_min_match_score` 和 `mc_registration_stable_frames` 确认端头前景稳定出现。成功返回 `SUCCESS`，超时、相机不可用、缺少基准帧或配准持续不可信返回 `FAILURE` 并写入 `decision_last_failure_*`；该节点只读相机，不发布 `/cmd_vel` 或机构命令。

`WaitStartSignalAndNotifyAction` 服务组合树和带启动 gate 的 MF 预选独立树：启动前的 `FRONT_LIMIT_SWITCH_TRIGGERED(0x06)` 表示人为开始信号，收到后通过 `/mechanism/send_command` 下发 `COMPETITION_START(0x10)` 空 payload，service ACK 成功后继续等待同 `seq` 的 `COMPETITION_START_DONE(0x0C)`；完成反馈到达后若 `mc_registration_gate_enable=true`，会等待 `mc_registration_capture_settle_s` 并采集一张 MC 相机灰度基准帧写入黑板，外层树再进入后续流程。MC `VisualServoGrabAction` 前探阶段继续把同一个 `0x06` 当作前方限位开关触发，两种语义通过不同 BT 阶段和独立节点隔离。
...
2026-07-03 同步：新增 `mf_preselection_start_tree.xml` 作为带启动 gate 的 MF 预选独立入口。该树在原 `mf_preselection_tree.xml` 流程前增加 `WaitStartSignalAndNotify`，用于需要人工 `0x06 -> 下行 0x10 -> 上行 0x0C` 握手后再进入 MF 预选的场景；现有 `mf_preselection_tree.xml`、红/蓝默认 `behavior_tree_file` 和 MF 外部触发目标保持不变。
...
2026-07-01 同步：新增 `mc_mf_preselection_tree.xml` 作为默认 MC + MF 预选组合入口，并新增 `mf_preselection_after_mc_tree.xml` 承载 MC 后置 MF 预选入口导航；原 `mf_preselection_tree.xml` 和 `mf_preselect_entry2_nav_*` 独立调试入口保持不变。组合树启动前先由 `WaitStartSignalAndNotify` 等待人工 `FRONT_LIMIT_SWITCH_TRIGGERED(0x06)`，随后下发 `COMPETITION_START(0x10)` 并在 ACK 后延时 500ms 进入 MC；MC 视觉夹取前探阶段仍独立消费 `0x06` 作为前方限位触发。`mc_tree.xml` 末尾不再停在 `WaitForever`，而是在旋转完成后通过 MC 末尾视觉 gate 等待确认，再延时 5s 返回成功进入 `mf_preselection_after_mc_tree.xml`。后置入口导航使用新的 `mc_to_mf_preselect_nav_*` 参数，路线为 `OdomDriveX(-2.4m) -> RelativeYawTarget(team 派生 -90deg) -> OdomTurnToYaw -> OdomDriveX(+1.6m)`；该改动只调整行为树编排、启动通知和 odom 单轴段参数，不新增 `/cmd_vel` 权威或 ROS topic/service/action。
