...
- `mc_mf_preselection_tree.xml`：first managed 完整入口，入口 `WaitPreselectionBranchGate` 同时监听 `/mechanism/command_feedback` 上的人工 `FRONT_LIMIT_SWITCH_TRIGGERED(0x06)` 与 `MF_PRESELECTION_TRIGGER(0x10)`。0x06 分支下发 `COMPETITION_START(0x10)` 并等待同 `seq` 的 `COMPETITION_START_DONE(0x0C)`，随后按 `preselection_entry_continue_delay_msec` 延时进入 `MCAreaTree -> MFPreselectionAfterMCTree`；0x10 分支下发 `SECOND_PRESELECTION_START(0x11)` 并等待同 `seq` 的 `SECOND_PRESELECTION_START_DONE(0x0D)`，成功后请求 `decision_node` halt 当前树并切换到 `mf_preselection_tree.xml`。
...
- `second_preselection_combo_tree.xml`：second managed 完整入口，入口 `WaitPreselectionBranchGate switch_tree_file="second_preselection_tree.xml"` 后执行斜坡子树、`RelativeYawTarget + OdomTurnToYaw` 转 90°，再进入 `SecondPreselectionTree`。入口 gate 收到 0x10 时会先发送 0x11 等 0x0D，再直接切到 `second_preselection_tree.xml`；该直达树会通过黑板键跳过重复 0x11。
...
`WaitPreselectionBranchGate` 是 first/second managed 入口的分支 gate。节点只消费 `/mechanism/command_feedback` 上的人工 `FRONT_LIMIT_SWITCH_TRIGGERED(0x06)` 与 `MF_PRESELECTION_TRIGGER(0x10)`，不采集 MC 相机基准帧，也不依赖 MC 末尾视觉配准。0x06 分支会通过 `/mechanism/send_command` 下发 `COMPETITION_START(0x10)`，等待同 `seq` 的 `COMPETITION_START_DONE(0x0C)` 后返回 `SUCCESS`；0x10 分支会下发 `SECOND_PRESELECTION_START(0x11)`，等待同 `seq` 的 `SECOND_PRESELECTION_START_DONE(0x0D)` 后写入黑板切树请求。`decision_node` 在本轮 tick 返回后消费该请求，halt 当前树并加载 `switch_tree_file` 指向的目标树；重复请求目标等于当前树时会被抑制。managed first/second 入口下，旧 `mf_preselection_external_trigger_*` 全局监听保持禁用，0x10 只能在 branch gate 内完成 0x11/0x0D 握手后切树。
...
2026-07-03 同步：新增 `WaitPreselectionBranchGate` 并以 first/second managed 入口替代历史启动 XML。`mc_mf_preselection_tree.xml` 入口 gate 的 0x06 分支继续 MC + MC 后置 MF 流程，0x10 分支先发送 `SECOND_PRESELECTION_START(0x11)` 并等待同 `seq` 的 `SECOND_PRESELECTION_START_DONE(0x0D)` 后切到 `mf_preselection_tree.xml`；`mc_tree.xml` 末尾删除 `WaitForRegistrationConfirm` 和 5s 视觉配准等待，旋转后改为同一 branch gate。新增 `preselection_ramp_forward_tree.xml` 与 `second_preselection_combo_tree.xml`，second managed 流程为入口 gate、斜坡两段前进、90° 转向、`second_preselection_tree.xml`。历史 `mf_preselection_start_tree.xml` 已删除。
...
2026-07-02 同步：新增 MCU 上行 `MF_PRESELECTION_TRIGGER(0x10)` 历史全局触发能力；当前 managed first/second 入口下该能力被配置禁用，0x10 由 `WaitPreselectionBranchGate` 接管，并在 0x11/0x0D 握手完成后再切树。
