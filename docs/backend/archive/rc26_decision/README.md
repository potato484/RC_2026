...
- `second_preselect_*`：第二个预选赛独立树参数。命令参数仍为 `0x11/0x0D` 开始握手、`0x14/0x12` 机械臂放下握手、`0x12/0x11` KFS 夹取触发与动作完成握手、下行 `0x13` ACK-only 放置；搜索/夹取链继续使用 `second_preselect_search_*`、R2/R1 标签集合、`second_preselect_kfs_align_*`、`second_preselect_kfs_approach_*`、bbox 深度采样、可选单目兜底和夹取消失验证参数。夹取后导航改用 `second_preselect_post_pickup_forward_x_m=1.5`、`second_preselect_nav_y1_m=0.75`、`second_preselect_total_x_target_m=4.2`、`second_preselect_total_x_tolerance_m=0.03`；`second_preselect_nav_x2_m` 与 `second_preselect_place_forward_x_m` 已从正式流程和红蓝配置删除。放置准备新增 `second_preselect_place_occupied_center_x_min_ratio/max_ratio`、`upper_y_min_ratio/split_y_ratio`、`max_center_dx_px`、稳定帧、两级横移距离、横移速度和超时参数；红蓝方向通过内部 `team_mirror_sign` 派生。最终放置只使用 MF bbox 多 ROI 返回的真实深度，`second_preselect_place_arm_reach_m=0.45` 独立于夹取臂长，`second_preselect_place_approach_timeout_s=8.0` 超时只降级为停车后继续下发首次 `0x13`，初始化或上下文错误仍失败。放置后仍由 `second_preselect_post_place_retreat_x_m=-1.5` 和 `second_preselect_post_place_*` 参数完成后退与上阶；人工前轮激光 `0x15` 不等同于台阶前轮高度突变 `0x04`。`second_preselect_dynamic_roi_ui_enable` 控制搜索夹取、占位判断和任意 KFS 放置对齐的本地 OpenCV 调试窗口。上行 `feedback_id=0x13` 只由 `start_r2_auto.sh` 监听器消费并写回 `r2_active_side.yaml`，不属于 decision 行为树内部切换。
...
2026-07-10 同步：第二预选赛在 `SecondPreselectionKfsPickup` 首次搜索速度发布前记录新鲜 odom 搜索原点；夹取后执行 `+X 1.5m -> red +Y 0.75m / blue -Y 0.75m -> 总 X 净投影补齐至 4.2m`。新增 `SecondPreselectionDriveToTotalX`、`SecondPreselectionKfsPlacePrepare`、`SecondPreselectionPlaceApproach`：前方上下双框连续两帧占据时先镜像横移 `0.56m`，再次占据反向 `1.10m`，第三次仍占据直接降级放置；清空后对任意真实深度有效 KFS 横移对齐，稳定深度中位数减独立臂长 `0.45m` 后趋近，`8s` 超时停车并继续唯一一次首次 `0x13`。正式流程删除 `second_preselect_nav_x2_m`、`second_preselect_place_forward_x_m` 和 R1 专用放置对齐节点，保留 `-X 1.5m` 后退与原放置后上阶链。
