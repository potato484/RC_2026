...
2026-07-05 同步：第二预选赛放置链删除九宫格动态 ROI 观察、中层选空位、`selected_lateral` 横移和放置后后退。`second_preselection_tree.xml` 现在在夹取验证成功后执行 `+Y 0.7m`（blue 镜像）和 `+X 4.5m`，然后 `SecondPreselectionR1KfsPlaceAlign` 观察前方最近 `R1_KFS` 并视觉横移对齐，对齐后 `+X 0.8m` 前进，最后发送 ACK-only `SECOND_PRESELECTION_PLACE_KFS(0x13)`。
