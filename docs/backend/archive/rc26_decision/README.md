...
- 本树不调用 `NavToPose`，不做连续位姿归正；它验证 `PlanGridTransition -> GridTurn -> GridHeadingAlign -> GridTransition` 的同一套 MF 离散格间动作、yaw 对齐、台阶执行与状态提交，并在到达 `grid10` 后复用独立下台阶状态机完成梅林外离场。最后一段离场不再提交新的 `current_grid`，黑板中的离散格仍停在 `10`。
