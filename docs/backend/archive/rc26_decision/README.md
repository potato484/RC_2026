...
- `second_preselect_after_ramp_turn_delta_rad`、`second_preselect_after_ramp_turn_timeout_s`：历史 second managed 斜坡后 90° 相对转向参数；当前默认 `second_preselection_combo_tree.xml` 不再使用，`0x06` 分支斜坡后停车等待人工触发外部限位 2 上行 `0x10`，并在再次完成 `0x11/0x0D` 握手后进入 `SecondPreselectionTree` 搜寻；入口 `0x10` 分支完成首次 `0x11/0x0D` 握手后直接切到 `second_preselection_tree.xml`。
...
2026-07-09 同步：second 默认组合树对齐 `mc_mf_preselection_tree.xml` 的入口分支模型。`second_preselection_combo_tree.xml` 先由入口 `WaitPreselectionBranchGate` 同时等待上行 `0x06/0x10`，两条分支都执行 `SECOND_PRESELECTION_START(0x11)` / `SECOND_PRESELECTION_START_DONE(0x0D)` 握手；`0x06` 分支继续合并斜坡并停车等待斜坡后上行 `0x10`，收到后再次执行 `0x11/0x0D` 握手，再进入内嵌 `SecondPreselectionTree` 搜寻；入口 `0x10` 分支直接请求切到 `second_preselection_tree.xml`，不走斜坡。组合树删除斜坡后 `RelativeYawTarget + OdomTurnToYaw`；gate 对 second profile 的 continue/switch 分支都会写入 second-start-done 黑板键，避免进入独立树时重复发送 `0x11`。
