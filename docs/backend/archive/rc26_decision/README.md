...
`decision_node` 启动时会加载 `r2_runtime.decision.ros__parameters`，把共享参数写入 blackboard，并注册 MC、MF、MF 预选赛、台阶和 odom 导航节点。`team` 是当前红蓝方场地镜像参数：`red` 使用 YAML 中维护的红方基准路线，`blue` 在启动加载参数时自动派生侧向 Y 和 yaw 的镜像值；非法值会告警并按 `red` 运行。`startup_wait_for_odom` 仍是可选能力，但标准红/蓝自动配置已将它设为 `false`：`start_r2_auto.sh` 启动后立即创建并 tick 行为树，不再由整树启动 gate 等待 `/odom`；每个 odom 闭环动作仍独立等待新鲜 `/odom`，因此关闭全局 gate 不等于允许无里程计盲走。
...
- `odom_timeout_s`
- `timeout_s`
- `succeed_on_reach_or_overshoot`：默认 `false`；开启后轴向到达容差或越过目标即停车成功，不发布反向回拉速度。
- `succeed_on_timeout`：默认 `false`；开启后，仅在已经捕获有效 odom 目标的前提下，动作达到 `timeout_s` 时停车并返回成功。没有建立目标、输入非法和运行上下文错误仍失败。
...
- `mc_repeat_preselection_tree.xml`：first managed 默认入口，只执行 MC-only 可重复流程，不进入 `MFPreselectionAfterMCTree`。每轮先由入口 gate 只接受人工触发外部限位 1 `MANUAL_LIMIT_SWITCH_1_TRIGGERED(0x06)`，下发 `COMPETITION_START(0x10)` 并等待同 `seq` 的 `COMPETITION_START_DONE(0x0C)`，随后按 `preselection_entry_continue_delay_msec` 延时进入 `MCAreaTree`；`MCAreaTree` 末尾 gate 只接受人工触发外部限位 2 `MANUAL_LIMIT_SWITCH_2_TRIGGERED(0x10)`，再执行同一组 `0x10/0x0C` 握手作为舵机重新放下动作，握手完成后不切树。`MCPreselectionRepeatControl` 默认开启，第 0 轮使用当前红/蓝运行配置中的 `mc_nav_forward_x_m`，后续重复轮按当前 `active_side` 对应的带符号 step 调整后写入 `mc_preselection_effective_forward_x_m`。
...
- `mc_tree.xml`：`OdomDriveX(mc_preselection_effective_forward_x_m) -> RelativeYawTarget(mc_nav_right_turn_delta_rad=team 派生侧向 yaw) -> OdomTurnToYaw(mc_nav_right_turn_target_yaw) -> OdomDriveX(mc_nav_reverse_x_m) -> VisualServoGrab -> Delay(500ms) -> RotateInPlace -> Delay(500ms) -> OdomDriveX(-0.4) -> OdomDriveY(mc_after_rotate_retreat_y_m) -> WaitPreselectionBranchGate`。MC 正式流程已回退为旧串行去程和旋转后 X/Y 退让，不再调用 `OdomDriveXTurnX` 或 `RotateRetreat`；其中夹取后旋转完成的 Y 段由红/蓝运行配置显式给出，当前红方为 `-0.4m`，蓝方为正向 `0.2m`。`VisualServoGrab` 在进入视觉阶段后会保持右转后的目标 odom yaw；无端头时固定沿车体系 `+Y` 低速横移搜寻，不随 `team` 镜像，直到端头出现；同屏多个端头时初次获锁优先选择画面左侧 bbox，随后在锁定窗口内持续跟踪同一物理端头。端头框短暂不稳定时，MC 视觉伺服沿用最近 offset 并按 `mc_align_lost_servo_speed_scale` 低速继续伺服，重新识别后用 `mc_align_offset_filter_alpha` 平滑 offset，再参与稳定帧和夹取触发；若最近 offset 位于 `mc_align_lost_search_suppress_px` 近中心保护区内，即使丢框帧数超过普通短丢阈值，也会在 `mc_align_lost_search_suppress_timeout_s` 内禁止切入固定方向搜索，并按 `mc_align_near_center_lost_speed_scale` 低速补偿或停车等待，避免模型居中附近短暂丢框时把端头扫出画面。MC 末尾正式流程不再执行 `WaitForRegistrationConfirm` 或 5s 视觉配准等待，旋转前后各保留 0.5s 延时，再进入可由 XML/黑板配置的 branch gate。默认 first 重复树把该 gate 配置为只收 0x10 且不切树；旧组合树仍保持 0x06/0x10 分支。右转后的目标 odom yaw 会传给 `VisualServoGrab target_yaw_rad`，作为视觉阶段 heading hold 目标；`team=blue` 时侧向 yaw 和后续旋转方向相对红方基准取反。
...
- `preselection_ramp_forward_tree.xml`：second managed 入口的合并斜坡子树，执行 `SecondPreselectionRampForward(preselection_ramp_forward_x_m)`；该动作只沿 `+X` 发布，达到或超调立即停车成功，`preselection_ramp_forward_timeout_s` 超时也停车成功并继续后续逻辑，不会发布负 `linear.x` 拉回目标点。
- `second_preselection_combo_tree.xml`：second managed 组合入口，入口 `WaitPreselectionBranchGate` 保持同时接受人工触发外部限位 1/2 上行 `0x06/0x10`，两条分支都使用 `switch_start_profile="second"` / `continue_start_profile="second"` 下发 `SECOND_PRESELECTION_START(0x11)`；正常路径只接受同 `seq` 的 `SECOND_PRESELECTION_START_DONE(0x0D)`，机构 service/ACK/done 超时、拒绝、请求异常或非 BUSY `0xFE` 则告警并按握手逻辑完成放行。入口直接收到 `0x10` 时，握手完成后请求切换到 `second_preselection_climb_place_tree.xml`，不执行斜坡；入口收到 `0x06` 时，首次握手完成后执行 `PreselectionRampForwardTree` 合并斜坡前进并停车，再由只接受 `0x10` 的斜坡后 gate 完成第二次握手并请求切换到同一目标树。两个 gate 提交切树请求后都保持 `RUNNING`，`decision_node` 在当前 tick 结束后 halt 组合树并加载目标树，因此组合树不会继续执行其它动作，也不再 include 或内嵌 `SecondPreselectionTree`。
- `second_preselection_kfs_compat_combo_tree.xml`：仅由 `r2_active_side.yaml` 的 second KFS 兼容开关选择。它保持默认组合树的 `0x06/0x10` 分支、`0x11/0x0D` 握手、合并斜坡和切树时机不变，但两个 gate 都切换到 `second_preselection_kfs_compat_climb_place_tree.xml`。显式 `runtime_config_file` 覆盖 managed selector 时不会自动启用该 profile。
- `second_preselection_tree.xml`：保留的独立完整第二预选树，不再由 `preselection_mode: second` 的 managed 组合入口调用。显式运行时会先执行 `SECOND_PRESELECTION_START(0x11)` 并等待同 `seq` 的 `SECOND_PRESELECTION_START_DONE(0x0D)`。`SecondPreselectionKfsPickup` 在首次取得新鲜 odom、尚未发布搜索速度时固化搜索原点 `x/y/yaw`，随后沿 `+X` 搜索 KFS、视觉横移对齐，按 `0x14/同 seq 0x12` 确认机械臂放下，根据锁定深度完成夹取趋近，再按 `0x12/同 seq 0x11` 和目标消失验证确认夹取；若验证期内有新视觉帧但原目标仍未稳定消失，达到 `second_preselect_grab_verify_timeout_s` 后会停车告警并进入 settle 继续后续流程，无新视觉帧仍失败。夹取后树按顺序执行 `OdomDriveX(+1.5m) -> OdomDriveY(red +0.75m / blue -0.75m) -> SecondPreselectionDriveToTotalX`；其中固定 `OdomDriveX/OdomDriveY` 段启用 `succeed_on_reach_or_overshoot=true`，到达或超调即停车成功，不等待 yaw/stable ticks 做反向回拉；总 X 是当前位置相对搜索原点沿搜索起始 yaw 的有符号 odom 净投影，搜索、夹取趋近和固定 `+X 1.5m` 自动计入，入口斜坡与最终放置固定前进不计入，目标为 `4.2m`、容差 `0.03m`，进入节点时已经超调则停车成功且不倒退；该补齐段属于夹取后导航补偿，速度上限使用 `second_preselect_nav_max_speed_mps`。随后 `SecondPreselectionKfsPlacePrepare` 只用中/下层视觉分层判断放置前准备：KFS 中心需位于画面宽度 `20%~80%`，中层高度 `12%~45%` 只表示当前检查点占据，下层高度 `45%~100%` 只作为横向中心对齐候选，上层不参与占据判断，也不要求上下框成对或读取深度。连续稳定帧数由 `second_preselect_place_occupied_stable_frames` 控制；中层占据稳定后触发原两段横移避让，首次 red `+Y 0.56m` / blue `-Y 0.56m`，再次占据反向 `1.10m`；每次避让完成后会丢弃当前已有帧，只从下一张新视觉帧重新累计占据/清空稳定。若中层不存在且下层存在，则跳过避让，先以下层 KFS 做视觉横向中心对齐；若中层不存在且下层也不存在，则视为可放置并直接进入固定前进。若两段避让后仍只有中层、没有下层，也不失败，直接进入固定前进；若最终检查点同时有中层和下层，则先以下层完成横向中心对齐。每个观察 checkpoint 若连续 `second_preselect_place_observe_timeout_s` 无法得到稳定判断，也进入固定前进，写入 `PLACE_OBSERVE_TIMEOUT_FIXED_FORWARD`，不直接失败也不跳过到 `0x13`；下层 KFS 横向对齐仍由 `second_preselect_kfs_align_timeout_s` 控制。放置准备完成后统一写入 `second_preselect_place_approach_distance_m=1.8`，`SecondPreselectionPlaceApproach` 仅按 odom 沿 `+X` 固定前进 `1.8m`，不再使用 D455 深度、`depth-arm_reach` 或无深度降级距离；固定前进超时使用 `second_preselect_place_fixed_forward_timeout_s`，超时停车后仍返回成功并继续后续唯一一次 ACK-only 下行 `SECOND_PRESELECTION_PLACE_KFS(0x13)`，再执行 `OdomDriveX(-1.5m)` 和原有上阶链。初始化、参数或运行上下文错误仍返回失败；可选 OpenCV UI 贯穿搜索夹取、分层占据判断和下层 KFS 放置对齐；失败、halt、odom/视觉/机构异常均保持停车语义。`start_r2_auto.sh` 的上行 `feedback_id=0x13` 红蓝切换监听器不改变这里的下行放置命令语义。
- `second_preselection_post_place_climb_tree.xml`：第二预选赛放置后上阶独立调试树，只执行 `SecondPreselectionPostPlaceClimb`。该树不包含放置后的 `OdomDriveX(second_preselect_post_place_retreat_x_m)` 后退段，也不执行 KFS 搜索、占位避让、任意 KFS 放置对齐或下行 `0x13` 首次放置；使用前需要人工确认机器人已经处于放置后上阶流程起点。
- `second_preselection_climb_place_tree.xml`：既可显式选择调试，也是 `preselection_mode: second` 两条 `0x10` 分支的统一切树目标。树内不新增外部限位 gate，不发送 `0x11`，也不等待 `0x0D`；managed 使用时启动握手已由组合树完成。切树后的下一轮 tick 先进入 `SecondPreselectionClimbPlacePreloadPickup`，仅下发一次预装 KFS 夹取 `0x15` 并只等待通用 ACK；ACK 成功、拒绝、service/请求异常或 ACK 超时后都立即执行 `OdomDriveX(second_preselect_climb_place_forward_x_m) -> OdomDriveY(red +0.3m / blue -0.3m) -> Delay(20s)`，不订阅也不等待同 seq `0x14`。`SecondPreselectionClimbFrontStage` 在前推杆伸出前尝试 yaw 对齐；对齐成功时正常继续，对齐失败或超时时先发布零速并记录 WARN，但不再让行为树失败，仍发送前推杆伸出 `0x08` 并在 ACK 后零速稳定 `stair_climb_front_extend_delay_s`；稳定完成后仍持续发布全零 `/cmd_vel`，不再调用前轮行驶速度曲线，也不做 yaw 运动，直到收到本等待阶段开始后的无 seq 上行人工前激光 `0x15`。旧 `0x15`、错误反馈和后轮 `0x05` 均不会放行；等待超过 `stair_front_event_timeout_s` 时失败停车。有效人工 `0x15` 到达后才并发发送前收 `0x09` 与后伸 `0x0A`，稳定后由独立 `OdomDriveX(+0.6m)` 推动后轮上阶；该节点单独启用 `succeed_on_timeout=true`，在已经捕获有效 odom 目标后达到 `second_preselect_climb_place_rear_timeout_s` 时会先发布零速、记为成功并继续后续逻辑。尚未捕获有效 odom 目标、距离输入非法或运行上下文缺失仍返回失败。后轮段完成即进入 `SecondPreselectionRearRetractPlace`：节点立即下发后收 `0x0B`，并从进入节点、也就是上阶完成时点开始累计 25s；`0x0B` 机构步骤结束且延时到达后发送最终 `0x13`。收尾节点不再发送预装夹取 `0x15`，也不再等待 `0x14`；`0x0B/0x13` 的机构失败同样只告警并继续或正常结束目标树。
- `second_preselection_kfs_compat_climb_place_tree.xml`：second managed 无视觉 KFS 兼容目标树。切树后立即用通用 `SecondPreselectionCommand` 下发机械臂放下 `0x14`，只接受同 seq `0x12` 完成反馈；随后按 `second_preselect_pre_approach_lower_settle_s=0.5` 派生的内部 blackboard 毫秒值停车稳定，再执行现有 `OdomDriveX(+1.5m)`。前进完成后下发 KFS 夹取 `0x12`，只接受同 seq `0x11` 完成反馈，然后直接进入现有 `OdomDriveY`、20s、人工前激光和后轮 odom 路线。两个机构命令均使用可靠 ACK 和统一机构容错语义；树内不再包含 `SecondPreselectionKfsPickup`，不会启动视觉搜索、横向对齐、深度趋近或目标消失验证，也不执行树首预装 `0x15`。最后进入 `SecondPreselectionRearRetractCompatPickupPlace`：立即可靠处理 `0x0B`，从节点进入时累计 25s；时间到后单次发起 `0x15` 且 service 请求 `wait_ack=false`，不等待 service response、MCU ACK 或 `0x14`，无论 service 是否可用或请求是否成功都立即开始 `second_preselect_climb_place_compat_pickup_to_final_delay_s=10s` 固定延时，之后可靠发送最终 `0x13`。全阶段持续发布零 `/cmd_vel`，`0x0B/0x13` 仍使用统一机构容错语义。
- 机构命令统一容错边界：`StairActionBase`、`MfPreselectionFlow`、MC 夹取、managed/旧启动 gate、独立 second 和上阶夹取放置节点在机构 service 不可用、请求异常、`accepted=false`、ACK/业务完成反馈超时、串口暂不可用或非 BUSY 两字节 `0xFE` 时记录 WARN，并按该机构步骤成功结束时相同的状态转移继续；并发命令在两条请求分别进入成功或容错终态后继续，整体超时也直接推进。`BUSY(0x01)` 仍继续等待原有最终反馈上限。以上容错不写终止性的 `decision_last_failure_*`；blackboard/node 缺失、非法参数、内部状态损坏，以及人工限位/激光、普通 odom、视觉等非机构错误仍保持原失败或专用容错语义。
...
旧外部 action 位姿导航节点、TF 采点节点、旧双点位姿测试树和相关 action helper 已删除。当前 MC/MF 入口坐标不再是绝对地图位姿；`mc_nav_forward_x_m` / `mc_nav_reverse_x_m` 是 MC 复合 `X-turn-X` 动作的两段 X 距离，`mf_preselect_entry2_nav_segment1_*` 与 `mc_to_mf_preselect_nav_*` 仍是按启动姿态和分段动作顺序标定的相对单轴段。YAML 中 yaw/Y 路线值按红方基准维护并由 `team` 镜像；first MC 重复流程第 0 轮直接使用红/蓝配置中已有的 `mc_nav_forward_x_m` 现场标定值，后续重复距离由 `r2_active_side.yaml` 中当前 `active_side` 对应的带符号 `first_preselection_mc_repeat_forward_x_step_m` 写入 `mc_preselection_effective_forward_x_m`。

## MC 链路

`VisualServoGrabAction` 内嵌相机采集和端头推理，在工作线程中执行目标锁定、无端头固定 `+Y` 横移搜寻、横移 P 控制、短暂丢框预测伺服、近中心丢框禁止固定搜索保护、odom yaw 姿态保持、限位前探和 `GRAB_TIP(0x01)` service 请求。它只通过 `/cmd_vel`、`/mechanism/send_command` 和 `/mechanism/command_feedback` 与下层交互，不直接打开串口。

2026-07-12 同步：MC `VisualServoGrab` 无端头搜索方向统一为车体系 `+Y`，不再随 `team_mirror_sign` 在 red/blue 间取反；`mc_align_search_speed_mps` 仍只表示速度幅值，并继续受 `mc_align_max_speed_mps` 限幅。该改动只影响端头未识别时的 `/cmd_vel.linear.y` 搜寻方向，不改变右转 yaw 镜像、视觉 P 对齐、短丢框保护、前探限位或夹取命令。

2026-07-09 同步：MC `VisualServoGrab` 增加近中心丢框保护。新增 `mc_align_lost_search_suppress_px`、`mc_align_lost_search_suppress_timeout_s` 和 `mc_align_near_center_lost_speed_scale` 参数；当模型在居中附近短暂丢失端头框且最近 offset 仍落入保护区时，视觉伺服不会立即恢复 team 派生固定方向搜索，而是在限时窗口内按最近 offset 极低速补偿或停车等待重新识别。该改动只调整 `VisualServoGrab` 内部 `/cmd_vel` 横移策略，不新增 ROS topic/service/action、机构协议或新的运动命令权威。
...
`WaitPreselectionBranchGate` 是 first/second managed 入口和 MC 末尾的分支 gate。节点只消费 `/mechanism/command_feedback` 上的人工触发外部限位 1 `MANUAL_LIMIT_SWITCH_1_TRIGGERED(0x06)` 与人工触发外部限位 2 `MANUAL_LIMIT_SWITCH_2_TRIGGERED(0x10)`，不采集 MC 相机基准帧，也不依赖 MC 末尾视觉配准。XML 通过 `accepted_branch` 选择 `both`、`continue_only` 或 `switch_only`；不被当前 gate 接受的 0x06/0x10 会记录节流告警并忽略。进入等待分支阶段时，gate 会在 `/decision/preselection_gate_state` 发布 transient-local JSON 状态（包含 `waiting`、`gate`、`accepted_branch`、`continue_start_profile`、`switch_start_profile`），供 bringup 启动就绪通知判断“上位机已经在等人工限位”。该状态只用于启动协调，不改变行为树控制权，也不新增运动命令权威。`continue_start_profile` 和 `switch_start_profile` 决定分支握手使用 `mc` 还是 `second` profile：`mc` profile 下发 `COMPETITION_START(0x10)` 并等待同 `seq` 的 `COMPETITION_START_DONE(0x0C)`，`second` profile 下发 `SECOND_PRESELECTION_START(0x11)` 并等待同 `seq` 的 `SECOND_PRESELECTION_START_DONE(0x0D)`。gate 可通过 `continue_pre_command_delay_msec` / `switch_pre_command_delay_msec` 在收到对应人工触发限位后、下发启动命令前等待；0x06 分支握手成功后返回 `SUCCESS` 继续当前树；0x10 分支握手成功后若 `switch_tree_file` 为空则返回 `SUCCESS`，否则写入黑板切树请求。旧全局 0x10 监听链路已删除，0x10 只能在 branch gate 内按当前 XML gate 语义消费。
...
- `startup_wait_for_odom` / `startup_odom_*`：可选的整树启动前 odom 新鲜度和低速稳定 gate；默认红/蓝自动配置关闭，独立右转验证入口仍显式开启。
- `team`：红蓝方场地镜像选择；`red` 使用红方基准，`blue` 自动镜像 MC/MF/第二预选赛侧向 Y 和 yaw，非法值按 `red`。
- `mc_nav_forward_x_m`、`mc_nav_right_turn_delta_rad`、`mc_nav_reverse_x_m`、`mc_nav_timeout_sec`：MC 去程旧串行 `X -> yaw -> X` 路线参数，继续保留红/蓝运行配置中的现场标定值。first 默认重复树把 `mc_nav_forward_x_m` 作为第 0 轮距离；运行时每轮实际传给 MC 的是内部黑板值 `mc_preselection_effective_forward_x_m`。`mc_nav_right_turn_delta_rad` 和旋转方向继续按 `team` 镜像；无端头搜寻方向固定为车体系 `+Y`，`mc_nav_timeout_sec` 是各段动作超时输入。
...
- `first_preselection_mc_repeat_enable`、`first_preselection_mc_repeat_max_count`、`first_preselection_mc_repeat_forward_x_step_m.red/blue`：first MC-only 重复控制参数。默认开启；`max_count` 表示初始 MC 后最多重复次数。第 0 轮距离来自当前红/蓝运行配置中的 `mc_nav_forward_x_m`，后续重复轮直接叠加当前 `active_side` 对应的带符号 step。当前 red first 为 `0.05m -> 0.25m -> 0.45m`，当前 blue first 为 `1.05m -> 0.85m -> 0.65m`。
- `preselection_ramp_forward_x_m`、`preselection_ramp_max_speed_mps`、`preselection_ramp_min_speed_mps`、`preselection_ramp_forward_timeout_s`：second managed 合并斜坡前进参数；默认 `5.00m` / `5.0s`，只允许非负 X 速度，超调和超时都停车后继续。
- `second_preselect_after_ramp_turn_delta_rad`、`second_preselect_after_ramp_turn_timeout_s`：历史 second managed 斜坡后 90° 相对转向参数；当前默认 `second_preselection_combo_tree.xml` 不再使用。`0x06` 分支斜坡后停车等待人工触发外部限位 2 上行 `0x10`，再次完成 `0x11/0x0D` 握手后切到 `second_preselection_climb_place_tree.xml`；入口 `0x10` 分支完成首次握手后也直接切到同一目标树。
...
- `second_preselect_*`：第二个预选赛独立树参数。命令参数仍为 `0x11/0x0D` 开始握手、`0x14/0x12` 机械臂放下握手、`0x12/0x11` KFS 夹取触发与动作完成握手、下行 `0x13` ACK-only 放置；搜索/夹取链继续使用 `second_preselect_search_*`、R2/R1 标签集合、`second_preselect_kfs_align_*`、`second_preselect_kfs_approach_*`、bbox 深度采样、可选单目兜底和夹取消失验证参数。`second_preselect_grab_verify_timeout_s` 现在也是夹取消失验证兜底上限：有新帧但原目标仍可见时超时后继续后续流程，无新视觉帧仍失败。夹取后导航使用 `second_preselect_post_pickup_forward_x_m=1.5`、`second_preselect_nav_y1_m=0.75`、`second_preselect_nav_max_speed_mps/min_speed_mps`、`second_preselect_total_x_target_m=4.2`、`second_preselect_total_x_tolerance_m=0.03`；`second_preselection_tree.xml` 内三段 `OdomDriveX/Y` 与自定义 `SecondPreselectionDriveToTotalX` 均使用 second 专用导航速度口径，其中三段固定 `OdomDriveX/Y` 开启到达/超调即成功，不再复用通用 `odom_relative_nav_max_speed_mps/min_speed_mps`；`second_preselect_nav_x2_m` 与 `second_preselect_place_forward_x_m` 已从正式流程和红蓝配置删除。放置准备使用 `second_preselect_place_observe_timeout_s`、`second_preselect_place_occupied_center_x_min_ratio/max_ratio`、`second_preselect_place_occupied_middle_y_min_ratio/max_ratio`、`second_preselect_place_occupied_lower_y_min_ratio/max_ratio`、稳定帧、两级横移距离、横移速度和超时参数；观察阶段超时后进入固定前进，`second_preselect_kfs_align_timeout_s` 只控制下层 KFS 对齐阶段；中层只表示占据，下层只做横向中心对齐，红蓝避让方向通过内部 `team_mirror_sign` 派生。最终放置固定使用 `second_preselect_place_fixed_forward_x_m=1.8`，前进超时使用 `second_preselect_place_fixed_forward_timeout_s` 且超时后仍继续 0x13 放置；旧深度放置参数和旧上下框成对配对阈值已从代码与红蓝配置删除，放置阶段不再依赖真实深度、无深度等待、降级累计距离或深度减机械臂距离计算。`second_preselect_kfs_approach_timeout_s=8.0` 仍用于初次夹取前向趋近，不再控制最终放置前进。放置准备会写入当前模式、终止原因和固定前进距离等 `second_preselect_place_*` blackboard 观测状态。放置后仍由 `second_preselect_post_place_retreat_x_m=-1.5` 和 `second_preselect_post_place_*` 参数完成后退与上阶；人工前轮激光 `0x15` 不等同于台阶前轮高度突变 `0x04`。`second_preselect_dynamic_roi_ui_enable` 控制搜索夹取、分层占据判断和下层 KFS 放置对齐的本地 OpenCV 调试窗口。上行 `feedback_id=0x13` 只由 `start_r2_auto.sh` 监听器消费并写回 `r2_active_side.yaml`，不属于 decision 行为树内部切换。
- `second_preselect_climb_place_*`：默认和 KFS 兼容上阶目标树的共享参数。默认树首使用 `preload_pickup=0x15` 并只等通用 ACK；兼容树首改用 best-effort `SecondPreselectionKfsPickup`。两者共享 `forward_x=1.5m`、红方基准 `lateral_y=+0.3m`、`pre_climb_delay=20000ms`、人工前激光上阶和后轮 `rear_forward_x=0.6m` 参数。默认收尾使用 `rear_retract=0x0B`、`final_delay=25s`、`final=0x13`；兼容收尾在相同 25s gate 后使用 `preload_pickup=0x15(wait_ack=false)`，等待新增的 `compat_pickup_to_final_delay_s=10s` 后再发送 `final=0x13`。`preload_pickup_done_feedback_id=0x14` 继续作为配置兼容字段，但默认树首和兼容收尾都不消费。
...
2026-07-14 同步：second managed 的 KFS 兼容 profile 改为无视觉确定性动作。`second_preselection_kfs_compat_combo_tree.xml` 保持原 `0x06/0x10 -> 0x11/0x0D` 与斜坡语义，但兼容目标树按 `0x14/同 seq 0x12 -> 0.5s -> OdomDriveX(+1.5m) -> 0x12/同 seq 0x11` 放下并夹取，随后继续原上阶路线，不再调用 `SecondPreselectionKfsPickup` 或任何视觉验证。兼容收尾仍为 `0x0B -> 从节点进入起累计25s -> 单次0x15(wait_ack=false) -> 固定10s -> 0x13`；默认组合树、默认目标树和独立完整视觉 second 树保持原行为。
...
2026-07-13 同步：second managed 统一目标树把预装夹取 `0x15` 从上阶收尾调到树首。新增 `SecondPreselectionClimbPlacePreloadPickup`，进入目标树后立即且只发送一次 `0x15`；2026-07-14 起该节点只等待通用 ACK，不再等待同 seq `0x14`，随后允许首段 OdomDrive。原收尾节点改为 `SecondPreselectionRearRetractPlace`，后轮上阶完成后只发送 `0x0B`，从进入收尾节点时起累计 25s，满足延时后发送 `0x13`。后段不再发送 `0x15` 或等待 `0x14`；ROS 接口、MCU 编号和现有运行参数名保持不变。

2026-07-13 同步：`SecondPreselectionClimbFrontStage` 的上阶前 yaw 对齐改为 best-effort。对齐成功时行为不变；对齐失败、无有效 heading 或达到对齐超时时，节点先发布零速并记录 WARN，然后仍发送前推杆伸出 `0x08`，继续零速等待人工前激光 `0x15` 和后续流程。2026-07-14 起 `0x08` 等机构命令失败也按统一容错推进；运行上下文初始化失败和人工 `0x15` 等待超时仍保持失败停车。

2026-07-13 同步：通用 `OdomDriveX/OdomDriveY` 新增默认关闭的 `succeed_on_timeout` 端口，仅 `second_preselection_climb_place_tree.xml` 的后轮 `+X 0.6m` 节点启用。该段在捕获有效 odom 目标后达到当前红蓝配置的 `5.0s` 动作超时时发布零速、返回成功并继续 `SecondPreselectionRearRetractPlace`；未建立 odom 目标、输入非法和运行上下文错误仍保持失败。该扩展不改变 ROS 接口、MCU 协议、blackboard 公共键或其它 OdomDrive 节点的默认超时语义。

2026-07-13 同步：`SecondPreselectionClimbFrontStage` 的前推杆后人工激光阶段改为强制零速等待。节点仍在 `0x08` 前完成 yaw 对齐，并在 `0x08` ACK 后保留 `stair_climb_front_extend_delay_s` 零速稳定；此后每个 tick 持续发布全零 `/cmd_vel`，不再使用前轮速度曲线或 yaw gate，直到收到该等待阶段开始后的人工前激光 `0x15` 才发送前收 `0x09` 与后伸 `0x0A`。人工事件超时、错误反馈、失败和 halt 继续停车；通用 `StairClimb` 行驶语义、ROS 接口、MCU ID 和参数集合均不变。

2026-07-13 同步：`preselection_mode: second` 的两个 `0x10` 分支统一改为在完成各自 `SECOND_PRESELECTION_START(0x11)` / 同 seq `SECOND_PRESELECTION_START_DONE(0x0D)` 握手后切换到 `second_preselection_climb_place_tree.xml`。入口直接 `0x10` 跳过斜坡；入口 `0x06` 在首次握手后完成合并斜坡并停车，斜坡后 `0x10` 再次握手后切到同一树。组合树删除 `second_preselection_tree.xml` include 和内嵌 `SecondPreselectionTree`；`second_preselection_tree.xml` 继续作为可显式选择的独立完整第二预选树。该调整不新增 ROS 接口、BT 节点端口、运行参数或 MCU 协议编号。

2026-07-13 同步：`second_preselection_climb_place_tree.xml` 既可通过显式 `tree_file` 选择，也作为 second managed 统一目标树；不增加第三种 `preselection_mode`。当前顺序为 `树首下行0x15并只等ACK -> +X 1.5m -> red +Y 0.3m / blue -Y 0.3m -> 20s -> 人工前激光 0x15 上阶前段 -> odom +X 0.6m -> 下行0x0B并从上阶完成起等待25s -> 0x13`，且不重复 managed 入口已完成的 `0x11/0x0D` 握手；后轮 `0x05` 不参与流程，后段也不再重复下行预装夹取 `0x15`。这些异步节点继续只消费现有 `/cmd_vel`、`/mechanism/send_command`、`/mechanism/command_feedback` 契约，不新增串口 ID、ROS 接口或运动命令权威。

2026-07-13 同步：通用 `OdomDriveX/OdomDriveY` 新增可选端口 `succeed_on_reach_or_overshoot`，默认关闭；开启后轴向到达容差或越过目标即停车成功，不再发布反向速度拉回。`second_preselection_tree.xml` 中夹取后 `+X 1.5m`、红蓝镜像 `Y 0.75m` 和放置后 `-X 1.5m` 三段固定 OdomDrive 已启用该端口，配合 second 专用 `0.60m/s` 速度口径避免目标附近来回纠偏。

2026-07-13 同步：second managed 入口斜坡从两段 `OdomDriveX` 改为 `SecondPreselectionRampForward` 单段动作。新参数为 `preselection_ramp_forward_x_m=5.00`、`preselection_ramp_forward_timeout_s=5.0` 与原速度上下限；原两段距离和原 timeout 参数已删除且不做兼容读取。合并 ramp 到达、超调或超时均停车返回成功，继续后续人工限位 2 gate，不再发布负 `linear.x` 做回拉纠偏。

2026-07-09 同步：第二预选赛放置 KFS 后新增可配置放置后上阶流程。`second_preselection_tree.xml` 在 ACK-only 下行 `SECOND_PRESELECTION_PLACE_KFS(0x13)` 后先按 `second_preselect_post_place_retreat_x_m=-1.5m` 后退，再由 `SecondPreselectionPostPlaceClimb` 并发下发前置推杆伸出和预装 KFS 夹取 `SECOND_PRESELECTION_PRELOAD_KFS_PICKUP(0x15)`，等待同 `seq` 的 `SECOND_PRESELECTION_PRELOAD_KFS_PICKUP_DONE(0x14)` 与可配置前推杆延时；随后停车等待人工前轮激光放行 `SECOND_PRESELECTION_MANUAL_FRONT_LASER_TRIGGERED(0x15)`，确认后执行剩余上阶流程，最终延时默认 25s 后再次下发 `0x13`。该人工 `0x15` 是新增上行反馈 ID，不复用 `FRONT_LASER_HEIGHT_JUMP(0x04)`。

2026-07-09 同步：新增 `second_preselection_post_place_climb_tree.xml` 作为放置后上阶动作的独立调试入口。该 XML 只加载 `SecondPreselectionPostPlaceClimb`，不执行完整 second tree 的 KFS 搜索、占位避让、任意 KFS 放置对齐、首次放置命令或放置后后退段，便于现场在已到位状态下单独验证 `0x08 + 0x15 -> 0x14 -> 人工 0x15 -> 剩余上阶 -> 延时 -> 0x13` 链路。
...
2026-07-12 同步：first repeat 的 `first_preselection_mc_repeat_forward_x_step_m` 改为 red/blue 必选带符号增量映射；`MCPreselectionRepeatControl` 仍维护内部黑板值 `mc_preselection_effective_forward_x_m`，后续重复轮按 `mc_nav_forward_x_m + selected_step * repeat_count` 计算。当前 red first 使用 `0.05m -> 0.25m -> 0.45m`，当前 blue first 使用 `1.05m -> 0.85m -> 0.65m`。旧 `mc_mf_preselection_tree.xml` 和 `mf_preselection_after_mc_tree.xml` 保留为兼容/调试入口。

2026-07-05 历史同步：当时的第二预选赛放置链使用 `+Y 0.7m -> +X 4.5m -> R1_KFS 对齐 -> +X 0.8m`；该路线已被 2026-07-10 的总 X 与任意 KFS 放置流程替代。

2026-07-12 同步：第二预选赛放置前准备改为中/下层视觉分层与固定前进口径。`SecondPreselectionKfsPlacePrepare` 不再要求上下双框成对，也不再等待真实深度；中层 KFS 只驱动占据稳定计数和两级横移避让，下层 KFS 只作为横向中心对齐目标。中层不存在时，若有下层则先对齐再固定前进，若无下层则直接固定前进；两级避让后若仍只有中层也直接固定前进。每个观察 checkpoint 新增 `second_preselect_place_observe_timeout_s=5.0s` 兜底，连续无有效新帧或占据/清空稳定帧迟迟不足时进入固定前进，终止原因写为 `PLACE_OBSERVE_TIMEOUT_FIXED_FORWARD`；下层 KFS 对齐继续由 `second_preselect_kfs_align_timeout_s` 控制。最终 `SecondPreselectionPlaceApproach` 统一沿 `+X` 前进 `second_preselect_place_fixed_forward_x_m=1.8m`，超时 `second_preselect_place_fixed_forward_timeout_s` 后停车并继续后续唯一一次首次 `0x13`；旧深度放置参数和旧上下框成对配对阈值已从代码、测试和红蓝配置彻底删除，不保留兼容读取。

2026-07-12 同步：第二预选赛 KFS 夹取消失验证增加超时兜底。`SecondPreselectionKfsPickup` 仍只用同 label 且 IoU 达阈值的原锁定框判断原目标是否可见；若在 `second_preselect_grab_verify_timeout_s` 内收到过新视觉帧但原目标未达到连续消失帧数，不再失败，而是停车告警、进入 `grab_settle` 并继续后续夹取后导航。若验证期内一直没有新视觉帧，仍按视觉异常失败停车。

2026-07-05 同步：第二预选赛 KFS 搜索夹取链在视觉横移对齐稳定后新增机械臂彻底放下握手。`SecondPreselectionKfsPickup` 会先发送 `SECOND_PRESELECTION_ARM_LOWER(0x14)`，等待同 `seq` 的 `SECOND_PRESELECTION_ARM_LOWER_DONE(0x12)`，再按 `second_preselect_pre_approach_lower_settle_s` 停车等待，之后才启动原有 KFS odom 前向趋近。原趋近后的 `SECOND_PRESELECTION_PICKUP_KFS(0x12)` 与 `SECOND_PRESELECTION_PICKUP_KFS_DONE(0x11)` 夹取完成握手、视觉消失验证和后续放置路线保持不变。
...
2026-07-04 同步：第二预选赛本地 OpenCV UI overlay 扩展为全流程视觉调试窗口。`second_preselect_dynamic_roi_ui_enable` 开启后，`SecondPreselectionKfsPickup` 会在 KFS 搜索、视觉对齐、odom 趋近、等待 `0x11` 夹取完成反馈和视觉消失验证期间持续显示彩色帧、识别框、锁定目标、目标线和阶段状态；2026-07-10 起放置前 UI 由 `SecondPreselectionKfsPlacePrepare` 展示占位判断、避让和任意 KFS 对齐。该 UI 只用于现场调试观察，不发布 topic、不改变行为树返回值、不新增 `/cmd_vel` 权威；窗口创建或渲染失败时仅告警并自动关闭 UI。
...
2026-07-03 同步：新增 `WaitPreselectionBranchGate` 并以 first/second managed 入口替代历史启动 XML。当时旧 first 组合树语义为武馆+梅林完整人工触发外部限位 1 分支 `0x06 -> 下行 0x10 -> 上行 0x0C -> MC -> MC 后置 MF`，以及人工触发外部限位 2 分支 `0x10 -> 下行 0x10 -> 上行 0x0C -> mf_preselection_tree.xml`；当前 first 默认入口已改为 MC-only 重复树，second 默认组合树的两条 `0x10` 路径均在 `0x11/0x0D` 握手后切到 `second_preselection_climb_place_tree.xml`，其中 `0x06` 入口分支会先完成合并斜坡并停车等待第二次 `0x10`。`mc_tree.xml` 末尾删除 `WaitForRegistrationConfirm` 和 5s 视觉配准等待，旋转后改为同一 branch gate。新增 `preselection_ramp_forward_tree.xml` 与 `second_preselection_combo_tree.xml`，历史 `mf_preselection_start_tree.xml` 已删除。
...
2026-07-02 同步：MCU 上行 `MANUAL_LIMIT_SWITCH_2_TRIGGERED(0x10)` 定义为人工触发外部限位 2 事件；当前 managed first/second 入口下，0x10 由 `WaitPreselectionBranchGate` 消费，并按当前 XML profile 完成 0x10/0x0C 或 0x11/0x0D 握手后再继续或切树。
