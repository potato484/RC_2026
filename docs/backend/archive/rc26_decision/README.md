...
- `second_preselection_tree.xml`：第二个预选赛独立树。若不是从 branch gate 直达，会先执行 `SECOND_PRESELECTION_START(0x11)` 并等待同 `seq` 的 `SECOND_PRESELECTION_START_DONE(0x0D)`；随后进入 `SecondPreselectionKfsPickup`，启动视觉、odom、`/cmd_vel` 和 `/mechanism/send_command`/`/mechanism/command_feedback`。该节点按红方基准沿车体系 `+X` 低速搜索前方 KFS：最近有效 KFS 为 R2 目标（默认 `T_` 前缀、深度有效）时停车进入视觉横移对齐；识别到 R1 KFS 时不停车等待，继续前进搜索 R2。视觉对齐复用 `rc26_vision::tip_alignment` 的目标线偏置、目标锁定、像素容差、稳定帧、heading hold/yaw gate 和丢帧停车口径，并可在已有真实锁定深度约束下用 KFS 尺寸估距兜底。对齐完成后按 `max(0, locked_depth - second_preselect_kfs_grab_distance_m) * second_preselect_kfs_approach_x_sign` 规划车体系 X 轴 odom 前向趋近，保持进入趋近时 yaw；趋近完成后发送 `second_preselect_pickup_command_id=0x12`，只要求 service ACK，再用原目标 label 与 bbox IoU 做视觉消失验证，连续新帧消失达到阈值才算夹取成功。夹取确认后执行红方基准 `OdomDriveY(second_preselect_nav_y1_m=+0.7m) -> OdomDriveX(second_preselect_nav_x2_m=+2.5m)`，blue 启动时只镜像 Y 段。随后 `SecondPreselectionObserve` 做动态九宫格观察；未观察到前方 KFS 时也继续复用中层 `r=0` 三格选空位策略，按中列、场地可移动空间侧、另一侧的顺序选择空格。动态 ROI 在观察开始时捕获 odom 参考点，后续用 `D=D0-Δx`、`S=S0+sign*Δy` 和 Color 640x480 内参投影 9 个格子；列宽按红方图纸维护为左列 `0.54m`、中列 `0.58m`、右列 `0.50m`，行距 `0.54m`。可通过 `second_preselect_dynamic_roi_ui_enable` 打开本地 OpenCV 调试窗口，叠加显示 3x3 ROI、检测框、占据 mask 和中层选位结果；窗口默认关闭，创建或渲染失败只告警并自动关闭 UI，不改变观察/放置结果。选位结果写入已镜像的 `second_preselection_selected_lateral_m`，然后执行 `OdomDriveY(selected_lateral) -> OdomDriveX(place_forward) -> 0x13 -> OdomDriveX(retreat)`。搜索、对齐、趋近、夹取验证、观察、odom、命令任一阶段失败或 halt 都会停车并写 `DecisionFailure`。
...
2026-07-04 同步：第二预选赛 `SecondPreselectionTree` 内部流程改为搜索夹取链。保留 `second_preselection_combo_tree.xml` 的 `0x11/0x0D` gate、斜坡和 90° 转向入口；树内新增 `SecondPreselectionKfsPickup`，在 `0x11` start-once 后沿 `+X` 搜索最近 R2 KFS，按 MF 口径做视觉横移对齐、odom 前向趋近、发送 `0x12` 夹取触发并用原目标视觉消失验证夹取成功。夹取后再执行红方基准 `+Y 0.7m -> +X 2.5m` 到放置观察位；旧固定第一段、观察前高抬节点、对应兼容参数及放置前 KFS 必见 gate 已删除。`SecondPreselectionObserve` 使用动态九宫格 ROI 选择中层空位，未观察到 KFS 时继续按空位逻辑放置。

2026-07-04 同步：第二预选赛动态 ROI 观察新增本地 OpenCV UI overlay。`second_preselect_dynamic_roi_ui_enable` 默认关闭；开启后 `SecondPreselectionObserve` 会在彩色帧上绘制 3x3 动态 ROI、检测框、占据 mask、odom delta 和当前中层选位结果，窗口名由 `second_preselect_dynamic_roi_ui_window_name` 控制。该 UI 只用于现场调试观察，不发布 topic、不改变行为树返回值、不新增 `/cmd_vel` 权威；窗口创建或渲染失败时仅告警并自动关闭 UI。
