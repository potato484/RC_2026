...
2026-07-03 同步：新增 `WaitPreselectionBranchGate` 并以 first/second managed 入口替代历史启动 XML。当时旧 first 组合树语义为武馆+梅林完整人工触发外部限位 1 分支 `0x06 -> 下行 0x10 -> 上行 0x0C -> MC -> MC 后置 MF`，以及人工触发外部限位 2 分支 `0x10 -> 下行 0x10 -> 上行 0x0C -> mf_preselection_tree.xml`；当前 first 默认入口已改为 2026-07-08 的 MC-only 重复树。second 语义仍为对抗区完整人工触发外部限位 1 分支 `0x06 -> 下行 0x11 -> 上行 0x0D -> 斜坡 -> 转 90° -> second_preselection_tree.xml`，以及对抗区不完整人工触发外部限位 2 分支 `0x10 -> 下行 0x11 -> 上行 0x0D -> second_preselection_tree.xml`。`mc_tree.xml` 末尾删除 `WaitForRegistrationConfirm` 和 5s 视觉配准等待，旋转后改为同一 branch gate。新增 `preselection_ramp_forward_tree.xml` 与 `second_preselection_combo_tree.xml`，历史 `mf_preselection_start_tree.xml` 已删除。
