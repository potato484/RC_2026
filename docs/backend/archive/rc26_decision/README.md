...
- `second_preselection_kfs_compat_climb_place_tree.xml`：second managed 兼容目标树。树首使用 `SecondPreselectionKfsPickup continue_on_failure=true` 复用旧完整 second 树的 KFS 前进搜索、视觉横移对齐、odom 趋近、机构夹取和消失验证，不执行 `SecondPreselectionClimbPlacePreloadPickup`，因此不会在树首直接发送预装 `0x15`。搜索、视觉、普通 odom 或夹取链失败时动作发布零速、记录 WARN 并返回 `SUCCESS` 继续；缺失 blackboard/node/参数等结构性上下文错误仍失败。无论搜索成功还是容错，后续都从当前停车位置重新捕获相对起点并完整执行默认目标树的 X/Y、20s、人工前激光和后轮 odom 路线，不扣除搜索过程位移。最后进入 `SecondPreselectionRearRetractCompatPickupPlace`：立即可靠处理 `0x0B`，从节点进入时累计 25s；时间到后单次发起 `0x15` 且 service 请求 `wait_ack=false`，不等待 service response、MCU ACK 或 `0x14`，无论 service 是否可用或请求是否成功都立即开始 `second_preselect_climb_place_compat_pickup_to_final_delay_s=10s` 固定延时，之后可靠发送最终 `0x13`。全阶段持续发布零 `/cmd_vel`，`0x0B/0x13` 仍使用统一机构容错语义。
