...
- `second_preselect_climb_place_*`：上阶夹取放置目标树的专属参数，既服务显式独立运行，也服务 second managed 切树后的执行。路线默认 `forward_x=1.5m`、红方基准 `lateral_y=+0.3m`、`pre_climb_delay=20000ms`，blue 在参数加载时自动镜像为 `-0.3m`；前半上阶命令/反馈为 `0x08 -> 零速等待上行0x15 -> 0x09+0x0A`，推杆稳定延时、heading 预对齐和人工前激光等待超时复用 `stair_*`，但这棵目标树在 `0x08` 伸出后不再使用 `stair_climb_front_drive_*` 速度曲线。后轮段使用 `rear_forward_x=0.6m`、`rear_max/min_speed=0.40/0.10m/s` 的 `OdomDriveX`，当前红蓝运行配置把 `rear_timeout` 设为 `5.0s`，并复用通用 odom 增益、容差和新鲜度；该段启用 `succeed_on_timeout`，有效目标捕获后的动作超时会停车成功并继续收尾，不把其它初始化或目标捕获错误吞成成功。收尾参数为 `rear_retract=0x0B`、`preload_pickup=0x15/done=0x14`、`final_delay=25s`、`final=0x13`；机构 service/topic、通用 ACK 超时、done 超时和日志周期继续复用其它 `second_preselect_*` 公共参数。
