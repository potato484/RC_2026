...
- `mc_tree.xml`：`OdomDriveX(mc_nav_forward_x_m) -> RelativeYawTarget(mc_nav_right_turn_delta_rad=team 派生侧向 yaw) -> OdomTurnToYaw -> OdomDriveX(mc_nav_reverse_x_m) -> VisualServoGrab -> Delay -> RotateInPlace -> Delay(0.5s) -> OdomDriveX(-0.4m) -> OdomDriveY(-0.4m) -> WaitPreselectionBranchGate`。MC 末尾正式流程不再执行 `WaitForRegistrationConfirm` 或 5s 视觉配准等待；旋转后先复用 odom 单轴分段动作做车体系 -X/-Y 退让，再等待 0x06/0x10 branch gate。MC 末尾 gate 的 0x06 分支会在收到限位后按 `preselection_after_mc_continue_delay_msec` 延时，再下发 `COMPETITION_START(0x10)`，避免下位机在人工触发瞬间立即进入发车状态。转向后的绝对 odom yaw 会传给 `VisualServoGrab target_yaw_rad`，作为视觉阶段 heading hold 目标；`team=blue` 时侧向 yaw 和后续原地旋转方向相对红方基准取反。
...
- `second_preselection_tree.xml`：第二个预选赛独立树。若不是从 branch gate 直达，会先执行 `SECOND_PRESELECTION_START(0x11)` 并等待同 `seq` 的 `SECOND_PRESELECTION_START_DONE(0x0D)`；随后进入 `SecondPreselectionKfsPickup`，启动视觉、odom、`/cmd_vel` 和 `/mechanism/send_command`/`/mechanism/command_feedback`。该节点按红方基准沿车体系 `+X` 低速搜索前方 KFS：任意识别标签属于 KFS 的最近有效目标都会停车进入视觉横移对齐和夹取，不再区分 R2/R1 或等待特定 KFS 类型。视觉对齐复用 `rc26_vision::tip_alignment` 的目标线偏置、目标锁定、像素容差、稳定帧、heading hold/yaw gate 和丢帧停车口径，并可在已有真实锁定深度约束下用 KFS 尺寸估距兜底。对齐完成后按 `max(0, locked_depth - second_preselect_kfs_grab_distance_m) * second_preselect_kfs_approach_x_sign` 规划车体系 X 轴 odom 前向趋近，保持进入趋近时 yaw；趋近完成后发送 `second_preselect_pickup_command_id=0x12`，service ACK 后必须等待同 `seq` 的 `second_preselect_pickup_done_feedback_id=0x11`，再用原目标 label 与 bbox IoU 做视觉消失验证，连续新帧消失达到阈值才算夹取成功。夹取确认后执行红方基准 `OdomDriveY(second_preselect_nav_y1_m=+0.7m) -> OdomDriveX(second_preselect_nav_x2_m=+2.5m)`，blue 启动时只镜像 Y 段。随后 `SecondPreselectionObserve` 做动态九宫格观察；未观察到前方 KFS 时也继续复用中层 `r=0` 三格选空位策略，按中列、场地可移动空间侧、另一侧的顺序选择空格。动态 ROI 在观察开始时捕获 odom 参考点，后续用 `D=D0-Δx`、`S=S0+sign*Δy` 和 Color 640x480 内参投影 9 个格子；列宽按红方图纸维护为左列 `0.54m`、中列 `0.58m`、右列 `0.50m`，行距 `0.54m`。可通过 `second_preselect_dynamic_roi_ui_enable` 打开本地 OpenCV 调试窗口；该窗口贯穿 KFS 搜索、视觉对齐、odom 趋近、`0x12/0x11` 夹取握手、视觉消失验证和九宫格观察，持续叠加 KFS 检测框、当前阶段、锁定目标、目标线、3x3 ROI、占据 mask 和中层选位结果。窗口创建或渲染失败只告警并自动关闭 UI，不改变搜索、夹取、观察或放置结果。选位结果写入已镜像的 `second_preselection_selected_lateral_m`，然后执行 `OdomDriveY(selected_lateral) -> OdomDriveX(place_forward) -> 0x13 -> OdomDriveX(retreat)`。搜索、对齐、趋近、夹取完成反馈、夹取验证、观察、odom、命令任一阶段失败或 halt 都会停车并写 `DecisionFailure`。
...
`WaitPreselectionBranchGate` 是 first/second managed 入口和 MC 末尾的分支 gate。节点只消费 `/mechanism/command_feedback` 上的第一个限位 `FRONT_LIMIT_SWITCH_TRIGGERED(0x06)` 与第二个限位 `MF_PRESELECTION_TRIGGER(0x10)`，不采集 MC 相机基准帧，也不依赖 MC 末尾视觉配准。XML 通过 `continue_start_profile` 和 `switch_start_profile` 决定两条分支使用的启动握手：first 树两条分支都使用 `mc` profile，也就是下发 `COMPETITION_START(0x10)` 并等待同 `seq` 的 `COMPETITION_START_DONE(0x0C)`；second 树两条分支都使用 `second` profile，也就是下发 `SECOND_PRESELECTION_START(0x11)` 并等待同 `seq` 的 `SECOND_PRESELECTION_START_DONE(0x0D)`。gate 可通过 `continue_pre_command_delay_msec` / `switch_pre_command_delay_msec` 在收到对应限位后、下发启动命令前等待；当前 MC 末尾 0x06 分支把 `continue_pre_command_delay_msec` 绑定到 YAML 参数 `preselection_after_mc_continue_delay_msec`。0x06 分支握手成功后返回 `SUCCESS` 继续当前完整流程；0x10 分支握手成功后写入黑板切树请求，`decision_node` 在本轮 tick 返回后 halt 当前树并加载 `switch_tree_file` 指向的目标树。managed first/second 入口下，旧 `mf_preselection_external_trigger_*` 全局监听保持禁用，0x10 只能在 branch gate 内按当前 profile 握手后切树。
...
`MfPreselectionFlow` 的入口、行前方、周身和 `TransitionObserve` 检测窗口在未发现最近 R2 KFS 时必须先等满对应 `mf_preselect_*_detect_timeout_s`，再按 `mf_preselect_detect_lost_stable_frames` 确认未命中并切到下一阶段；连续丢失帧只用于抗抖，不再提前截短检测窗口。发现最近 R2 KFS 或假 KFS 仍可在窗口内立即触发对应夹取/避障分支。R2 KFS 夹取命令完成后若视觉验证超时且原目标仍可见，不再把该目标加入 ignored 列表：入口侧会按本次前向趋近规划距离反向后退到可重新识别位置，再重新进入 KFS 视觉对齐和夹取；梅林内部路径前方或台阶前观察夹取失败时，会先复用当前格 `GridCenterAlign` 归中，再回到原前方观察阶段继续夹取。除 2 号入口必须无限重试的夹取确认失败外，没有新视觉帧、目标已消失但未达到稳定消失帧、机构命令失败、odom 运动失败或归中失败仍沿原失败/硬失败语义处理。R1 阻挡标签新增 `R1_KFS` 兼容，但会用 `mf_preselect_r1_kfs_min_score` 做低置信度过滤，默认 0.50，低于阈值的 `R1_KFS` 不参与最近 R1/R2 KFS 判断。R2 KFS 候选若被过滤或最近 KFS 不是 R2，会把最近一次拒绝原因代码写入 `mf_preselect_r2_lock_reject_reason/detail/sequence` 黑板键，并在同一检测窗口内按原因去重打印中文 INFO 日志；检测 miss 日志会附带中文摘要，便于区分夹取数已满、视觉帧无效、标签不匹配、已忽略目标、深度采样失败、最近 KFS 不是 R2 或目标选择失败。最近 KFS 门控日志会打印类型、标签、深度、offset、depth source 和被屏蔽候选数。
...
- `preselection_entry_continue_delay_msec`、`preselection_after_mc_continue_delay_msec`：first managed 的 0x06 分支延时参数。`preselection_entry_continue_delay_msec` 仍用于入口 gate 握手完成后进入 MC 前的 `Delay`；`preselection_after_mc_continue_delay_msec` 用于 MC 末尾 gate 收到 0x06 后、下发 `COMPETITION_START(0x10)` 前的等待。参数侧按有符号整数读取并归零裁剪，写入 blackboard 时按 BehaviorTree.CPP `unsigned int` 端口类型保存。
...
- `second_preselect_*`：第二个预选赛独立树参数。命令参数默认 `0x11/0x0D` 开始握手、`0x12/0x11` KFS 夹取触发与动作完成握手、`0x13` ACK-only 放置。`0x12` service ACK 只表示 MCU 收到命令，`SecondPreselectionKfsPickup` 必须等同 `seq` 的 `SECOND_PRESELECTION_PICKUP_KFS_DONE(0x11)` 后，才进入原目标视觉消失验证。搜索/夹取参数包括 `second_preselect_cmd_vel_topic`、`second_preselect_search_*`、R2/R1 标签集合、深度窗口、`second_preselect_kfs_align_*`、`second_preselect_kfs_approach_*`、`second_preselect_kfs_grab_distance_m`、bbox 深度采样、尺寸估距、`second_preselect_grab_verify_*` 和 `second_preselect_grab_settle_s`；这些参数按 MF 当前 KFS 夹取口径维护，但作用范围只在第二预选赛。导航参数按红方基准维护，新流程夹取成功后默认 `+Y 0.7m -> +X 2.5m` 到放置观察位，blue 启动时自动翻转 Y 段，放置段继续由 `second_preselect_place_forward_x_m -> 0x13 -> second_preselect_retreat_x_m` 控制。第二预选赛放置前只使用动态 3x3 ROI：`SecondPreselectionObserve` 必须读取观察期 odom delta，并按 Color 640x480 内参、红方基准初始距离/横向偏移、相机高度、中层高度、安全宽高和九宫格实地尺寸投影 9 个格子；列宽按红方图纸维护为左列 `0.54m`、中列 `0.58m`、右列 `0.50m`，行距 `0.54m`，不保留固定 ROI 或等距 pitch 兜底。命中规则为检测框中心点落入某格动态 ROI 且 `class_name` 非空；标签前缀/精确名过滤列表为空时，所有非空 `class_name` 都有效。动态 ROI 内没有任何有效 KFS 检测时继续按中层空位逻辑放置。`second_preselect_dynamic_roi_ui_enable` 控制第二预选赛本地 OpenCV 视觉调试窗口；开启后在 KFS 搜索/对齐/夹取验证阶段绘制识别框、锁定目标、目标线和阶段状态，在九宫格观察阶段继续绘制动态 ROI、检测框、mask、odom delta 和选位结果，`second_preselect_dynamic_roi_ui_window_name` 控制窗口名。红/蓝运行配置默认省略 `second_preselect_grid_label_prefixes` 与 `second_preselect_grid_label_exact_names` 来表达空过滤列表，不写 `[]`，避免 launch 参数数组元素类型不可推断。blue 启动时会把横向偏移、`base_y_to_grid_x_sign` 和横向偏置按 `team_mirror_sign` 翻转。观察结果写入 `second_preselection_grid_occupied_mask`、`second_preselection_selected_middle_col`、`second_preselection_selected_lateral_m` 和 `second_preselection_middle_empty`，供树内对齐和失败分支使用。
...
2026-07-04 同步：第二预选赛 `SecondPreselectionTree` 内部流程改为搜索夹取链。保留 `second_preselection_combo_tree.xml` 的 `0x11/0x0D` gate、斜坡和 90° 转向入口；树内新增 `SecondPreselectionKfsPickup`，在 `0x11` start-once 后沿 `+X` 搜索最近有效 KFS，任意识别标签属于 KFS 的目标都会按 MF 口径做视觉横移对齐、odom 前向趋近、发送 `0x12` 夹取触发，等待同 `seq` 的 MCU 上行 `0x11` 夹取完成反馈后，再用原目标视觉消失验证夹取成功，不再因 R1/R2 类型差异继续搜索。夹取后再执行红方基准 `+Y 0.7m -> +X 2.5m` 到放置观察位；旧固定第一段、观察前高抬节点、对应兼容参数及放置前 KFS 必见 gate 已删除。`SecondPreselectionObserve` 使用动态九宫格 ROI 选择中层空位，未观察到 KFS 时继续按空位逻辑放置。

2026-07-04 同步：第二预选赛本地 OpenCV UI overlay 从动态 ROI 观察扩展为全流程视觉调试窗口。`second_preselect_dynamic_roi_ui_enable` 开启后，`SecondPreselectionKfsPickup` 会在 KFS 搜索、视觉对齐、odom 趋近、等待 `0x11` 夹取完成反馈和视觉消失验证期间持续显示彩色帧、识别框、锁定目标、目标线和阶段状态；`SecondPreselectionObserve` 继续在同一窗口绘制 3x3 动态 ROI、检测框、占据 mask、odom delta 和当前中层选位结果。该 UI 只用于现场调试观察，不发布 topic、不改变行为树返回值、不新增 `/cmd_vel` 权威；窗口创建或渲染失败时仅告警并自动关闭 UI。
...
2026-07-04 同步：`WaitPreselectionBranchGate` 新增 `continue_pre_command_delay_msec` / `switch_pre_command_delay_msec` 端口，用于在收到 0x06/0x10 分支反馈后、下发对应启动命令前等待。`mc_tree.xml` 末尾 0x06 分支将 `continue_pre_command_delay_msec` 绑定到 `preselection_after_mc_continue_delay_msec`，所以红方当前为手动触发限位后先等 5s，再下发 `COMPETITION_START(0x10)`；该延时不再放在 `0x10/0x0C` 握手完成之后。
