...
当前 `ExecuteMechanism` 通用入口已经不再覆盖前/后推杆 4 条 sidecar 命令；遥控链改为直接调用 `/mechanism/transport/send_command`，并由 `rc26_merge_odom` 在目标串口上按下沿单次下发 `FRONT_PUSHROD_EXTEND/RETRACT(0x0E/0x0F)` 与 `REAR_PUSHROD_EXTEND/RETRACT(0x10/0x11)`。这些命令都已经走可靠 ACK 路径；若 MCU 不回通用 `ACK(0x00)`，会像其它可靠命令一样重传并打印超时日志。MCU 若再上送 `0x13~0x16` 业务 ACK，也会继续经由 `/mechanism/transport/feedback` 透传给上层。
...
- `ExecuteMechanism` 继续作为机构动作的统一执行入口，但前/后推杆 4 条 sidecar 命令已从这个入口中移除。
...
- 遥控模式下的前/后推杆控制已改成 teleop 直接走 transport service，`rc26_mechanism` 不再消费这些 sidecar 命令。
- 最小 MCU 链现在也会由 `pose_sender_node` 挂出 `/mechanism/transport/*`，shared_serial HAL 不再强依赖 `merge_odom_node` 本体。
- `test_shared_serial_transport` 继续覆盖共享串口发送、反馈收敛、服务异常和超时场景，同时验证前/后推杆 sidecar 命令会被 `/mechanism/execute` 拒绝。
