...
- `target_serial_port`，当前默认 `/dev/ttyUSB1`
  ...
  - `MechanismTransportBridge` 继续作为共享桥接接口，供 `rc26_mechanism` 与 teleop 前置履带 / 推杆按钮节点复用同一串口
  - service：`/mechanism/transport/send_command`
  - topic：`/mechanism/transport/feedback`
  - 现在即使只启动 `pose_sender_node`，也会继续挂出这组共享 transport 接口，保证 `start_r2_mcu_teleop.sh` 的最小串口栈仍能联调机构类 ACK 指令
...
`launch/merge_odom.launch.py` 当前除了 `use_can_odom` / `start_ekf` 外，还支持 `use_imu_for_ekf` 与 `start_imu`：
...
- `pose_sender_node` 现在也会挂出同一组 `/mechanism/transport/*`，因此 `minimal-mcu` 栈不再只有速度下发，也能承接基于 ACK 的共享机构 transport。
- 新增 `PUSHROD_EXTEND(0x10)` / `PUSHROD_RETRACT(0x11)` 两条 ACK 协议；它们和其它普通机构命令一样走可靠 send + ACK，应答成功即视为 MCU 已收指令，不要求额外 `DONE` 反馈。
