...
- 仓库根目录新增 `start_r2_mcu_teleop.sh`，用于只拉起 `pose_sender_node + joy_node + rc26_telecontrol` 的最小遥控链；当反馈串口缺失、和目标串口重复，或调用者显式传 `__disabled__` 时，`PoseSenderNode` 现在会跳过反馈串口初始化，只保留目标串口下发。
