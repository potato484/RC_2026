...
- **KFS action test 夹取触发受深度窗口约束**：独立 KFS 动作链现在用 `min(kfs_action_grab_distance_m, vision_depth_max_m)` 作为有效夹取距离上限，并要求目标深度不小于 `vision_depth_min_m`；启动日志会同时打印 `grab_dist`、`effective_grab` 和 `depth_range`。因此现场把 `vision_depth_max_m` 调到 `0.55m` 后，即使 `kfs_action_grab_distance_m` 仍是 `0.70m`，`0.65m` 目标也不会触发 `GRAB_KFS_UP/DOWN`。
