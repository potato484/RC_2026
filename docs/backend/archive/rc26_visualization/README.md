...
`rc26_visualization` 已从单纯诊断聚合包扩展成 R2 的本地 Web 可视化平台承载包。

当前真实职责分成两块：

- 诊断聚合：生成 `r2/diag/operator_status`、`r2/diag/events` 等值守语义
- Web adapter：提供 `viewer/`、`visualization_server.py` 和 live bridge，把浏览器消费模型统一收口到本包
...
- viewer 虽然可以执行 `surface-route execute`，但那是受控 action 下发，不是浏览器直控底盘
