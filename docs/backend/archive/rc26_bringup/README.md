...
- `bringup.launch.py`
  ...
  - `config/nav2_params.yaml` 中 `planner_server.GridBased.tolerance` 当前收紧为 `0.05m`，避免 NavFn 在目标点附近半米内选替代终点后仍被上层当作真实到点；`controller_server.progress_checker` 当前按低速实车调为 `20s` 内至少前进 `0.05m`，减少慢速起步或局部恢复期间的误判卡死。普通导航继续使用 `FollowPath`，红方 MC 去程专用 Nav2 BT 使用 `MCPositiveXYRed` 与 `mc_xy_goal_checker`：第一阶段只采样车体系 `linear.x/y` 正向，`angular.z` 保持 0，yaw 到点交给后续 MC 决策层 `RotateInPlace`。梅林预选 2 号入口导航专用 Nav2 BT 使用 `MFPreselectEntryXPositiveYNegative`，只采样车体系 `linear.x >= 0`、`linear.y <= 0`、`|angular.z| <= 0.10rad/s`，并且 recovery 只做 costmap clear，避免恢复动作绕过方向约束。Nav2 obstacle layer / inflation layer 都是 local/global costmap 的共享插件，不是 controller 级开关；当前共享 costmap 加载 obstacle layer 与 inflation layer，因此 `MCPositiveXYRed` 与 `MFPreselectEntryXPositiveYNegative` 导航会消费动态障碍，也会对静态或动态障碍生成膨胀缓冲；当前膨胀半径为 `0.60m`，代价衰减系数为 `3.0`
...
- Nav2 obstacle layer 与 inflation layer 当前在 `config/nav2_params.yaml` 的 local/global costmap `plugins` 中加载；膨胀半径为 `0.60m`，代价衰减系数为 `3.0`，用于给静态地图障碍和 `/sensor_scan` 动态障碍提供规划缓冲。`/sensor_scan` (`PointCloud2`) 作为障碍输入进入 costmap，`registered_scan` 继续只供定位链使用
...
- `test_navigation.launch.py` 的验收目标改为 `/navigate_to_pose`、`sensor_scan` 链路存在、Nav2 lifecycle nodes、costmap/plan topics 与 `/cmd_vel` 输出；当前默认将 `sensor_scan` 作为 obstacle layer 输入，并在 local/global costmap 后接 inflation layer
