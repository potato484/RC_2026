...
`bringup.launch.py` 默认按 headless 口径装配，但声明默认关闭的 `use_rviz:=false` 与 `rviz_config_file:=...`，现场需要时可临时随正式链路打开 RViz2。`odometry.launch.py` 继续保持 headless。面向现场联调的 `test_mapping.launch.py` 与 `test_navigation.launch.py` 是调试入口，默认复用对应链路并额外打开 RViz2，可用 `use_rviz:=false` 关闭图形界面，也可用 `rviz_config_file:=...` 替换 RViz 配置。

- `bringup.launch.py`
  - 装配 Point-LIO、里程计接口、定位、Nav2 基础导航栈和决策
  - `run_mode:=navigation` 时启动 `rc26_localization`、`map_server` 与 Nav2 `navigation_launch.py`
  - 导航模式默认按犀牛派 X1 低算力口径错峰启动：odometry/Point-LIO 先起，随后延时拉起 localization、map_server、Nav2、decision 和 RealSense，避免 PCD 加载、GICP、Nav2 lifecycle、DWB 初始化和相机驱动在同一秒叠加。延时可通过 `startup_delay_localization_sec`、`startup_delay_map_sec`、`startup_delay_nav2_sec`、`startup_delay_decision_sec`、`startup_delay_realsense_sec` 覆盖，设为 `0.0` 即恢复不延时
  ...
  - 当前默认运行时在 local/global costmap 加载 obstacle layer 与 inflation layer；`/sensor_scan` (`PointCloud2`) 进入 Nav2 costmap，障碍高度过滤范围为 `0.05m <= z <= 2.0m`，且不再依赖 `/scan`
  - `config/nav2_params.yaml` 中 local costmap 当前设置 `transform_tolerance=0.5s`，用于吸收启动阶段和实机链路中的轻微 TF 滞后；controller 插件自身 TF 容忍仍按各 controller 参数维护
  - `config/nav2_params.yaml` 默认采用低算力档：降低 BT tick、controller、planner、velocity smoother、waypoint follower 和 recovery 频率，减少 DWB 速度采样、路径平滑迭代和 costmap 完整发布，优先保证 X1 上完整链路不被启动峰值和持续 DWB 评分压垮。需要更激进控制响应时应先确认 CPU 余量，再小步提高对应频率和采样数量
  ...
  - `r2_runtime.decision.ros__parameters` 中的 `mc_nav_*` 参数是当前红方武馆区行为树去程目标真源；`mc_tree.xml` 从黑板读取这些值，不再在 XML 中写死取端头导航点。`mc_nav_behavior_tree_file` 当前指向 `config/nav2_bt_mc_red_positive_xy.xml`，让去程 `NavToPose` 先以 xy-only 方式到达 `mc_nav_x/y`，到点后再由 `RotateInPlace target_yaw_rad={mc_nav_yaw}` 发布角速度转向。MC 树完成视觉夹取和原地旋转后直接进入 `WaitForever`，当前不再维护固定返程目标，也不再声明 `mc_return_nav_*` 参数
  ...
  - `use_realsense:=true` 时，完整 bringup 通过 `rc26_bringup/launch/realsense_d455.launch.py` 启动 D455，默认固定选择当前 R2 D455 序列号 `239222303644`，避免多 RealSense 或 USB 枚举顺序变化时选错设备；现场更换相机时仍可用 `realsense_serial_no:=<new_serial>` 临时覆盖。完整 bringup 还会显式把 `camera_namespace` 置空，让相机发布 `/camera/color/image_raw`、`/camera/aligned_depth_to_color/image_raw` 和 `/camera/color/camera_info` 这组单层 topic；该口径与 `rc26_vision/test_kfs_vision.launch.py` 保持一致，并匹配 `VisionInferenceManager` 的默认订阅，避免默认 `camera_namespace=camera` 与 `camera_name=camera` 组合生成 `/camera/camera/...` 后正式决策拿不到图像/深度。
  ...
  - `config/nav2_params.yaml` 中 `planner_server.GridBased.tolerance` 当前收紧为 `0.05m`，避免 NavFn 在目标点附近半米内选替代终点后仍被上层当作真实到点；`controller_server.progress_checker` 当前按低速实车调为 `20s` 内至少前进 `0.05m`，减少慢速起步或局部恢复期间的误判卡死。普通导航继续使用 `FollowPath`，红方 MC 去程专用 Nav2 BT 使用 `MCPositiveXYRed`，只采样车体系 `linear.x/y` 正向和角速度。`MCNegativeXYRed` 仍作为可手工选择的负向约束 controller 配置保留，但当前默认 MC 树不再引用它。梅林预选 2 号入口导航专用 Nav2 BT 使用 `MFPreselectEntryXPositiveYNegative`，当前保留低速上限但不再硬限制车体系 x/y 方向，并且在 `FollowPath` 上显式使用 `general_goal_checker`，避免 Humble controller_server 收到空 goal checker 名称后拒绝 action；recovery 只做 costmap clear，不额外启用恢复运动动作。Nav2 obstacle layer / inflation layer 都是 local/global costmap 的共享插件，不是 controller 级开关；当前共享 costmap 加载 obstacle layer 与 inflation layer，`/sensor_scan` 障碍层过滤 `0.05m` 以下低矮点，因此 `MCPositiveXYRed` 与 `MFPreselectEntryXPositiveYNegative` 导航会消费过滤后的动态障碍，并在静态与动态障碍周围生成膨胀代价
  ...
  - 导航模式默认通过 `point_lio_full_map_publish_en=false` 覆盖关闭 Point-LIO 完整累计地图可视化点云，保留 `/cloud_registered` 到 `rc26_odom_interface` 的主链输入；建图模式默认仍打开该可视化发布，现场需要在导航中观察完整累计图时可显式传 `point_lio_full_map_publish_en:=true`
- `odometry.launch.py`
...
- `test_navigation.launch.py`
  ...
  - 默认 `use_rviz=true`，加载 `rviz/navigation_default.rviz` 观察静态地图、Nav2 合成 costmap、TF、全局规划路径与局部小车姿态路径
...
- `config/nav2_params.yaml` 当前加载 local/global obstacle layer 与 inflation layer；`/sensor_scan` (`PointCloud2`) 作为障碍输入进入 costmap，并在 obstacle layer 侧过滤 `0.05m` 以下低矮点，不再维护 `/scan` LaserScan 兼容链；该文件的实际 YAML 配置项已经逐行补充中文行尾说明，便于现场直接查参数语义
- `scripts/capture_nav_points.py` 是现场只读采点辅助入口，依赖当前主链 TF 结果生成 Nav2 目标点文本，不接管 `map -> odom`、`/navigate_to_pose` 或 `/cmd_vel` 权威
- `bringup.launch.py` 的 `use_rviz:=true`、`test_mapping.launch.py` 与 `test_navigation.launch.py` 的 RViz2 都只作为下游观察和 Nav2 目标点发布工具；`/point_lio/map_cloud` 是现场建图观察输出，不作为定位或导航权威
- 默认完整 bringup 针对 X1 板卡按低算力部署：启动错峰由 launch 负责，运行时降载由 `nav2_params.yaml`、`localization.yaml`、`odom_interface.yaml` 和 Point-LIO 发布开关负责；这些都不改变 `map -> odom`、`odom -> base_footprint`、`/cmd_vel` 或机构串口的权威归属。
...
- Nav2 obstacle layer 当前在 `config/nav2_params.yaml` 的 local/global costmap `plugins` 中加载，并在其后接入 inflation layer，为静态地图与 `/sensor_scan` 动态障碍生成膨胀缓冲。`/sensor_scan` (`PointCloud2`) 作为障碍输入进入 costmap，当前 `min_obstacle_height=0.05m`、`max_obstacle_height=2.0m`，用于先过滤地面噪声和车身低矮回波；`registered_scan` 继续只供定位链使用
- 本轮自动导航链将 `base_footprint` 固定为地面投影 2D 基座，`base_link` 固定为底盘最下层刚性主板中心，二者高度差当前由 `config/odom_interface.yaml` 维护，当前为 `0.065m`
...
- `bringup.launch.py` 新增默认关闭的 `use_rviz` 与 `rviz_config_file`；`test_navigation.launch.py` 仍默认随导航联调入口启动 RViz2
- `navigation_default.rviz` 已收敛为轻量导航观察预设：只默认显示 `/map`、`/global_costmap/costmap`、`/local_costmap/costmap`、TF、`/plan` 全局路径与 `/local_plan` 小车姿态路径，并保留 Nav2 GoalTool；不再默认加载 Nav2 面板、RobotModel、里程计、定位位姿、点云或旧 terrain/keepout 输出
...
- 2026-06-27 同步：删除旧 `kfs_stair_test.launch.py`、decision 侧 `KfsStairPickup` 节点和 `kfs_stair_*_test_tree.xml` 独立等待树；本车 `T_*` KFS 独立实机夹取测试统一走 `rc26_vision/test_kfs_vision.launch.py action_enable:=true`，MF 预选赛仍在 `MfPreselectionFlow` 内部完成 `GRAB_KFS_UP/DOWN` 后的视觉消失确认。
- 2026-06-27 同步：`r2_runtime.yaml` 的 MF 预选赛参数新增 `mf_preselect_second_arm_lower_command_id=14` 与 `mf_preselect_second_arm_lower_done_feedback_id=10`，用于向下夹取在锁定开环距离后、前进前确认第二节机械臂已经放下。
- 2026-06-28 同步：`nav2_params.yaml` 在 local/global costmap 均启用 `inflation_layer`，接在 obstacle/static 层之后；当前膨胀半径为 `0.60m`、代价衰减系数为 `3.0`，用于给静态地图和 `/sensor_scan` 动态障碍留出规划缓冲。
- 2026-06-29 同步：`nav2_bt_mf_preselect_entry_x_positive_y_negative.xml` 的 `FollowPath` 补齐 `goal_checker_id="general_goal_checker"`；梅林预选入口专用 Nav2 BT 仍使用 `MFPreselectEntryXPositiveYNegative` controller 和仅清 costmap 的 recovery，但不会再向 controller server 发送空 goal checker 名称。
- 2026-06-30 同步：`MFPreselectEntryXPositiveYNegative` 取消入口导航阶段的 `linear.x >= 0`、`linear.y <= 0` 硬方向约束，改为低速全向 DWB，以避免局部避障或起点微调时 DWB 全部轨迹无效；入口 BT 仍只做 costmap clear recovery。
- 2026-06-29 同步：Grid heading 的角速度上限拆成 `grid_heading_turn_max_speed_radps` 与 `grid_heading_align_max_speed_radps`；合并角速度参数已从运行配置和决策节点参数声明中删除。
- 2026-06-29 同步：local/global costmap 的 `sensor_scan` observation source 增加 `min_obstacle_height=0.05m`，obstacle layer 继续加载，但过滤 5cm 以下低矮点，避免地面噪声或车身低矮回波直接刷满近场代价图。
- 2026-06-29 同步：完整 `bringup.launch.py use_realsense:=true` 现在默认固定选择 RealSense 序列号 `239222303644`，并显式清空 `camera_namespace`，让 D455 在正式 bringup 下也发布单层 `/camera/...` topic，与 `VisionInferenceManager` 默认订阅和 `rc26_vision/test_kfs_vision.launch.py` 保持一致；该改动只修正相机选择与 topic 装配，不改变模型、KFS 标签或 MF 决策状态机语义。
