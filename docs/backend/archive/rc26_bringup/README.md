...
- `bringup.launch.py`
  ...
  - `r2_runtime.decision.ros__parameters` 同时维护 `stair_*` 台阶动作参数；这些参数只在单独加载 `rc26_decision` 的 `stair_climb_tree.xml` / `stair_descend_tree.xml`，或加载串联台阶动作的 `mf_red_middle_column_tree.xml` 时生效，默认主流程不引用台阶动作。上台阶的前推杆伸出后零速等待、前收+后伸后零速等待，以及下台阶的后推杆伸出后零速等待、后收+前伸后零速等待也在这里配置
