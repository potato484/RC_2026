...
- `bringup.launch.py`
  ...
  - 当前默认运行时在 local/global costmap 加载 obstacle layer 与 inflation layer；`/sensor_scan` (`PointCloud2`) 进入 Nav2 costmap，且不再依赖 `/scan`
...
- `config/nav2_params.yaml` 当前加载 local/global obstacle layer 与 inflation layer；`/sensor_scan` (`PointCloud2`) 作为障碍输入进入 costmap，不再维护 `/scan` LaserScan 兼容链；该文件的实际 YAML 配置项已经逐行补充中文行尾说明，便于现场直接查参数语义
...
- Nav2 obstacle layer 当前在 `config/nav2_params.yaml` 的 local/global costmap `plugins` 中加载，并在其后接入 inflation layer，为静态地图与 `/sensor_scan` 动态障碍生成膨胀缓冲。`/sensor_scan` (`PointCloud2`) 作为障碍输入进入 costmap，`registered_scan` 继续只供定位链使用
