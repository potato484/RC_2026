...
## 关键源码行段速览
- `src/rc26_interfaces/CMakeLists.txt:15-40`：`rosidl_generate_interfaces()` 列出所有被生成的消息、服务和动作，是“接口是否存在”的构建真源。
...
- `action/ExecuteMechanism.action`、`GrabTip.action`、`AssembleWeapon.action`、`PlaceKFSGrid.action`：机构执行类异步契约。
- `srv/SetNavMode.srv`：导航模式切换服务，是 `rc26_nav_mode_manager` 的核心跨包接口。
