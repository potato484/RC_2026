...
- `rc26_omni_controller`
  - 读取 geometry profile，并用 `safety.stop_envelope_half_width_m` 约束局部地形风险采样宽度
...
- 当前静态 `compact` profile 已经被 `rc26_topo_nav` 用于离线 body-aware graph 注解、运行时 body-aware overlay，以及转交给独立 `rc26_surface_body_planner` 做 heading-aware 全车体全局搜索；它不再只是投影半径脚手架。
