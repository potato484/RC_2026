...
Options:
  ...
  --refresh-deps   Force npm ci for src/rc26_topo_nav/sim_viewer.
...
if [[ ! -d "$SIM_VIEWER_DIR/node_modules" || $REFRESH_DEPS -eq 1 ]]; then
  run_logged_step \
    "viewer-deps" \
    "ci" \
    "安装 sim_viewer Node 依赖" \
    "npm --prefix \"$SIM_VIEWER_DIR\" ci"
else
  record_step "viewer-deps" "ci" "安装 sim_viewer Node 依赖" "PASS" "Reused existing node_modules in src/rc26_topo_nav/sim_viewer." ""
fi
...
run_logged_step \
  ...
  "ci" \
  "运行 sim_viewer 单测" \
  "npm --prefix \"$SIM_VIEWER_DIR\" test"
...
run_logged_step \
  "adapter-pycompile" \
  "ci" \
  "校验 topo adapter Python 脚本语法" \
  "python3 -m py_compile src/rc26_topo_nav/scripts/topo_sim_server.py src/rc26_topo_nav/scripts/topo_sim_algorithms.py src/rc26_topo_nav/scripts/render_graph_sim_html.py"
...
record_note "浏览器 E2E 当前使用 docs/test/e2e/topo_sim_stub_server.py，而不是依赖真实 ROS2 / planner_trace_cli。"
record_note "真实 topo_sim_server.py 仍可用于人工联调；本地预演优先保证 CI 可重复和浏览器路径稳定。"
