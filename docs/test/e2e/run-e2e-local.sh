...
FRONTEND_DIR="$ROOT_DIR/src/rc26_topo_nav/sim_viewer"
...
BACKEND_PORT="$(find_available_port "$BACKEND_CHECK_HOST" "$BACKEND_PORT" "Topo Sim E2E Stub 后端")"
FRONTEND_PORT="$(find_available_port "$FRONTEND_CHECK_HOST" "$FRONTEND_PORT" "Topo Sim 前端预览服务")"

if [[ "$BACKEND_PORT" != "$REQUESTED_BACKEND_PORT" ]]; then
  echo "==> Topo Sim E2E Stub 后端端口 ${REQUESTED_BACKEND_PORT} 已被占用，改用 ${BACKEND_PORT}"
fi

if [[ "$FRONTEND_PORT" != "$REQUESTED_FRONTEND_PORT" ]]; then
  echo "==> Topo Sim 前端预览端口 ${REQUESTED_FRONTEND_PORT} 已被占用，改用 ${FRONTEND_PORT}"
fi
...
echo "==> 启动 Topo Sim E2E Stub 后端"
(
  cd "$ROOT_DIR"
  exec python3 docs/test/e2e/topo_sim_stub_server.py --host "$BACKEND_BIND_HOST" --port "$BACKEND_PORT"
) >"$BACKEND_LOG" 2>&1 &
BACKEND_PID=$!

wait_for_http "${BACKEND_BASE_URL}/health" "Topo Sim E2E Stub 后端" "$BACKEND_PID" "$BACKEND_LOG"

rm -rf "$FRONTEND_DIST"

echo "==> 使用 E2E Stub API 构建 sim_viewer"
(
  cd "$FRONTEND_DIR"
  exec env \
...
wait_for_http "${FRONTEND_ORIGIN}/" "sim_viewer 静态预览" "$FRONTEND_PID" "$FRONTEND_LOG"

echo "==> 运行 sim_viewer 浏览器 E2E"
exec env \
  E2E_FRONTEND_URL="$FRONTEND_ORIGIN" \
  E2E_ARTIFACT_DIR="$ARTIFACT_DIR" \
  python3 "$ROOT_DIR/docs/test/e2e/sim_viewer_flow.py"
