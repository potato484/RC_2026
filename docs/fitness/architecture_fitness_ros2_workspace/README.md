...
- **补充口径**：当前默认完整入口由 `r2_active_side.yaml` 的 `preselection_mode` 选择。first 表示武馆+梅林决策族，使用 `mc_mf_preselection_tree.xml`：0x06 分支执行 `0x10/0x0C` 握手后进入 MC 和 MC 后置 MF，0x10 分支同样执行 `0x10/0x0C` 握手后切到 `mf_preselection_tree.xml`。second 表示对抗区决策族，使用 `second_preselection_combo_tree.xml`：0x06 分支执行 `0x11/0x0D` 握手后进入斜坡、90° 转向和 `second_preselection_tree.xml`，0x10 分支同样执行 `0x11/0x0D` 握手后直接切到 `second_preselection_tree.xml`。正式 XML 不再使用 MC 末尾 HSV/背景配准确认 gate。`mf_preselection_tree.xml` 仍作为独立调试入口保留；`grid_heading_tree.xml`、`stair_climb_tree.xml` / `stair_descend_tree.xml` 仍是受限独立运动入口。正式 `MfPreselectionFlow`、MC 视觉伺服、second managed 斜坡/转向和视觉动作测试都会直接发布 `/cmd_vel`；运行这些树或测试前必须停用遥控和其它运动命令权威，失败或 halt 时相关动作只发布零速，不做额外推杆补偿。
