...
- **补充口径**：当前默认完整入口由 `r2_active_side.yaml` 的 `preselection_mode` 选择。first 使用 `mc_mf_preselection_tree.xml`：入口 `WaitPreselectionBranchGate` 的 0x06 分支下发 `COMPETITION_START(0x10)` 并等待同 `seq` 的 `COMPETITION_START_DONE(0x0C)`，延时后执行 `mc_tree.xml`；MC 旋转结束后再次进入 branch gate，0x06 分支继续到 `mf_preselection_after_mc_tree.xml`，任一 gate 收到 0x10 都会先下发 `SECOND_PRESELECTION_START(0x11)`、等待同 `seq` 的 `SECOND_PRESELECTION_START_DONE(0x0D)`，再切到目标树。second 使用 `second_preselection_combo_tree.xml`：入口 gate 后执行 `preselection_ramp_forward_tree.xml` 两段 odom 前进、相对转 90°，再进入 `second_preselection_tree.xml`；入口 gate 收到 0x10 时完成 0x11/0x0D 握手后直接切到 `second_preselection_tree.xml`。正式 XML 不再使用 MC 末尾 HSV/背景配准确认 gate。`mf_preselection_tree.xml` 仍作为独立调试入口保留；`grid_heading_tree.xml`、`stair_climb_tree.xml` / `stair_descend_tree.xml` 仍是受限独立运动入口。正式 `MfPreselectionFlow`、MC 视觉伺服、second managed 斜坡/转向和视觉动作测试都会直接发布 `/cmd_vel`；运行这些树或测试前必须停用遥控和其它运动命令权威，失败或 halt 时相关动作只发布零速，不做额外推杆补偿。
