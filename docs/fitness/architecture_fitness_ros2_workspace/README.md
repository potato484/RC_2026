...
- **补充口径**：当前常规位姿导航运行权威是 `rc26_decision` 的 odom 相对闭环导航。普通导航段由 `OdomDriveX`、`OdomDriveY`、`OdomTurnToYaw` 和按需保留的 `OdomDriveXTurnX` 按具体行为树路线串行发布 `/cmd_vel`；正式 MC 树当前使用旧串行 `OdomDriveX -> RelativeYawTarget -> OdomTurnToYaw -> OdomDriveX` 去程，并在视觉夹取后使用 `RotateInPlace -> OdomDriveX -> OdomDriveY` 退让。受限独立 heading、台阶、MC/MF 视觉对齐等直接 `cmd_vel` 动作必须按 3.13 的命令权威规则串行执行。
...
- **补充口径**：当前默认完整入口由 `r2_active_side.yaml` 的 `preselection_mode` 选择。MCU 上行 `0x06/0x10/0x13` 均来自人工触发的外部限位开关，当前分别作为人工触发外部限位 1/2/3 映射到不同上位机动作。first 使用 `mc_repeat_preselection_tree.xml`，只执行武馆 MC 可重复流程：入口只接受人工触发外部限位 1 上行 `0x06 -> 0x10/0x0C` 后启动 MC，MC 末尾只接受人工触发外部限位 2 上行 `0x10 -> 0x10/0x0C` 作为舵机重新放下握手且不切树，随后等待下一次 `0x06` 重复 MC；第 0 轮 MC 前进距离直接使用当前红/蓝运行配置中的 `mc_nav_forward_x_m`，后续重复轮直接叠加当前 `active_side` 对应的带符号 `first_preselection_mc_repeat_forward_x_step_m`，当前 red first 为 `0.05m -> 0.25m -> 0.45m`，当前 blue first 为 `1.05m -> 0.85m -> 0.65m`。second 表示对抗区决策族，使用 `second_preselection_combo_tree.xml`：人工触发外部限位 1 的 `0x06` 分支先执行 `0x11/0x0D` 握手和斜坡两段 odom 前进，随后等待人工触发外部限位 2 的 `0x10`，再次完成 `0x11/0x0D` 后进入内嵌 `SecondPreselectionTree`；入口直接收到 `0x10` 的分支完成同一握手后直接切到 `second_preselection_tree.xml`，当前组合树不再执行斜坡后 90° 转向。`SecondPreselectionTree` 内部权威流程为：`0x11` start-once fallback -> `SecondPreselectionKfsPickup` 在首次搜索速度发布前记录新鲜 odom 搜索原点，沿 `+X` 搜索/对齐 KFS，执行 `0x14/同 seq 0x12` 放下握手、深度趋近、`0x12/同 seq 0x11` 夹取握手和视觉消失验证 -> `+X 1.5m` -> red `+Y 0.75m` / blue `-Y 0.75m` -> `SecondPreselectionDriveToTotalX` 将沿搜索起始 yaw 的真实 odom 净投影补齐至 `4.2m`，超调不倒退 -> `SecondPreselectionKfsPlacePrepare` 用连续两帧上下双框判定占位，按红蓝镜像先横移 `0.56m`、再次占据反向 `1.10m`，第三次仍占据直接降级放置；清空后等待任意真实深度 KFS，连续 `second_preselect_kfs_approach_timeout_s` 无真实深度时按固定 odom 原点累计 `+X second_preselect_place_no_depth_forward_x_m`。降级前进期间中心 ROI 任意 KFS 单张新帧立即停车并重跑完整占位/避让/对齐流程，多次中断只恢复剩余距离；获得真实深度后取消降级剩余并由 `SecondPreselectionPlaceApproach` 按 `max(0, depth-0.45m)` 正常趋近。初次夹取趋近、无深度等待和最终趋近共用 `second_preselect_kfs_approach_timeout_s`，降级前进走满或达到 `second_preselect_nav_timeout_s` 后直接进入放置 -> 唯一一次首次 ACK-only 下行 `SECOND_PRESELECTION_PLACE_KFS(0x13)` -> `-X 1.5m` -> 放置后上阶。最终放置深度只接受 MF bbox 多 ROI 的 `CenterRoi` / `BboxMultiRoi`，不启用单目估距。`start_r2_auto.sh` 额外启动的上行人工触发外部限位 3 `feedback_id=0x13` 红蓝切换监听器只写回 `r2_active_side.yaml`，只影响下一次启动，不改变 second 树下行放置命令；上下行 `0x13` 分属不同协议方向。第二预选赛九宫格放置观察、动态 ROI 选位和 `selected_lateral` 已从正式流程删除；本地 OpenCV UI 仅为可关闭的调试窗口，覆盖搜索夹取、占位判断、降级中断和任意 KFS 放置对齐，不新增 topic/service/action 或运动命令权威。正式 XML 不再使用 MC 末尾 HSV/背景配准确认 gate，也不再在第二预选赛观察前执行旧 `0x12/0x0F` 高抬节点。`mc_mf_preselection_tree.xml`、`mf_preselection_after_mc_tree.xml` 和 `mf_preselection_tree.xml` 仍作为兼容/独立调试入口保留；`grid_heading_tree.xml`、`stair_climb_tree.xml` / `stair_descend_tree.xml` 仍是受限独立运动入口。正式 `MfPreselectionFlow`、MC 视觉伺服、second managed 斜坡、`SecondPreselectionKfsPickup`、`SecondPreselectionDriveToTotalX`、`SecondPreselectionKfsPlacePrepare`、`SecondPreselectionPlaceApproach` 和视觉动作测试都会直接发布 `/cmd_vel`；运行这些树或测试前必须停用遥控和其它运动命令权威，失败或 halt 时相关动作发布零速，不做额外推杆补偿。
