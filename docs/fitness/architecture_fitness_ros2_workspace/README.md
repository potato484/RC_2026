...
- **补充口径**：当前默认完整入口由 `r2_active_side.yaml` 的 `preselection_mode` 选择。MCU 上行 `0x06/0x10/0x13` 均来自人工触发的外部限位开关，当前分别作为人工触发外部限位 1/2/3 映射到不同上位机动作。first 使用 `mc_repeat_preselection_tree.xml`，只执行武馆 MC 可重复流程：入口只接受人工触发外部限位 1 上行 `0x06 -> 0x10/0x0C` 后启动 MC，MC 末尾只接受人工触发外部限位 2 上行 `0x10 -> 0x10/0x0C` 作为舵机重新放下握手且不切树，随后等待下一次 `0x06` 重复 MC；第 0 轮 MC 前进距离直接使用当前红/蓝运行配置中的 `mc_nav_forward_x_m`，后续重复轮按该值的符号叠加 `abs(first_preselection_mc_repeat_forward_x_step_m)`。second 表示对抗区决策族，使用 `second_preselection_combo_tree.xml`：人工触发外部限位 1 的 0x06 分支执行 `0x11/0x0D` 握手后进入斜坡、90° 转向和 `second_preselection_tree.xml`，人工触发外部限位 2 的 0x10 分支同样执行 `0x11/0x0D` 握手后直接切到 `second_preselection_tree.xml`。`SecondPreselectionTree` 内部当前权威流程为：`0x11` start-once fallback -> `SecondPreselectionKfsPickup` 沿红方基准 `+X` 搜索最近 KFS、视觉横移对齐、发送 `0x14` 并等待同 `seq` 的 `0x12` 放下完成反馈和固定停车等待、odom 前向趋近、发送 `0x12` 夹取触发、等待同 `seq` 的 MCU 上行 `0x11` 夹取完成反馈、再做视觉消失验证 -> 夹取后 `+Y 0.7m`（blue 运行时镜像为负向）-> `+X 4.5m` -> 观察前方 `R1_KFS`，多目标时选择 bbox 中心距相机图像中线最近者并做视觉横移对齐 -> `+X 0.8m` -> 发送 ACK-only 下行 `SECOND_PRESELECTION_PLACE_KFS(0x13)`。`start_r2_auto.sh` 额外启动的上行人工触发外部限位 3 `feedback_id=0x13` 红蓝切换监听器只写回 `r2_active_side.yaml`，只影响下一次启动，不改变 second 树下行放置命令；上下行 0x13 分属不同协议方向。第二预选赛九宫格放置观察、动态 ROI 选位、`selected_lateral` 横移和放置后后退已经从正式流程删除。第二预选赛本地 OpenCV UI 仅为可关闭的调试观察窗口，贯穿 KFS 搜索/放下握手/夹取验证和 R1KFS 放置对齐，不发布 topic/service/action，也不新增运动命令权威。正式 XML 不再使用 MC 末尾 HSV/背景配准确认 gate，也不再在第二预选赛观察前执行旧 `0x12/0x0F` 高抬节点。`mc_mf_preselection_tree.xml`、`mf_preselection_after_mc_tree.xml` 和 `mf_preselection_tree.xml` 仍作为兼容/独立调试入口保留；`grid_heading_tree.xml`、`stair_climb_tree.xml` / `stair_descend_tree.xml` 仍是受限独立运动入口。正式 `MfPreselectionFlow`、MC 视觉伺服、second managed 斜坡/转向、`SecondPreselectionKfsPickup`、`SecondPreselectionR1KfsPlaceAlign` 和视觉动作测试都会直接发布 `/cmd_vel`；运行这些树或测试前必须停用遥控和其它运动命令权威，失败或 halt 时相关动作只发布零速，不做额外推杆补偿。
