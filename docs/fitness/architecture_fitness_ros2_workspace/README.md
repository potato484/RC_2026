...
- **补充口径**：当前完整入口由 `r2_active_side.yaml` 的 `preselection_mode` 选择。MCU 上行 `0x06/0x10/0x13` 均来自人工触发的外部限位开关，分别映射人工触发外部限位 1/2/3。first 使用 `mc_repeat_preselection_tree.xml`，入口 `0x06 -> 0x10/同 seq 0x0C` 后执行 MC，MC 末尾 `0x10 -> 0x10/同 seq 0x0C` 只作为舵机重新放下握手且不切树，再等待下一轮 `0x06`；重复轮距离继续按当前 `active_side` 的带符号 step 派生。second 默认使用 `second_preselection_combo_tree.xml`：入口直接收到 `0x10` 时完成 `0x11/同 seq 0x0D` 后切到 `second_preselection_climb_place_tree.xml`；入口收到 `0x06` 时先完成同一握手并执行合并斜坡、停车等待，再由斜坡后 `0x10 -> 0x11/0x0D` 切到同一目标树。默认目标树先下发唯一一次预装 `0x15` 并只等待通用 ACK，不等待 `0x14`，然后执行 X/Y、20s、人工前激光和后轮 odom 路线；收尾为 `0x0B -> 从上阶完成起累计25s -> 0x13`。当 `second_preselection_kfs_search_compat_enable: true` 时，bringup 改选独立 `second_preselection_kfs_compat_combo_tree.xml`，两条 managed 路径切到 `second_preselection_kfs_compat_climb_place_tree.xml`：切树后按 `0x14/同 seq 0x12 -> 停车0.5s -> OdomDriveX(+1.5m) -> 0x12/同 seq 0x11` 直接完成机械臂放下和 KFS 夹取，不启用视觉搜索、对齐、趋近或消失验证；后续继续默认 Y/20s/上阶路线，收尾为 `0x0B -> 同样累计25s -> 单次0x15(wait_ack=false) -> 固定10s -> 0x13`，不等待 0x15 的 service response、ACK 或 `0x14`。显式 `runtime_config_file` 覆盖 selector 时不自动启用该兼容 profile；`second_preselection_tree.xml` 继续保留为显式独立完整视觉树。所有组合树切树请求仍由 `decision_node` 在当前 tick 后执行，目标树内不重复等待外部限位或发送 `0x11/0x0D`。自动脚本默认不启动 RealSense，需要旧视觉流程时显式启用；启动就绪 `0x20` 只等待 managed gate 与机构 service，且 `0x06/0x10` 先到时跳过。上述导航、斜坡、视觉、台阶和夹取放置动作会串行发布 `/cmd_vel`；运行这些树或测试前必须停用遥控和其它运动命令权威，失败或 halt 时相关动作保持零速停车语义。`start_r2_auto.sh` 的上行人工触发外部限位 3 `feedback_id=0x13` 监听器仍只写回 `active_side` 并影响下一次启动，不改变下行 `0x13` 放置语义。
