...
- **补充口径**：当前常规位姿导航运行权威是 `rc26_decision` 的 odom 相对闭环导航。普通导航段由 `OdomDriveX`、`OdomDriveY`、`OdomTurnToYaw` 和按旧 `X -> yaw -> X` 参数计算终点、再跟踪起终点最短线段的 `OdomDriveXTurnX` 按具体行为树路线串行发布 `/cmd_vel`；受限独立 heading、台阶、MC/MF 视觉对齐等直接 `cmd_vel` 动作必须按 3.13 的命令权威规则串行执行。
