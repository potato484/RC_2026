...
- R2 当前运行时已经统一为麦克纳姆全向底盘。当前里程计与动态 TF 以 Point-LIO 和 `rc26_odom_interface` 为准，底盘速度下发由 `rc26_mcu_transport` 负责，`rc26_telecontrol` 用于人为遥控测试 R2 机器人。

## 执行顺序

在这个根仓库内协作时，默认按以下顺序获取上下文和约束：

1. 先读 `docs/` 下与任务直接相关的入口文档，例如 `docs/backend/README.md`、`docs/fitness/README.md`
2. 再读更具体的模块入口，例如 `docs/backend/archive/<pkg>/README.md`、`docs/fitness/<topic>/README.md`
3. 再进入具体代码目录实施修改与验证
4. 修改完成后，把简短总结与必要的新增模块说明回写到对应 `docs/` 文档，再交付结果

一句话要求：

> 在 `/home/potato/RC_2026` 中做任何实际修改前，先用建立上下文；改完代码后，再把简短总结和必要的模块更新写回对应 `docs/` 文档。
