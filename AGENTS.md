...
本文件面向在 `/home/aidlux/RC_2026` 根仓库内协作的 AI 代理与开发者。
...
- 只要对具体代码做了实际修改，完成实现后就必须把“这次改了什么、为什么这样改、改完后有什么新的注意点或边界”用简短的话语写回 `/home/aidlux/RC_2026/docs` 中与该改动最直接对应的文档和模块。
...
- 编译验证统一使用 `MAKEFLAGS='-j4 -l4' colcon build --parallel-workers 2 --packages-select <pkg...>`；这是当前 AidLux 环境下的默认包级验证命令。若确认是多包独立链路且机器资源仍有余量，再考虑把 `--parallel-workers` 小幅提高到 `3`。
...
> 在 `/home/aidlux/RC_2026` 中做任何实际修改前，先用 `ace-tool` 与 `docs/` 的入口 `README.md` 建立上下文；改完代码后，再把简短总结和必要的模块更新写回对应 `docs/` 文档。
