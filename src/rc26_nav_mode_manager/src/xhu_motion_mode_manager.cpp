#include "rc26_nav_mode_manager/xhu_motion_mode_manager.hpp"
...
XhuMotionModeManager::XhuMotionModeManager(const rclcpp::NodeOptions &options)
  ...
  if (profiles_file.empty()) {
    const auto pkg_dir = ament_index_cpp::get_package_share_directory("rc26_nav_mode_manager");
...
int main(int argc, char **argv) {
  ...
  rclcpp::spin(std::make_shared<rc26_nav_mode_manager::XhuMotionModeManager>());
  rclcpp::shutdown();
  return 0;
}
