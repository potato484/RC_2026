...
rc26_surface_body_planner::SurfaceGraph toBodyPlannerGraph(const FieldGraph& graph) {
    rc26_surface_body_planner::SurfaceGraph planner_graph;
    planner_graph.team = graph.team;
    planner_graph.schema_version = graph.schema_version;
    for (const auto& [node_id, node] : graph.nodes) {
        planner_graph.nodes[node_id] = rc26_surface_body_planner::SurfaceNode{
            node_id,
    ...
    for (const auto& edge : graph.edges) {
        planner_graph.edges.push_back(rc26_surface_body_planner::SurfaceEdge{
            edge.id,
            edge.from,
            edge.to,
            edge.motion_type,
...
std::unordered_map<std::string, rc26_surface_body_planner::NodeOverlay> toBodyPlannerNodeOverlays(
    const std::unordered_map<std::string, NodeOverlay>& node_overlays) {
    std::unordered_map<std::string, rc26_surface_body_planner::NodeOverlay> planner_overlays;
    planner_overlays.reserve(node_overlays.size());
    for (const auto& [node_id, overlay] : node_overlays) {
        planner_overlays[node_id] = rc26_surface_body_planner::NodeOverlay{
...
std::unordered_map<std::string, rc26_surface_body_planner::EdgeOverlay> toBodyPlannerEdgeOverlays(
    const std::unordered_map<std::string, EdgeOverlay>& edge_overlays) {
    std::unordered_map<std::string, rc26_surface_body_planner::EdgeOverlay> planner_overlays;
    planner_overlays.reserve(edge_overlays.size());
    for (const auto& [edge_id, overlay] : edge_overlays) {
        planner_overlays[edge_id] = rc26_surface_body_planner::EdgeOverlay{
...
rc26_surface_body_planner::PlannerWeights toBodyPlannerWeights(const PlannerWeights& weights) {
    rc26_surface_body_planner::PlannerWeights planner_weights;
...
PlanResult convertBodyPlannerResult(
    const FieldGraph& graph,
    const rc26_surface_body_planner::PlanResult& body_result,
    std::string& error) {
    PlanResult result;
...
SurfacePlanResult planSurfaceRoute(
    ...
    if (options.backend == SurfacePlannerBackend::BODY_PLANNER) {
        ...
        rc26_surface_body_planner::PlanRequest request;
        ...
        rc26_surface_body_planner::RobotGeometry geometry;
        geometry.half_length_m = options.geometry->half_length_m;
        geometry.half_width_m = options.geometry->half_width_m;

        const auto body_result = rc26_surface_body_planner::planRoute(
