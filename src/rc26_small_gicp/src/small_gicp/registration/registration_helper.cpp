...
preprocess_points(const PointCloud& points, double downsampling_resolution, int num_neighbors, int num_threads) {
    if (num_threads == 1) {
...
std::pair<PointCloud::Ptr, std::shared_ptr<KdTree<PointCloud>>>
preprocess_points(const std::vector<Eigen::Matrix<T, D, 1>>& points, double downsampling_resolution, int num_neighbors,
                  int num_threads) {
    return preprocess_points(*std::make_shared<PointCloud>(points), downsampling_resolution, num_neighbors,
                             num_threads);
}

// Explicit instantiation
template std::pair<PointCloud::Ptr, std::shared_ptr<KdTree<PointCloud>>>
preprocess_points(const std::vector<Eigen::Matrix<float, 3, 1>>&, double, int, int);
template std::pair<PointCloud::Ptr, std::shared_ptr<KdTree<PointCloud>>>
preprocess_points(const std::vector<Eigen::Matrix<float, 4, 1>>&, double, int, int);
template std::pair<PointCloud::Ptr, std::shared_ptr<KdTree<PointCloud>>>
preprocess_points(const std::vector<Eigen::Matrix<double, 3, 1>>&, double, int, int);
template std::pair<PointCloud::Ptr, std::shared_ptr<KdTree<PointCloud>>>
preprocess_points(const std::vector<Eigen::Matrix<double, 4, 1>>&, double, int, int);
...
RegistrationResult align(const std::vector<Eigen::Matrix<T, D, 1>>& target,
                         const std::vector<Eigen::Matrix<T, D, 1>>& source, const Eigen::Isometry3d& init_T,
                         const RegistrationSetting& setting) {
    auto [target_points, target_tree] = preprocess_points(*std::make_shared<PointCloud>(target),
                                                          setting.downsampling_resolution, 10, setting.num_threads);
    auto [source_points, source_tree] = preprocess_points(*std::make_shared<PointCloud>(source),
                                                          setting.downsampling_resolution, 10, setting.num_threads);
...
template RegistrationResult align(const std::vector<Eigen::Matrix<float, 3, 1>>&,
                                  const std::vector<Eigen::Matrix<float, 3, 1>>&, const Eigen::Isometry3d&,
                                  const RegistrationSetting&);
template RegistrationResult align(const std::vector<Eigen::Matrix<float, 4, 1>>&,
                                  const std::vector<Eigen::Matrix<float, 4, 1>>&, const Eigen::Isometry3d&,
                                  const RegistrationSetting&);
template RegistrationResult align(const std::vector<Eigen::Matrix<double, 3, 1>>&,
                                  const std::vector<Eigen::Matrix<double, 3, 1>>&, const Eigen::Isometry3d&,
                                  const RegistrationSetting&);
template RegistrationResult align(const std::vector<Eigen::Matrix<double, 4, 1>>&,
                                  const std::vector<Eigen::Matrix<double, 4, 1>>&, const Eigen::Isometry3d&,
                                  const RegistrationSetting&);

// Align point clouds
RegistrationResult align(const PointCloud& target, const PointCloud& source, const KdTree<PointCloud>& target_tree,
                         const Eigen::Isometry3d& init_T, const RegistrationSetting& setting) {
    switch (setting.type) {
    default:
    ...
    case RegistrationSetting::VGICP: {
        std::cerr << "error: use align(const GaussianVoxelMap&, const GaussianVoxelMap&, const Eigen::Isometry3d&, "
                     "const RegistrationSetting&) for VGICP"
                  << std::endl;
...
RegistrationResult align(const GaussianVoxelMap& target, const PointCloud& source, const Eigen::Isometry3d& init_T,
                         const RegistrationSetting& setting) {
    if (setting.type != RegistrationSetting::VGICP) {
