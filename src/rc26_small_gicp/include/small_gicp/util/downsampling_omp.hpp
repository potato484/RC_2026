...
///        in a deviation of the number of points in the downsampling results (up to 10% increase from the single-thread
///        version).
...
std::shared_ptr<OutputPointCloud> voxelgrid_sampling_omp(const InputPointCloud& points, double leaf_size,
                                                         int num_threads = 4) {
...
#pragma omp parallel for num_threads(num_threads) schedule(guided, 32)
    ...
    quick_sort_omp(
        coord_pt.begin(), coord_pt.end(), [](const auto& lhs, const auto& rhs) { return lhs.first < rhs.first; },
        num_threads);
