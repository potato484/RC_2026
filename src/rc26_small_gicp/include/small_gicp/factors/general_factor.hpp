...
struct NullFactor {
    ...
    void update_linearized_system(const TargetPointCloud& target, const SourcePointCloud& source,
                                  const TargetTree& target_tree, const Eigen::Isometry3d& T,
                                  Eigen::Matrix<double, 6, 6>* H, Eigen::Matrix<double, 6, 1>* b, double* e) const {}
    ...
    void update_error(const TargetPointCloud& target, const SourcePointCloud& source, const Eigen::Isometry3d& T,
                      double* e) const {}
};

/// @brief Factor to restrict the degrees of freedom of optimization (e.g., fixing roll, pitch rotation).
/// @note  This factor only enables *soft* constraints. The source point cloud can move to the restricted directions
/// slightly).
