...
struct GICPFactor {
    ...
    GICPFactor(const Setting& setting = Setting())
        : target_index(std::numeric_limits<size_t>::max()), source_index(std::numeric_limits<size_t>::max()),
    ...
    bool linearize(const TargetPointCloud& target, const SourcePointCloud& source, const TargetTree& target_tree,
                   const Eigen::Isometry3d& T, size_t source_index, const CorrespondenceRejector& rejector,
                   Eigen::Matrix<double, 6, 6>* H, Eigen::Matrix<double, 6, 1>* b, double* e) {
        //
        ...
        const Eigen::Matrix4d RCR =
            traits::cov(target, k_index) + T.matrix() * traits::cov(source, source_index) * T.matrix().transpose();
        const Eigen::Matrix3d cov_block = RCR.block<3, 3>(0, 0);
        if (!cov_block.allFinite()) {
            return false;
        }
        const Eigen::LDLT<Eigen::Matrix3d> ldlt(cov_block);
        ...
        mahalanobis.setZero();
        mahalanobis.block<3, 3>(0, 0) = ldlt.solve(Eigen::Matrix3d::Identity());
