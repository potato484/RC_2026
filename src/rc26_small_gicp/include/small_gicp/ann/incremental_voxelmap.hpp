...
///        This class supports incremental point cloud insertion and LRU-based voxel deletion that removes voxels that
///        are not recently referenced.
/// @note  This class can be used as a point cloud as well as a neighbor search structure.
/// @note  This class can handle arbitrary number of voxels and arbitrary range of voxel coordinates (in 32-bit int
/// range).
...
public:
    ...
    explicit IncrementalVoxelMap(double leaf_size)
        : inv_leaf_size(1.0 / leaf_size), lru_horizon(100), lru_clear_cycle(10), lru_counter(0) {
    ...
    void insert(const PointCloud& points, const Eigen::Isometry3d& T = Eigen::Isometry3d::Identity()) {
        ...
        for (size_t i = 0; i < traits::size(points); i++) {
            ...
            if (found == voxels.end()) {
                auto voxel = std::make_shared<std::pair<VoxelInfo, VoxelContents>>(VoxelInfo(coord, lru_counter),
                                                                                   VoxelContents());
        ...
        if ((++lru_counter) % lru_clear_cycle == 0) {
            ...
            auto remove_counter =
                std::remove_if(flat_voxels.begin(), flat_voxels.end(),
    ...
    /// @note  1: center only, 7: center + 6 adjacent neighbors (+- 1X/1Y/1Z), 27: center + 26 neighbors (3 x 3 x 3
    /// cube)
    void set_search_offsets(int num_offsets) {
        switch (num_offsets) {
        default:
            std::cerr << "warning: unsupported search_offsets=" << num_offsets << " (supported values: 1, 7, 27)"
                      << std::endl;
        ...
        case 1:
            ...
            break;
        case 7:
            search_offsets = {Eigen::Vector3i(0, 0, 0), Eigen::Vector3i(1, 0, 0),  Eigen::Vector3i(0, 1, 0),
                              Eigen::Vector3i(0, 0, 1), Eigen::Vector3i(-1, 0, 0), Eigen::Vector3i(0, -1, 0),
...
struct Traits<IncrementalVoxelMap<VoxelContents>> {
    ...
    static size_t nearest_neighbor_search(const IncrementalVoxelMap<VoxelContents>& voxelmap, const Eigen::Vector4d& pt,
                                          size_t* k_index, double* k_sq_dist) {
        return voxelmap.nearest_neighbor_search(pt, k_index, k_sq_dist);
    }

    static size_t knn_search(const IncrementalVoxelMap<VoxelContents>& voxelmap, const Eigen::Vector4d& pt, int k,
                             size_t* k_index, double* k_sq_dist) {
