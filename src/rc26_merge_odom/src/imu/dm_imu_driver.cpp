...
bool DmImuDriver::open(const std::string& port, int baudrate) {
    ...
    if (fd_ < 0) {
        ...
        RCLCPP_ERROR(dmImuLogger(), "打开串口失败：open(%s) errno=%d(%s)", port.c_str(), err, std::strerror(err));
        return false;
    }
...
DmImuDriver::Stats DmImuDriver::getStats() const {
    return Stats{cnt_ok_.load(), cnt_crc_.load(), cnt_short_.load(), cnt_nohdr_.load()};
}
...
void DmImuDriver::parseReceivedData() {
    while (recv_len_ >= DM_FRAME_LEN) {
        ...
        for (size_t i = 0; i + 1 < recv_len_; ++i) {
            if (recv_buffer_[i] == DM_FRAME_HEAD_0 && recv_buffer_[i + 1] == DM_FRAME_HEAD_1) {
                head_pos = i;
                found = true;
                break;
            }
...
bool DmImuDriver::parseFrame(const uint8_t* frame, size_t len) {
    ...
    uint16_t crc_wire = static_cast<uint16_t>(frame[16]) | (static_cast<uint16_t>(frame[17]) << 8);
