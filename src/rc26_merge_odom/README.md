...
- 台阶激光测距事件 `FRONT_LASER_HEIGHT_JUMP(0x17)`、`REAR_LASER_HEIGHT_JUMP(0x18)` 与武馆前方限位开关事件 `FRONT_LIMIT_SWITCH_TRIGGERED(0x19)` 也属于普通业务 feedback，桥接层不会过滤，会原样发布到 `/mechanism/command_feedback` 供台阶 BT 动作和武馆视觉前探消费
