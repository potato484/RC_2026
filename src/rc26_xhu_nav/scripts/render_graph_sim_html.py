    ...
    * { box-sizing: border-box; }
    ...
    return "\n".join(
        [
            ...
            "        <p>这页把 `rc26_xhu_nav` 的拓扑导航图、包内保留的 Gazebo 场地资产，以及离线规划过程放在同一坐标系里观察。主视图是仿真世界顶视图；路径代价仍然按当前 planner 的真实逻辑计算，所以你能同时看见场地位置、搜索顺序和高度语义。</p>",
            "      </header>",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render rc26_xhu_nav graph, Gazebo world context, and offline planner trace into one HTML page."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Gazebo world file, typically src/rc26_xhu_nav/topology/sim_assets/worlds/robocon2026_v2_aligned.world",
    )
