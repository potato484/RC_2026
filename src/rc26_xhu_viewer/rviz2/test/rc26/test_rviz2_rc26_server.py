...
def detect_repo_root() -> Path:
    for candidate in (PKG_ROOT, *PKG_ROOT.parents):
        if (candidate / "src" / "rc26_topo_nav").is_dir() and (candidate / "docs").is_dir():
            return candidate
    raise RuntimeError(f"Could not resolve repository root from {PKG_ROOT}")
...
SERVER = load_module("rviz2_rc26_server", PKG_ROOT / "scripts" / "rviz2_rc26_server.py")
