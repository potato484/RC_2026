...
def rviz2_package_path(*parts: str) -> Path:
...
def resolve_topo_nav_render_script() -> Path:
    script_path = topo_nav_package_path("scripts", "render_graph_sim_html.py")
    if script_path.is_file():
        return script_path
    raise FileNotFoundError(f"Could not resolve rc26_topo_nav render script: {script_path}")


RENDER = load_module("render_graph_sim_html", resolve_topo_nav_render_script())
...
def default_field_scene_manifest_file() -> Path:
    return rviz2_package_path("config", "field_scene_manifest.yaml")
...
def detect_local_planner_source_root() -> Path | None:
    candidates: list[Path] = []
    if SOURCE_PKG_ROOT is not None:
        candidates.append(SOURCE_PKG_ROOT.parent / "rc26_local_3d_planner")
    for workspace_root in source_workspace_candidates():
        candidates.append(workspace_root / "src" / "rc26_local_3d_planner")
...
class LiveRosBridge:
    ...
    async def start(self, namespace: str = "") -> dict[str, Any]:
        ...
        self.thread = threading.Thread(target=self._spin, name="xhu-viewer-live-bridge", daemon=True)
        self.thread.start()
        with self._lock:
            snapshot = dict(self.state)
    ...
    def _spin(self) -> None:
        ...
        node.create_subscription(
            OperatorStatus,
            self._topic("/r2/xhu_viewer/operator_status"),
            lambda msg: update_state(
                operatorStatus={
        ...
        node.create_subscription(
            VisualizationEventArray,
            self._topic("/r2/xhu_viewer/events"),
            lambda msg: update_state(
                visualizationEvents=[
                    {
                        "code": event.code,
...
if FRONTEND_DIST.is_dir():
    app.mount("/", StaticFiles(directory=FRONTEND_DIST, html=True), name="rviz2-rc26-web")
...
async def root() -> Any:
    ...
    return {
        "message": "RC26 XHU viewer web platform is running",
