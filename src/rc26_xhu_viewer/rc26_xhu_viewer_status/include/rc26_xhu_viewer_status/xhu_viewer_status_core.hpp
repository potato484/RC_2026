...
public:
  ...
  explicit XhuViewerStatusCore(XhuViewerStatusConfig config = {});

  void setConfig(const XhuViewerStatusConfig& config);
  Output evaluate(const EvaluationInput& input, const std_msgs::msg::Header& header) const;

private:
  XhuViewerStatusConfig config_;
};
