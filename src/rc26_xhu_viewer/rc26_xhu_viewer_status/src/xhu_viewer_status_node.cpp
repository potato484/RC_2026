...
class XhuViewerStatusNode : public rclcpp::Node {
public:
  XhuViewerStatusNode()
      : Node("rc26_xhu_viewer_status_node") {
    declareParameters();
    loadParameters();
    setupPublishers();
    setupServices();
...
private:
  ...
  void setupPublishers() {
    summary_pub_ = this->create_publisher<diagnostic_msgs::msg::DiagnosticArray>("r2/xhu_viewer/summary", 10);
    operator_status_pub_ =
        this->create_publisher<rc26_interfaces::msg::OperatorStatus>("r2/xhu_viewer/operator_status", 10);
    events_pub_ =
        this->create_publisher<rc26_interfaces::msg::VisualizationEventArray>("r2/xhu_viewer/events", 10);
  }

  void setupServices() {
    reset_topic_timeouts_service_ = this->create_service<std_srvs::srv::Trigger>(
        "r2/xhu_viewer/reset_topic_timeout_count",
...
int main(int argc, char** argv) {
  ...
  auto node = std::make_shared<rc26_xhu_viewer::XhuViewerStatusNode>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
