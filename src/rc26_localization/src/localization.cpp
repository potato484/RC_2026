...
void LocalizationNode::performRegistration() {
    ...
    if (global_reloc_running_.load()) {
        RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000, "全局重定位进行中，跳过常规配准");
        return;
    }
    ...
    const double normalized_error = (result.num_inliers > 0) ? (result.error / static_cast<double>(result.num_inliers))
    ...
    if (detectKidnapping(normalized_error)) {
        ...
        global_reloc_thread_ = std::thread(&LocalizationNode::performGlobalRelocalization, this, cloud_to_register);
        return;
    }
    ...
    if (!bad_quality) {
        ...
        if (result.converged) {
            ...
            if (delta_translation.norm() < kMaxDeltaTranslation && delta_rotation < kMaxDeltaRotation) {
                // 偏移在合理范围内，接受结果作为下次初值
...
std::vector<Eigen::Matrix4f> LocalizationNode::generateCandidateTransforms(const Eigen::Matrix4f& sac_transform) {
    ...
    if (yaw_hypotheses < 1) {
        RCLCPP_WARN(this->get_logger(), "num_yaw_hypotheses_=%d 无效，已钳制为 1", yaw_hypotheses);
        yaw_hypotheses = 1;
    }
...
void LocalizationNode::performGlobalRelocalization(pcl::PointCloud<pcl::PointXYZ>::Ptr passed_cloud) {
    ...
    try {
        ...
        RCLCPP_INFO(this->get_logger(), "全局重定位点云: source=%zu, target=%zu", source_down->size(),
                    target_down->size());
        ...
        if (source_down->size() < min_points || target_down->size() < min_points) {
            RCLCPP_WARN(this->get_logger(), "全局重定位失败：点云数量不足 (source=%zu, target=%zu, min=%zu)",
        ...
        if (use_iss_keypoints_) {
            ...
            RCLCPP_INFO(this->get_logger(), "ISS关键点: source=%zu, target=%zu", source_keypoints->size(),
        ...
        for (size_t ci = 0; ci < candidates.size(); ++ci) {
            ...
            if (use_ndt_refinement_) {
                ...
                if (ndt.hasConverged()) {
                    ...
                    RCLCPP_DEBUG(this->get_logger(), "候选%zu NDT收敛, score=%.4f", ci, ndt.getFitnessScore());
                }
            }
            ...
            RCLCPP_INFO(this->get_logger(), "候选%zu ICP: converged=%d, score=%.4f", ci, converged, fitness);
