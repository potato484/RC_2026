...
public:
    XhuMotionRuntime()
        ...
        rc26_local_3d_planner::PlannerConfig planner_config;
        ...
        if (!robot_geometry_file.empty()) {
            std::string error;
            const auto geometry = rc26_local_3d_planner::PlannerCore::loadRobotGeometryProfile(
...
private:
    ...
    void publishPlannerStateLocked(const rc26_local_3d_planner::PlannerResult& result) {
        if (!active_corridor_) {
            return;
        }
    ...
    void controlLoop() {
        ...
        rc26_local_3d_planner::PlannerInput input;
        input.has_pose = true;
    ...
    rc26_local_3d_planner::PlannerCore planner_;
...
int main(int argc, char** argv) {
    ...
    rclcpp::spin(std::make_shared<rc26_omni_controller::XhuMotionRuntime>());
    rclcpp::shutdown();
    return 0;
}
