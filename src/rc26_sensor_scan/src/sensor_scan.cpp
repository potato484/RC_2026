...
SensorScanNode::SensorScanNode(const rclcpp::NodeOptions& options) : Node("sensor_scan", options) {
    ...
    sync_ =
        std::make_unique<message_filters::Synchronizer<SyncPolicy>>(SyncPolicy(100), odometry_sub_, laser_cloud_sub_);
...
std::optional<tf2::Transform> SensorScanNode::getTransform(const std::string& target_frame,
                                                           const std::string& source_frame, const rclcpp::Time& time) {
    try {
        auto transform_stamped = tf_buffer_->lookupTransform(target_frame, source_frame, time,
                                                             rclcpp::Duration::from_seconds(tf_timeout_sec_));
        tf2::Transform transform;
        tf2::fromMsg(transform_stamped.transform, transform);
        return transform;
    } catch (tf2::TransformException& ex) {
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000, "TF lookup failed (%s -> %s): %s",
...
void SensorScanNode::publishTransform(const tf2::Transform& transform, const std::string& parent_frame,
...
void SensorScanNode::publishOdometry(const tf2::Transform& transform, std::string parent_frame,
                                     const std::string& child_frame, const rclcpp::Time& stamp) {
    ...
    if (odom_state_.initialized) {
        ...
        if (dt > 1e-6 && dt < 1.0) {
            ...
            // 角速度计算（含奇异性保护）
            tf2::Quaternion q_diff = transform.getRotation() * odom_state_.previous_transform.getRotation().inverse();
            q_diff.normalize();
            ...
            if (angle > 1e-6) {
                ...
                const auto angular_velocity_body = transform.getRotation().inverse() * angular_velocity_world;
            ...
            RCLCPP_DEBUG(this->get_logger(), "Skip velocity estimation (dt=%.3f out of bounds)", dt);
        }
    }
