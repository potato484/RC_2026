...
/// @brief Voxelgrid downsampling. This function computes exact average of points in each voxel, and each voxel can
/// contain arbitrary number of points.
...
std::shared_ptr<OutputPointCloud> random_sampling(const InputPointCloud& points, size_t num_samples, RNG& rng) {
    ...
    if (num_samples >= indices.size()) {
        std::cerr << "warning: num_samples >= points.size()!! (" << num_samples << " vs " << traits::size(points) << ")"
                  << std::endl;
