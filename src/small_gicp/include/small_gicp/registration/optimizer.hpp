...
struct GaussNewtonOptimizer {
    ...
    template <typename TargetPointCloud, typename SourcePointCloud, typename TargetTree,
              typename CorrespondenceRejector, typename TerminationCriteria, typename Reduction, typename Factor,
              typename GeneralFactor>
    RegistrationResult
    optimize(const TargetPointCloud& target, const SourcePointCloud& source, const TargetTree& target_tree,
             const CorrespondenceRejector& rejector, const TerminationCriteria& criteria, Reduction& reduction,
             const Eigen::Isometry3d& init_T, std::vector<Factor>& factors, GeneralFactor& general_factor) const {
        //
        if (verbose) {
        ...
        for (int i = 0; i < max_iterations && !result.converged; i++) {
            ...
            // Solve linear system
            const auto ldlt = (H + lambda * Eigen::Matrix<double, 6, 6>::Identity()).ldlt();
            if (ldlt.info() != Eigen::Success) {
                if (verbose) {
                    std::cout << "warning: LDLT failed, aborting iteration" << std::endl;
                }
                result.iterations = i;
                result.H = H;
                result.b = b;
            ...
            const Eigen::Matrix<double, 6, 1> delta = ldlt.solve(-b);

            if (verbose) {
                std::cout << "iter=" << i << " e=" << e << " lambda=" << lambda << " dt=" << delta.tail<3>().norm()
        ...
        result.num_inliers =
            std::count_if(factors.begin(), factors.end(), [](const auto& factor) { return factor.inlier(); });
...
struct LevenbergMarquardtOptimizer {
    LevenbergMarquardtOptimizer()
        : verbose(false), max_iterations(20), max_inner_iterations(10), init_lambda(1e-3), lambda_factor(10.0) {}
    ...
    optimize(const TargetPointCloud& target, const SourcePointCloud& source, const TargetTree& target_tree,
        ...
        for (int i = 0; i < max_iterations && !result.converged; i++) {
            ...
            for (int j = 0; j < max_inner_iterations; j++) {
                ...
                if (verbose) {
                    std::cout << "iter=" << i << " inner=" << j << " e=" << e << " new_e=" << new_e
                              << " lambda=" << lambda << " dt=" << delta.tail<3>().norm()
                              << " dr=" << delta.head<3>().norm() << std::endl;
                }
