...
template <typename TargetPointCloud, typename SourcePointCloud, typename TargetTree, typename CorrespondenceRejector,
          typename Factor>
struct LinearizeSum {
    LinearizeSum(const TargetPointCloud& target, const SourcePointCloud& source, const TargetTree& target_tree,
                 const CorrespondenceRejector& rejector, const Eigen::Isometry3d& T, std::vector<Factor>& factors)
        : target(target), source(source), target_tree(target_tree), rejector(rejector), T(T), factors(factors),
          H(Eigen::Matrix<double, 6, 6>::Zero()), b(Eigen::Matrix<double, 6, 1>::Zero()), e(0.0) {}

    LinearizeSum(LinearizeSum& x, tbb::split)
        : target(x.target), source(x.source), target_tree(x.target_tree), rejector(x.rejector), T(x.T),
          factors(x.factors), H(Eigen::Matrix<double, 6, 6>::Zero()), b(Eigen::Matrix<double, 6, 1>::Zero()), e(0.0) {}
...
struct ErrorSum {
    ErrorSum(const TargetPointCloud& target, const SourcePointCloud& source, const Eigen::Isometry3d& T,
             std::vector<Factor>& factors)
        : target(target), source(source), T(T), factors(factors), e(0.0) {}
...
struct ParallelReductionTBB {
    ...
    linearize(const TargetPointCloud& target, const SourcePointCloud& source, const TargetTree& target_tree,
        ...
        //
        LinearizeSum<TargetPointCloud, SourcePointCloud, TargetTree, CorrespondenceRejector, Factor> sum(
            target, source, target_tree, rejector, T, factors);
