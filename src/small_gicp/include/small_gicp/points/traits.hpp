...
/// @brief Get i-th point. 4D vector is used to take advantage of SIMD intrinsics. The last element must be filled by
/// one (x, y, z, 1).
template <typename T>
...
/// @brief Get i-th normal. 4D vector is used to take advantage of SIMD intrinsics. The last element must be filled by
/// zero (nx, ny, nz, 0).
template <typename T>
...
/// @brief Get i-th covariance. Only the top-left 3x3 matrix is filled, and the bottom row and the right col must be
/// filled by zero.
template <typename T>
