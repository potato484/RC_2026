...
std::vector<std::pair<std::string, std::function<OdometryEstimation::Ptr(const OdometryEstimationParams&)>>>
    odometry_registry;

size_t register_odometry(const std::string& name,
                         std::function<OdometryEstimation::Ptr(const OdometryEstimationParams&)> factory) {
...
std::vector<std::string> odometry_names() {
    ...
    std::transform(odometry_registry.begin(), odometry_registry.end(), names.begin(),
                   [](const auto& p) { return p.first; });
    return names;
}

OdometryEstimation::Ptr create_odometry(const std::string& name, const OdometryEstimationParams& params) {
    auto found = std::find_if(odometry_registry.begin(), odometry_registry.end(),
