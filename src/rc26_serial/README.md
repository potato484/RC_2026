...
*   managed second 入口下，人工触发外部限位 1 的上行 `0x06` 与人工触发外部限位 2 的上行 `0x10` 都会先下发 `SECOND_PRESELECTION_START(0x11)` 并等待同 `seq` 的 `SECOND_PRESELECTION_START_DONE(0x0D)`；`0x06` 继续斜坡+转向+对抗区树，`0x10` 切到 `second_preselection_tree.xml`。
