...
*   梅林预选赛到达 2 号入口后会先发送 `ARM_LOWER(0x05)` 并等待 `ARM_LOWER_DONE(0x03)`，再启动入口识别；2 号入口 R2 KFS 夹取使用普通高侧 `GRAB_KFS_UP(0x03)`。入口专用 `ENTRY_GRAB_KFS_UP(0x0F)` / `ENTRY_GRAB_KFS_UP_DONE(0x0B)` 仅用于决策层仍显式启用 `entry_high_protocol` 的入口高侧场景。service ACK 不等同于夹取完成，真正计数仍由决策层视觉消失验证决定。
