...
struct MerlinBlockSummary {
  ...
  int keepout_cells{0};
  bool occupied_layer_available{false};
  bool rule_layer_available{false};
  ...
  bool hasOccupiedLayer() const { return occupied_layer_available; }
  bool hasRuleLayer() const { return rule_layer_available; }
};
...
public:
  ...
  std::vector<int>
  legalGrabTargetsFromEntry(const std::vector<int> &candidates) const;

private:
  static std::array<MerlinBlockSummary, 13> buildSummaryCache(
  ...
  bool hasTraversableEdge(const grid_map::GridMap &map, int from_block,
                          int to_block) const;
  ...
  std::array<MerlinBlockSummary, 13> summary_cache_{};
  ...
  double footprint_radius_m_{0.22};
