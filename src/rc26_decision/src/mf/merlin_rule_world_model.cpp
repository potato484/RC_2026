...
MerlinRuleWorldModel::MerlinRuleWorldModel(rclcpp::Node& node) : node_(node) {
    node_.declare_parameter<std::string>("merlin_rule_world_model.terrain_grid_topic", terrain_grid_topic_);
    node_.declare_parameter<std::string>("merlin_rule_world_model.base_frame", base_frame_);
    node_.declare_parameter<std::string>("merlin_rule_world_model.block_id_layer", block_id_layer_);
    node_.declare_parameter<std::string>("merlin_rule_world_model.block_occupied_layer", block_occupied_layer_);
    node_.declare_parameter<std::string>("merlin_rule_world_model.keepout_layer", keepout_layer_);
    node_.declare_parameter<std::string>("merlin_rule_world_model.traversable_edge_layer", traversable_edge_layer_);
    node_.declare_parameter<std::string>("merlin_rule_world_model.rule_legality_layer", rule_legality_layer_);
    node_.declare_parameter<std::string>("merlin_rule_world_model.traversability_layer", traversability_layer_);
    node_.declare_parameter<std::string>("merlin_rule_world_model.fresh_layer", fresh_layer_);
    node_.declare_parameter<double>("merlin_rule_world_model.tf_timeout_sec", tf_timeout_sec_);
    node_.declare_parameter<double>("merlin_rule_world_model.map_stale_timeout_sec", map_stale_timeout_sec_);

    node_.get_parameter("merlin_rule_world_model.terrain_grid_topic", terrain_grid_topic_);
    node_.get_parameter("merlin_rule_world_model.base_frame", base_frame_);
    node_.get_parameter("merlin_rule_world_model.block_id_layer", block_id_layer_);
    node_.get_parameter("merlin_rule_world_model.block_occupied_layer", block_occupied_layer_);
    node_.get_parameter("merlin_rule_world_model.keepout_layer", keepout_layer_);
    node_.get_parameter("merlin_rule_world_model.traversable_edge_layer", traversable_edge_layer_);
    node_.get_parameter("merlin_rule_world_model.rule_legality_layer", rule_legality_layer_);
    node_.get_parameter("merlin_rule_world_model.traversability_layer", traversability_layer_);
    node_.get_parameter("merlin_rule_world_model.fresh_layer", fresh_layer_);
    node_.get_parameter("merlin_rule_world_model.tf_timeout_sec", tf_timeout_sec_);
    node_.get_parameter("merlin_rule_world_model.map_stale_timeout_sec", map_stale_timeout_sec_);
    ...
    terrain_sub_ = node_.create_subscription<grid_map_msgs::msg::GridMap>(
        ...
        std::bind(&MerlinRuleWorldModel::terrainGridCallback, this, std::placeholders::_1));
}

bool MerlinRuleWorldModel::copyTerrainMap(std::shared_ptr<grid_map::GridMap>& out) const {
...
void MerlinRuleWorldModel::terrainGridCallback(const grid_map_msgs::msg::GridMap::SharedPtr msg) {
    if (!msg) {
        return;
    }
...
bool MerlinRuleWorldModel::resolveCurrentBlock(int& block_id, std::string* reason) const {
    block_id = -1;
    std::shared_ptr<grid_map::GridMap> map;
    if (!copyTerrainMap(map)) {
        if (reason) {
    ...
    try {
        tf_map_base = tf_buffer_->lookupTransform(
            map_frame, base_frame_, rclcpp::Time(0), rclcpp::Duration::from_seconds(tf_timeout_sec_));
    ...
    const grid_map::Position base_pos(
    ...
    if (map->isValid(index, block_id_layer_)) {
        const int id = static_cast<int>(std::lround(map->at(block_id_layer_, index)));
        if (id >= 1 && id <= 12) {
            block_id = id;
            return true;
        }
    ...
    for (int dx = -1; dx <= 1; ++dx) {
        for (int dy = -1; dy <= 1; ++dy) {
            ...
            if (sample(0) < 0 || sample(1) < 0 ||
                sample(0) >= size(0) || sample(1) >= size(1)) {
                continue;
            }
            if (!map->isValid(sample, block_id_layer_)) {
                continue;
            }
            const int id = static_cast<int>(std::lround(map->at(block_id_layer_, sample)));
            if (id >= 1 && id <= 12) {
...
MerlinBlockSummary MerlinRuleWorldModel::summarizeBlock(int block_id) const {
    ...
    std::shared_ptr<grid_map::GridMap> map;
    if (!copyTerrainMap(map) || !map->exists(block_id_layer_)) {
        return summary;
    }

    const bool has_fresh = map->exists(fresh_layer_);
    const bool has_occupied = map->exists(block_occupied_layer_);
    const bool has_keepout = map->exists(keepout_layer_);
    const bool has_rule = map->exists(rule_legality_layer_);
    const bool has_traversability = map->exists(traversability_layer_);
    ...
    for (grid_map::GridMapIterator it(*map); !it.isPastEnd(); ++it) {
        ...
        const int sampled_block = static_cast<int>(std::lround(map->at(block_id_layer_, index)));
        if (sampled_block != block_id) {
            continue;
        }
        ...
        if (has_occupied) {
            if (map->isValid(index, block_occupied_layer_)) {
                if (map->at(block_occupied_layer_, index) >= 0.5F) {
        ...
        if (has_keepout && map->isValid(index, keepout_layer_) &&
            map->at(keepout_layer_, index) >= 0.5F) {
            summary.keepout_cells++;
        }
        if (has_rule && map->isValid(index, rule_legality_layer_)) {
            summary.min_rule_legality =
                std::min(summary.min_rule_legality, map->at(rule_legality_layer_, index));
        }
        if (has_traversability && map->isValid(index, traversability_layer_)) {
            summary.min_traversability =
                std::min(summary.min_traversability, map->at(traversability_layer_, index));
        }
    }
    return summary;
}

TransitionVerdict MerlinRuleWorldModel::canMove(int from_block, int to_block) const {
...
std::vector<int> MerlinRuleWorldModel::legalGrabTargetsFromEntry(
    ...
    for (const int id : candidates) {
        ...
        if (summary.hasUnknownOccupancy() || summary.isOccupied() || summary.hasKeepout()) {
            continue;
        }
...
bool MerlinRuleWorldModel::hasTraversableEdge(const grid_map::GridMap& map,
    ...
    for (grid_map::GridMapIterator it(map); !it.isPastEnd(); ++it) {
        const grid_map::Index index(*it);
        if (has_fresh && map.isValid(index, fresh_layer_) && map.at(fresh_layer_, index) < 0.5F) {
            continue;
        }
        if (!map.isValid(index, block_id_layer_) ||
        ...
        const int center_block = static_cast<int>(std::lround(map.at(block_id_layer_, index)));
        ...
        for (const auto& offset : offsets) {
            ...
            if (neighbor(0) < 0 || neighbor(1) < 0 ||
                neighbor(0) >= size(0) || neighbor(1) >= size(1)) {
                continue;
            }
            if (!map.isValid(neighbor, block_id_layer_)) {
                continue;
            }
            const int nb_block = static_cast<int>(std::lround(map.at(block_id_layer_, neighbor)));
