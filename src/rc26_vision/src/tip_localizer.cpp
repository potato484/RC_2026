#include "rc26_vision/tip_localizer.hpp"
...
TipLocalizer::TipLocalizer(const rclcpp::NodeOptions& options)
    ...
    if (model_path.empty()) {
        ...
        model_path = share + "/models/best.onnx";
    }
    ...
    yolo_ = std::make_unique<YoloEngine>(model_path, class_names, conf_thresh, iou_thresh);
...
void TipLocalizer::onColor(const sensor_msgs::msg::Image::ConstSharedPtr& msg) {
    ...
    const auto detections = yolo_->infer(color);
