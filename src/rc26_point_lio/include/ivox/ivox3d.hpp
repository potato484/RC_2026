...
public:
    ...
    std::unordered_map<KeyType, typename std::list<std::pair<KeyType, NodeType>>::iterator, hash_vec<dim>> grids_map_;
...
template <int dim, IVoxNodeType node_type, typename PointType>
Eigen::Matrix<int, dim, 1> IVox<dim, node_type, PointType>::Pos2Grid_(const IVox::PtType& pt,
...
std::vector<float> IVox<dim, node_type, PointType>::StatGridPoints() const {
    ...
    return std::vector<float>{static_cast<float>(valid_num), ave, static_cast<float>(max), static_cast<float>(min),
                              stddev};
}
