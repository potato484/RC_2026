...
PlannerResult PlannerCore::plan(const PlannerInput& input, PlannerTrace* trace) const {
    ...
    const double lookahead_heading = yawFromQuaternion(lookahead_pose.orientation);
    const double goal_heading = yawFromQuaternion(goal_pose.orientation);
    const double heading_error = normalizeAngle(lookahead_heading - input.robot_yaw);
