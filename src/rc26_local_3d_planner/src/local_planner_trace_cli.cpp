...
int main(int argc, char** argv) {
    ...
    try {
        ...
        rc26_local_3d_planner::PlannerConfig config;
        std::string label;
        std::string error;
        if (!rc26_local_3d_planner::loadSnapshotFile(
                snapshot_file, input, config, label, error)) {
            std::cerr << error << "\n";
            return 1;
        }

        rc26_local_3d_planner::PlannerCore planner(config);
        rc26_local_3d_planner::PlannerTrace trace;
        const auto result = planner.plan(input, &trace);
        rc26_local_3d_planner::printTraceJsonDocument(
